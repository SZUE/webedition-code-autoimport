			...
			if (in_array($tagname, $this->AppListviewItemsTags)) {// for App-Tags of type listviewitems
				...
				switch ($tagname) {
					...
					default :
						...
						switch ($tagname) {
							//FIXME: remove on remove of cache
							case 'ifshopexists':
							case 'ifobjektexists':
							case'ifnewsletterexists':
							case 'ifcustomerexists':
							case 'ifbannerexists':
							case 'ifvotingexists':
							case 'ifHasEntries':
							case 'ifNotHasEntries':
							case 'ifHasCurrentEntry':
							case 'ifNotHasCurrentEntry':
								$code = str_replace(
																$tag,
...
';
				...
				return str_replace($tag, modulFehltError('Object/DB', 'listview type="object"'), $code);
			}
		} else
		if ($type == "customer") {
...
';
				...
				return str_replace($tag, modulFehltError('Customer', 'listview type="customer"'), $code);
			}
		} else
...
';
				...
				return str_replace($tag, modulFehltError('Customer', 'listview type="onlinemonitor"'), $code);
			}
		} else
		if ($type == "order") {
...
';
				...
				return str_replace($tag, modulFehltError('Shop', 'listview type="order"'), $code);
			}
		} else
		if ($type == "orderitem") {
			if (defined("SHOP_TABLE")) {
				...
				if (strpos($orderid, '$') === false) {
					...
					$php.='$orderid = isset(' . $orderid . ') ? "' . $orderid . '" : $GLOBALS["' . str_replace('$', '', $orderid) . '"];';
				}
				$php .= '
							';
...
';
				...
				return str_replace($tag, modulFehltError('Shop', 'listview type="orderitem"'), $code);
			}
		} else
		if ($type == "multiobject") {
			if (defined("OBJECT_TABLE")) {
...
';
				...
				return str_replace($tag, modulFehltError('Object/DB', 'listview type="multiobject"'), $code);
			}
		} else
		if ($type == "banner") {
...
';
				...
				return str_replace($tag, modulFehltError('Banner', 'listview type="banner"'), $code);
			}
		} else
		if ($type == "shopVariant") {
...
';
				...
				return str_replace($tag, modulFehltError('Shop', 'listview type="shopVariant"'), $code);
			}
		} else
		if ($type == "category") {
...
?><?php if($GLOBALS["lv"]->avail): ?>';
		...
		?>';
			...
			return str_replace($tag, modulFehltError('Object/DB', 'object'), $code);
		}
	}
...
?><?php if($GLOBALS["lv"]->avail): ?>';
			...
			return str_replace($tag, modulFehltError('Customer', 'customer'), $code);
		}
	}
...
?><?php if($GLOBALS["lv"]->avail): ?>';
			...
			return str_replace($tag, modulFehltError('Shop', '"order"'), $code);
		}
	}
...
?><?php if($GLOBALS["lv"]->avail): ?>';
			...
			return str_replace($tag, modulFehltError('Shop', '"orderitem"'), $code);
		}
	}
	...
	?>';
			...
			return str_replace($tag, modulFehltError('Shop', '"repeatShopitem"'), $code);
		}
	}
	...
	function parsedeleteShop($tag, $code, $attribs = "") {
		if (defined("SHOP_TABLE")) {
			...
			return str_replace($tag, modulFehltError('Shop', '"deleteShop"'), $code);
		}
	}
	...
	function parsecreateShop($tag, $code, $attribs = "") {
		if (defined("SHOP_TABLE")) {
			...
			return str_replace($tag, modulFehltError('Shop', '"createShop"'), $code);
		}
	}
	...
	function parseadddelShopitem($tag, $code, $attribs = "") {
		if (defined("SHOP_TABLE")) {
			...
			return str_replace($tag, modulFehltError('Shop', '"adddelShopitem"'), $code);
		}
	}
...
';
	...
	function parseVotingTag($tag, $code, $attribs) {
		if (defined("VOTING_TABLE")) {
			...
			return str_replace($tag, modulFehltError('Voting', '"Voting"'), $code);
		}
	}
	...
	function parseVotingListTag($tag, $code, $attribs) {
		if (defined("VOTING_TABLE")) {
			...
			return str_replace($tag, modulFehltError('Voting', '"VotingList"'), $code);
		}
	}
	...
	function parseAnswersTag($tag, $code, $attribs) {
		if (defined("VOTING_TABLE")) {
			...
			return str_replace($tag, modulFehltError('Voting', '"Answer"'), $code);
		}
	}
