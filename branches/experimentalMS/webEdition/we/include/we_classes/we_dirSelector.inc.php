...
');
	...
	function query(){
		$this->db->query('SELECT ' . $this->fields . ' FROM ' . $this->db->escape($this->table) . ' WHERE IsFolder=1 AND ParentID=' . intval($this->dir ? $this->dir : 0) . ' AND((1=1' . makeOwnersSql() . ') ' . getWsQueryForSelector($this->table) . ')' . ($this->order ? (' ORDER BY ' . $this->order) : ''));
	}
...
function addEntry(ID,icon,text,isFolder,path,modDate){
	...
	function printFramesetJSFunctionAddEntries(){
		...
		while($this->next_record()){
			$ret.='addEntry(' . $this->f("ID") . ',"' . $this->f("Icon") . '","' . addcslashes($this->f("Text"), '"') . '",' . ($this->f("IsFolder") ? $this->f("IsFolder") : 0) . ',"' . addcslashes($this->f("Path"), '"') . '","' . date(g_l('date', '[format][default]'), (is_numeric($this->f("ModDate")) ? $this->f("ModDate") : 0)) . '");';
		}t_e("called a", $ret);
	...
	function printCmdAddEntriesHTML(){
		...
		$this->query();
		while($this->next_record()){
			$ret.='top.addEntry(' . $this->f("ID") . ',"' . $this->f("Icon") . '","' . $this->f("Text") . '",' . ($this->f("IsFolder") ? $this->f("IsFolder") : 0) . ',"' . $this->f("Path") . '","' . date(g_l('date', '[format][default]'), (is_numeric($this->f("ModDate")) ? $this->f("ModDate") : 0)) . '");';
		}
		$ret.='top.fsheader.' . ($this->userCanMakeNewDir() ? 'enable' : 'disable') . 'NewFolderBut();';t_e("called b", $ret);
		return $ret;
	}
...
function setRootDir(){
	...
	function printCMDWriteAndFillSelectorHTML(){t_e('b');
		$pid = $this->dir;
		$out = '';
		$c = 0;
		while($pid != 0){
