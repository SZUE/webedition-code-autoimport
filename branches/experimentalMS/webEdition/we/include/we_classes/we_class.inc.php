	...
	protected function i_savePersistentSlotsToDB($felder = ''){
		...
		if(count($fields)){
			...
			if(DB_CONNECT=='msconnect'){
				if($this->wasUpdate){
					$ret = (bool) ($this->DB_WE->query( 'UPDATE '. $this->DB_WE->escape($this->Table) . ' SET ' . we_database_base::arraySetter($fields) . $where) );
				} else {
					$ret = (bool) ($this->DB_WE->query( 'INSERT INTO '. $this->DB_WE->escape($this->Table) . we_database_base::arraySetterINSERT($fields) . $where) );
				}
			} else {
