		...
		if (isset($_REQUEST["we_webUser_logout"]) && $_REQUEST["we_webUser_logout"]) {
			...
			if (!isset($GLOBALS["we_editmode"]) || !$GLOBALS["we_editmode"]) {
						...
						if (isset($u["Password"]) && $u["LoginDenied"] != 1) {
							if ($_REQUEST['s']["Username"] == $u["Username"] && $_REQUEST['s']["Password"] == $u["Password"]) {
								...
								if ($persistentlogins && isset($_REQUEST["s"]["AutoLogin"]) && $_REQUEST["s"]["AutoLogin"] && $_SESSION["webuser"]["AutoLoginDenied"] !=1 ){
									...
									$q= "INSERT INTO " . CUSTOMER_AUTOLOGIN_TABLE . " (AutoLoginID,WebUserID,LastIp,LastLogin) VALUES('".mysql_real_escape_string(sha1($_SESSION["webuser"]["AutoLoginID"]))."','".abs($_SESSION["webuser"]["ID"])."','".htmlspecialchars((string) $_SERVER['REMOTE_ADDR'])."','".time()."')";
					...
					if ($autologinSeek!=''){
						...
						if (isset($a["WebUserID"]) && $a["WebUserID"]){
							...
							if(isset($u["Password"]) && $u["LoginDenied"] != 1 && $u["AutoLoginDenied"] != 1){
								...
								$_SESSION["webuser"]["registered"] = true;
								$_SESSION["webuser"]["AutoLoginID"] = uniqid(hexdec(substr(session_id(), 0, 8)),true);
								$q = "UPDATE ".CUSTOMER_AUTOLOGIN_TABLE." SET AutoLoginID=".mysql_real_escape_string(sha1($_SESSION["webuser"]["AutoLoginID"])).",LastIp=".htmlspecialchars((string) $_SERVER['REMOTE_ADDR']).",LastLogin=".time()." WHERE WebUserID=".abs($_SESSION["webuser"]["ID"])." AND AutoLoginID=".mysql_real_escape_string(sha1($autologinSeek));
								$GLOBALS["DB_WE"]->query($q);
								setcookie("_we_autologin", $_SESSION["webuser"]["AutoLoginID"],(time()+CUSTOMER_AUTOLOGIN_LIFETIME),'/');
							} else {
				...
				if (isset($_SESSION["webuser"]["registered"]) && isset($_SESSION["webuser"]["ID"]) && isset($_SESSION["webuser"]["Username"]) && $_SESSION["webuser"]["registered"] && $_SESSION["webuser"]["ID"] && $_SESSION["webuser"]["Username"]!='') {
					$GLOBALS["DB_WE"]->query("UPDATE " . CUSTOMER_TABLE . " SET LastAccess='" . time() . "' WHERE ID='" . mysql_real_escape_string($_SESSION["webuser"]["ID"]) . "'");
				}
