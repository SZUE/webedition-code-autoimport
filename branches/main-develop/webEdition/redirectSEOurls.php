...
if(! (isset($GLOBALS['we_editmode']) && $GLOBALS['we_editmode'])){
	...
	while($notfound && isset($path_parts['dirname']) && $path_parts['dirname']!='/' && $path_parts['dirname']!='\\'){
		...
		$displayid=intval(f("SELECT DISTINCT ID FROM ".FILE_TABLE." WHERE Path='" . $db->escape($display) . "' LIMIT 1", "ID", $db));
		if ($searchfor){
		...
		if(!$displayid && $hiddendirindex){
			foreach($dirindexarray as $dirindex){
				...
				$displayidtest=intval(f("SELECT DISTINCT ID FROM ".FILE_TABLE." WHERE Path='" . $db->escape($display) . "' LIMIT 1", "ID", $db));
		...
		if($displayid){
			...
			$objectid=intval(f("SELECT DISTINCT ID FROM ".OBJECT_FILES_TABLE." WHERE Url='" . $db->escape($searchforInternal) . "' LIMIT 1", "ID", $db));
			if ($objectid){
				$notfound=false;
			} else {
