...
function header_msg_update(newmsg_count, newtodo_count) {
	...
	if (changed) {
		new (WE().util.jsWindow)(window, WE().consts.dirs.WEBEDITION_DIR + "we_cmd.php?we_cmd[0]=newMsg&msg=" + newmsg_count + "&todo=" + newtodo_count + "&omsg=" + oldMsg + "&otodo=" + oldTodo, "we_delinfo", WE().consts.size.dialog.small, WE().consts.size.dialog.tiny, true, true, true);
	}
}
