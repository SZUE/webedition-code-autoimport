...
(function() {
	...
	//tinymce.PluginManager.requireLangPack('weabbr');
	...
	tinymce.create('tinymce.plugins.WeabbrPlugin', {
		...
		init : function(ed, url) {
			// Register the command so that it can be invoked by using tinyMCE.activeEditor.execCommand('mceWeabbr');
			ed.addCommand('mceWeabbr', function() {

				var se = ed.selection;

				// No selection and not span with attribute lang
				if (se.isCollapsed() && (!ed.dom.getParent(se.getNode(), 'ABBR')))

					return;
				
				ed.windowManager.open({
					file : url + '/../../../../../wysiwyg/abbrDialog.php?we_dialog_args[editor]=tinyMce',
			...
			// Register weabbr button
			ed.addButton('weabbr', {
				title : tinyMceGL.weabbr.tooltip,
				cmd : 'mceWeabbr',
				image : url + '/img/abbr.gif'
			});

			// Add a node change handler, selects the button in the UI when a image is selected
			ed.onNodeChange.add(function(ed, cm, n, co) {
				//cm.setDisabled('weabbr', co && !(n.nodeName == 'SPAN' && n.getAttribute('lang')));
				var active = n.nodeName == 'ABBR';
				cm.setDisabled('weabbr', co && !active);
				cm.setActive('weabbr', active);
			});
		},
