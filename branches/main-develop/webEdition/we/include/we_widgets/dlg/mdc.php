...
$divDynamic = we_html_element::htmlDiv(array("id" => "dynamic", "style" => (!$selection ? 'display:block;' : 'display:none;')
...
$divContent = we_html_element::htmlDiv(array(
		"style" => "display:block;"
			...
			), 1, ($selection ? "static" : "dynamic"), false, array('style' => "width:420px;border:#AAAAAA solid 1px;", 'onchange' => "closeAllSelection();we_submit();"), 'value') .
		we_html_element::htmlBr() .
		we_html_tools::htmlSelect("headerSwitch", $captions, 1, (!$selType ? FILE_TABLE : OBJECT_FILES_TABLE), false, array('style' => "width:420px;border:#AAAAAA solid 1px;margin-top:10px;", 'onchange' => "setHead(this.value);we_submit();"), 'value', 420) .
		$divStatic .
		$divDynamic .
		we_html_element::htmlBr() .
