...
//            $code = self::changeSubmitToButton($code);
		...
		foreach($SEEM_LinkArray[0] as $i => $link){
			...
			if(isset($_SESSION['weS']['we_mode']) && $_SESSION['weS']['we_mode'] == we_base_constants::MODE_SEE && $GLOBALS['we_doc']->EditPageNr == we_base_constants::WE_EDITPAGE_CONTENT){ //	in Super-Easy-Edit-Mode only in Editmode !
				switch($SEEM_LinkArray[2][$i]){
					...
					case 'edit_image':
						$handler = "if(top.edit_include){top.edit_include.close();}top.edit_include=new (WE().util.jsWindow)(window, '" . WEBEDITION_DIR . "we_cmd.php?we_cmd[0]=edit_include_document&we_cmd[1]=" . FILE_TABLE . "&we_cmd[2]=" . $SEEM_LinkArray[1][$i] . "&we_cmd[3]=" . we_base_ContentTypes::IMAGE . "&we_cmd[4]=" . FILE_TABLE . "&we_cmd[5]=" . $SEEM_LinkArray[1][$i] . "&we_cmd[6]=" . $trans . "&we_cmd[7]='" . ",'_blank',WE().consts.size.dialog.medium, WE().consts.size.dialog.small, true, true, true);return true;";
						$code = str_replace($link . '</a>', we_html_button::create_button(we_html_button::EDIT, 'javascript:' . $handler), $code);
						break;
					case 'include':
						//  a new window is opened which stays as long, as the browser is closed, or the window is closed manually
						$handler = "if(top.edit_include){top.edit_include.close();}top.edit_include=new (WE().util.jsWindow)(window,'" . WEBEDITION_DIR . 'we_cmd.php?we_cmd[0]=edit_include_document&we_cmd[1]=' . FILE_TABLE . "&we_cmd[2]=" . $SEEM_LinkArray[1][$i] . "&we_cmd[3]=" . we_base_ContentTypes::WEDOCUMENT . "&we_cmd[4]=" . FILE_TABLE . "&we_cmd[5]=" . $SEEM_LinkArray[1][$i] . "&we_cmd[6]=" . $trans . "&we_cmd[7]='" . ",'_blank',WE().consts.size.dialog.medium, WE().consts.size.dialog.small, true, true, true);return true;";
						$code = str_replace($link . '</a>', we_html_button::create_button('fa:btn_edit_include,fa-lg fa-pencil,fa-lg fa-file-text', 'javascript:' . $handler), $code);
						break;
	...
	static function getJavaScriptCommandForOneLink($link){
		...
		if($linkArray && is_array($linkArray)){
			...
			return self::link2we_cmd($linkArray);
		}
		return '';
	}
	 ...
	 * @return  string
	 */
	private static function link2we_cmd($linkArray){
		...
		if($linkArray[6][0] != -1){
			...
			if($linkArray[3][0] != ""){ //  we have several parameters, deal with them
				...
				if(array_key_exists("we_objectID", $theParameterArray)){ //	target is a object
					return we_base_jsCmd::singleCmd('doClickWithParameters', OBJECT_FILES_TABLE, intval($theParameterArray["we_objectID"]), we_base_ContentTypes::OBJECT_FILE);
				} //	target is a normal file.
				$theParameters = self::arrayToParameters($theParameterArray, "", ['we_cmd']);
				return we_base_jsCmd::singleCmd('doClickWithParameters', FILE_TABLE, $linkArray[6][0], $linkArray[7][0], $theParameters);
			} //	No Parameters
			return we_base_jsCmd::singleCmd('doClickWithParameters', FILE_TABLE, intval($linkArray[6][0]), $linkArray[7][0]);
		...
		if(strpos($linkArray[5][0], "http://") === 0){
			return we_base_jsCmd::singleCmd('seem_open_extern_document', $linkArray[5][0] . $linkArray[3][0]);
		...
		if(strpos($linkArray[5][0], WEBEDITION_DIR . 'we_cmd.php') === 0){
			...
			return (isset($theParameterArray) && is_array($theParameterArray) && array_key_exists("we_objectID", $theParameterArray) ? //	target is a object
				we_base_jsCmd::singleCmd('doClickWithParameters', OBJECT_FILES_TABLE, intval($theParameterArray["we_objectID"]), we_base_ContentTypes::OBJECT_FILE) :
				we_base_jsCmd::singleCmd('doClickWithParameters', FILE_TABLE, $GLOBALS['we_doc']->ID, we_base_ContentTypes::WEDOCUMENT, $theParameters));
		}
		//  we cant save data so we neednt make object
		//	not recognized change of document
		return we_base_jsCmd::singleCmd('doExtClick', $linkArray[5][0] . $linkArray[3][0]);
	}
