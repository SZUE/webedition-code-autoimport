 ...
 */
abstract class we_dialog_wysiwygWindow{
	...
	public static function getDialog(){
		...
		if(isset($fieldName) && we_base_request::_(we_base_request::BOOL, 'we_okpressed')){
			...
			echo we_html_tools::getHtmlTop($fieldName, $fields['charset'], '', we_html_element::jsElement('
try{
...
top.close();', we_html_element::htmlBody(['class' => "weDialogBody"])));
			return;
		}
		...
		$hiddens = '';
		foreach($fields as $v){
			$hiddens.=we_html_element::htmlHidden('we_cmd[' . ($pos++) . ']', $v);
		}
		...
		echo we_html_tools::getHtmlTop($fieldName, $fields['charset'], '', '', we_html_element::htmlBody(['class' => "weDialogBody", 'onload' => "top.focus();"], '<form action="' . $_SERVER['SCRIPT_NAME'] . '" name="we_form" method="post">
			<input type="hidden" name="we_okpressed" value="1" />' .
				...
				we_html_element::htmlDiv(['style' => 'position:absolute;height:40px;bottom:0px;left:0px;right:0px;overflow: hidden;'], we_html_element::htmlDiv(['class' => 'weDialogButtonsBody', 'style' => 'height:100%;'], we_html_button::position_yes_no_cancel(we_html_button::create_button(we_html_button::OK, "javascript:document.we_form.submit();"), we_html_button::create_button(we_html_button::CANCEL, "javascript:top.close()")))) .
				'</form>'
			)
		);
	}
