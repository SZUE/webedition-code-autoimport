 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class we_mail_TransportAbstract{
	/**
	 * Mail body
	 * @var string
	 * @access public
	 ...
	 * on the mail parts present in the Mail object present.
	 *
		...
		if(null !== $boundary){
			...
			if(!$type){
				if($this->_mail->hasAttachments){
					$type = we_mail_mime::MULTIPART_MIXED;
				} elseif($this->_mail->getBodyText() && $this->_mail->getBodyHtml()){
					$type = we_mail_mime::MULTIPART_ALTERNATIVE;
				} else {
					$type = we_mail_mime::MULTIPART_MIXED;
				}
			}
	 ...
	 * @access protected
	 * @return void
	 * @throws we_mail_exception if any header lines exceed 998
	 * characters
	 */
	protected function _prepareHeaders($headers){
		if(!$this->_mail){
			 ...
			 */
			throw new we_mail_exception('Missing Mail object in _mail property');
		}
		...
		if(!$sane){
			 ...
			 * @see Mail_Transport_Exception
			 */
			throw new we_mail_exception('At least one mail header line is too long');
		}
	}
	...
	protected function _buildBody(){
		if(($text = $this->_mail->getBodyText()) && ($html = $this->_mail->getBodyHtml())){
			...
			$mp->type = we_mail_mime::MULTIPART_ALTERNATIVE;
		...
		if(!$body){
			 ...
			 * @see Mail_Transport_Exception
			 */
			throw new we_mail_exception('No body specified');
		}
	 ...
	 *
	 * @param  we_mail_znd $mail
	 * @access public
	 * @return void
	 * @throws we_mail_exception if mail is empty
	 */
	public function send(we_mail_znd $mail){
		...
		if($count < 1){
			 ...
			 * @see Mail_Transport_Exception
			 */
			throw new we_mail_exception('Empty mail cannot be sent');
		}

		if($count > 1){
			// Multipart message; create new MIME object and boundary
			$mime = new we_mail_mime($this->_mail->getMimeBoundary());
		...
		// Create message body
		// This is done so that the same Mail object can be used in
		// multiple transports
		$message = new we_mail_mimeMessage();
