...
/**
 * Smtp implementation of Mail_Protocol_Abstract
 *
 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class we_mail_ProtocolSmtp extends we_mail_ProtocolAbstract{
	/**
	...
	public function __construct($host = '127.0.0.1', $port = null, array $config = []){
		if(isset($config['ssl'])){
			switch(strtolower($config['ssl'])){
				...
				default:
					throw new we_mail_exception($config['ssl'] . ' is unsupported SSL type');
			}
		}
	...
	public function helo($host = '127.0.0.1'){
		...
		if($this->_sess === true){
			throw new we_mail_exception('Cannot issue HELO to existing session');
		}

		// Validate client hostname
		if(!$this->_validHost->isValid($host)){
			throw new we_mail_exception(join(', ', $this->_validHost->getMessages()));
		}
		...
		if($this->_secure == 'tls'){
			...
			if(!stream_socket_enable_crypto($this->_socket, true, STREAM_CRYPTO_METHOD_TLS_CLIENT)){
				throw new we_mail_exception('Unable to connect via TLS');
			}
			$this->_ehlo($host);
		}
	...
	public function mail($from){
		if($this->_sess !== true){
			throw new we_mail_exception('A valid session has not been started');
		}
	...
	public function rcpt($to){
		if($this->_mail !== true){
			throw new we_mail_exception('No sender reverse path has been supplied');
		}
	...
	public function data($data){
		...
		if($this->_rcpt !== true){
			throw new we_mail_exception('No recipient forward path has been supplied');
		}

		$this->_send('DATA');
		$this->_expect(354, 120); // Timeout set for 2 minutes as per RFC 2821 4.5.3.2

		foreach(explode(we_mail_mime::LINEEND, $data) as $line){
			if(strpos($line, '.') === 0){
	 ...
	 *
	 * Not used by Mail, could be used to keep a connection alive or check if it is still open.
	 *
	 * @return void
	 */
	...
	public function auth(){
		if($this->_auth === true){
			throw new we_mail_exception('Already authenticated for this session');
		}
	}
