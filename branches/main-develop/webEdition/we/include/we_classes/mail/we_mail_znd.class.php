 ...
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
class we_mail_znd extends we_mail_mimeMessage{
	/*	 * #@+
	 * @access protected
	 */
	/**
	 * @var we_mail_TransportAbstract
	 * @static
	 */
	 ...
	 * @var string
	 */
	protected $_headerEncoding = we_mail_mime::ENCODING_QUOTEDPRINTABLE;
	 ...
	 * @param  we_mail_TransportAbstract $transport
	 */
	public static function setDefaultTransport(we_mail_TransportAbstract $transport){
	 ...
	 * @return we_mail_znd Implements fluent interface
	...
	public function setType($type){
		...
		if(!in_array($type, $allowed)){
			 ...
			 * @see Mail_Exception
			 */
			throw new we_mail_exception('Invalid content type "' . $type . '"');
		}
	 ...
	 *
	 * Either Mime::ENCODING_QUOTEDPRINTABLE or Mime::ENCODING_BASE64
	 *
	 * @return string
	 */
	 ...
	 * Set the encoding of mail headers
	 *
	 * @param  string $encoding Mime::ENCODING_QUOTEDPRINTABLE or Mime::ENCODING_BASE64
	 * @return we_mail_znd Provides fluent interface
	 */
	public function setHeaderEncoding($encoding){
		...
		if(!in_array($encoding, $allowed)){
			 ...
			 * @see Mail_Exception
			 */
			throw new we_mail_exception('Invalid encoding "' . $encoding . '"');
		}
	 ...
	 * @return we_mail_znd Provides fluent interface
	 */
	public function setBodyText($txt, $charset = null, $encoding = we_mail_mime::ENCODING_QUOTEDPRINTABLE){
		if($charset === null){
			$charset = $this->_charset;
		}

		$mp = new we_mail_mimePart($txt);
		$mp->encoding = $encoding;
		$mp->type = we_mail_mime::TYPE_TEXT;
		$mp->disposition = we_mail_mime::DISPOSITION_INLINE;
		$mp->charset = $charset;
	 ...
	 * @return false|we_mail_mimePart|string
	 */
	 ...
	 * @return we_mail_znd Provides fluent interface
	 */
	public function setBodyHtml($html, $charset = null, $encoding = we_mail_mime::ENCODING_QUOTEDPRINTABLE){
		if($charset === null){
			$charset = $this->_charset;
		}

		$mp = new we_mail_mimePart($html);
		$mp->encoding = $encoding;
		$mp->type = we_mail_mime::TYPE_HTML;
		$mp->disposition = we_mail_mime::DISPOSITION_INLINE;
		$mp->charset = $charset;
	 ...
	 *
	 * @param  we_mail_mimePart $attachment
	 * @return we_mail_znd Provides fluent interface
	 */
	public function addAttachment(we_mail_mimePart $attachment){
	 ...
	 * @return we_mail_mimePart Newly created Mime_Part object (to allow
	 * advanced settings)
	 */
	public function createAttachment($body, $mimeType = we_mail_mime::TYPE_OCTETSTREAM, $disposition = we_mail_mime::DISPOSITION_ATTACHMENT, $encoding = we_mail_mime::ENCODING_BASE64, $filename = null){

		$mp = new we_mail_mimePart($body);
		$mp->encoding = $encoding;
		$mp->type = $mimeType;
	...
	protected function _encodeHeader($value){
		if(we_mail_mime::isPrintable($value) === false){
			if($this->getHeaderEncoding() === we_mail_mime::ENCODING_QUOTEDPRINTABLE){
				$value = we_mail_mime::encodeQuotedPrintableHeader($value, $this->getCharset(), we_mail_mime::LINELENGTH, we_mail_mime::LINEEND);
			} else {
				$value = we_mail_mime::encodeBase64Header($value, $this->getCharset(), we_mail_mime::LINELENGTH, we_mail_mime::LINEEND);
			}
		}
	 ...
	 *
	 * @param string $headerName
	 * @return we_mail_znd Provides fluent inter
	 */
	 ...
	 * @param  string    $name
	 * @return we_mail_znd Provides fluent interface
	 * @throws we_mail_exception if called subsequent times
	 */
	public function setFrom($email, $name = null){
		if(null !== $this->_from){
			 ...
			 * @see Mail_Exception
			 */
			throw new we_mail_exception('From Header set twice');
		}
	 ...
	 * @param string $name
	 * @return we_mail_znd
	 * @throws we_mail_exception if called more than one time
	 */
	public function setReplyTo($email, $name = null){
		if(null !== $this->_replyTo){
			 ...
			 * @see Mail_Exception
			 */
			throw new we_mail_exception('Reply-To Header set twice');
		}
	...
	public function setReplyToFromDefault(){
		...
		if($replyTo === null){
			throw new we_mail_exception('No default Reply-To Address set to use');
		}
	 ...
	 * @param  string    $email
	 * @return we_mail_znd Provides fluent interface
	 * @throws we_mail_exception if set multiple times
	 */
	public function setReturnPath($email){
		if($this->_returnPath === null){
			 ...
			 * @see Mail_Exception
			 */
			throw new we_mail_exception('Return-Path Header set twice');
		}
		return $this;
	}
	...
	public function setSubject($subject){
		if($this->_subject === null){
			 ...
			 * @see Mail_Exception
			 */
			throw new we_mail_exception('Subject set twice');
		}
		return $this;
	}
	 ...
	 * Sets Date-header
	 *
	 * @param  timestamp|string|Date $date
	 * @return we_mail_znd Provides fluent interface
	 * @throws we_mail_exception if called subsequent times or wrong date format.
	 */
		...
		if($this->_date === null){
			if($date === null){
				...
				if($date === false || $date < 0){
					 ...
					 * @see Mail_Exception
					 */
					throw new we_mail_exception('String representations of Date Header must be ' .
				 ...
				 * @see Mail_Exception
				 */
				throw new we_mail_exception(__METHOD__ . ' only accepts UNIX timestamps and strtotime()-compatible strings');
			}
			$this->_date = $date;
			$this->_storeHeader('Date', $date);
		} else {
			throw new we_mail_exception('Date Header set twice');
		}
		return $this;
	}
	...
	public function setMessageId($id = true){
		...
		if($this->_messageId === null){
			...
			throw new we_mail_exception('Message-ID set twice');
		}
	...
	public function send($transport = null){
		if($transport === null){
			if(!self::$_defaultTransport instanceof we_mail_TransportAbstract){
				$transport = new we_mail_TransportSendmail();
			} else {
