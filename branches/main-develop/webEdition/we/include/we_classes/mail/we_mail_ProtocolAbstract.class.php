 ...
 */
abstract class we_mail_ProtocolAbstract{
	/**
	...
	/**
	 * Instance of Validate to check hostnames
	 * @var Validate
	 */
	...
	protected function _connect($remote){
		...
		if($this->_socket === false){
			...
			throw new we_mail_exception($errorStr);
		}

		if(($result = $this->_setStreamTimeout(self::TIMEOUT_CONNECTION)) === false){
			throw new we_mail_exception('Could not set stream timeout');
		}
	...
	protected function _send($request){
		if(!is_resource($this->_socket)){
			throw new we_mail_exception('No connection has been established to ' . $this->_host);
		}
		...
		if($result === false){
			throw new we_mail_exception('Could not send request to ' . $this->_host);
		}
	...
	protected function _receive($timeout = null){
		...
		if(!empty($info['timed_out'])){
			throw new we_mail_exception($this->_host . ' has timed out');
		}

		if($reponse === false){
			throw new we_mail_exception('Could not read from ' . $this->_host);
		}
	 ...
	 * Throws a Mail_Protocol_Exception if an unexpected code is returned.
	 *
	...
	protected function _expect($code, $timeout = null){
		...
		if($errMsg !== ''){
			throw new we_mail_exception($errMsg, $cmd);
		}
