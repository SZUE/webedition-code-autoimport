...
class we_rtf2html{
	...
	function initControlTable(){
		...
		$this->rtf_cons[98] = array("'c", "special", "�", "");
	...
	function parseRtf(){
		...
		while($ch != EOF){
			switch($ch){
				...
				case '}':
					...
					break;
				case '\\':
					if(($this->skip_group > $this->group) || ($this->skip_group == 0)){
						$this->parseControl();
					}
					break;
				case "\r":
		...
		return ($this->group < 0 ? false : true);
	}

	function pushGroup(){

		if($this->first_control == 0){
	...
	function popGroup(){
		while(!empty($this->stack[$this->group])){
			...
			if($key == 87){
				$this->prevFont[$this->group]["index"] = $this->dflFontIndex;
			}
			if($key == 3){
				$this->prevSize[$this->group]["index"] = 0;
			}
			if($key == 94){
				$this->prevColor[$this->group]["index"] = 0;
			}
		}
		if($this->group == $this->skip_group){
			$this->skip_group = 0;
		}
		$this->group--;
	}
	...
	function analyzeControl($control, $para = ""){
		...
		switch($value[1]){
			case "property": $this->propParse($key, $para);
				break;
			case "spec_char": $this->pasteChars($this->rtf_cons[$key][2]);
				break;
			case "jump":
			case "special":
			default:$this->specialParse($key, $para);
				break;
		}

		$this->first_control++;
		if($this->first_control > 1){
			$this->first_control = 0;
		}
