...
//convert 2nd gen values
	...
	private static function movedFiles(){
		if(file_exists(WEBEDITION_PATH . 'agency.php')){
			we_base_file::moveFile(WEBEDITION_PATH . 'agency.php', WEBEDITION_PATH . 'agency.inc.php');
		}
	}
	...
	public static function doUpdate($what = '', array $progress = []){
		...
		self::meassure('start');
		switch($what){
			case 'internal'://if we are in liveupdate, initial db updates already triggered
				self::replayUpdateDB();
