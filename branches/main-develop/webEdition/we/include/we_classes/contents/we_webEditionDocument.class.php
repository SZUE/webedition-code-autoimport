	...
	private function xformTemplatePopup($width = 50){
		...
		if($tlist){
			...
			foreach($tlist as $tid){
				if(($tid == $this->TemplateID) || we_users_util::in_workspace($tid, $ws, TEMPLATES_TABLE, $this->DB_WE)){
					$foo[] = $tid;
				}
			}
...
<table class="default">
	...
	private function getCharsetSelect(){
		if(isset($GLOBALS['meta']['Charset'])){ //	charset-tag available
			...
			$retSelect = we_html_tools::htmlSelect('we_' . $this->Name . '_attrib[Charset]', we_base_charsetHandler::inst()->getCharsetsByArray($chars), 1, $value, false, [
					'onchange' => '_EditorFrame.setEditorIsHot(true);'], 'value');
		} else {
...
//FIXME: maybe use $this->getUsedElements() to unset unused elements?! add setting to do this? check rebuild!
	...
	protected function i_scheduleToBeforeNow(){
		...
		return false; //(defined('SCHEDULE_TABLE') && ($this->To < time()));
	}
...
// tag we:control exists
	...
	public function getPropertyPage(we_base_jsCmd $jsCmd){
		...
		return we_html_multiIconBox::getHTML('PropertyPage', [
				...
				['icon' => we_html_multiIconBox::PROP_META, 'headline' => g_l('weClass', '[metainfo]'), 'html' => $this->formMetaInfos(), 'space' => we_html_multiIconBox::SPACE_ICON],
				...
				['icon' => we_html_multiIconBox::PROP_COPY, 'headline' => g_l('weClass', '[copyWeDoc]'), 'html' => $this->formCopyDocument(), 'space' => we_html_multiIconBox::SPACE_ICON],
