...
function we_tag_showShopItemNumber(array $attribs){
	...
	$attr = removeAttribs($attribs, ['option', 'inputfield', 'type', 'start', 'stop', 'shopname', 'nameto', 'to', 'floatquantities', 'num_format']);
	...
	$itemQuantity = (isset($GLOBALS['lv']) && isset($GLOBALS['lv']->ShoppingCartKey) ?
			$GLOBALS[$shopname]->getItemQuantity($GLOBALS['lv']->ShoppingCartKey) :
			0);
	...
	if($option || ($type === 'select')){
		...
		while($start <= $stop){
			$out .= ($itemQuantity == $start ?
					getHtmlTag('option', ['xml' => $xml, 'value' => $start, 'selected' => 'selected'], $start) :
					getHtmlTag('option', ['xml' => $xml, 'value' => $start], $start));

			$start = $start + $step;
		}
		return getHtmlTag('select', $attr, $out, true) . getHtmlTag('input', ['type' => 'hidden', 'name' => 't', 'value' => time()]);
	}
	if($inputfield || ($type === 'textinput')){
		...
		$attr = array_merge($attr, ['type' => 'text', 'name' => 'shop_cart_id[' . $GLOBALS['lv']->ShoppingCartKey . ']', 'size' => 2, 'value' => $itemQuantity]);
		return getHtmlTag('input', $attr) . getHtmlTag('input', ['type' => 'hidden', 'name' => 't', 'value' => time()]);
	}
