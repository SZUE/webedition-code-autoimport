...
function we_tag_href($attribs){
	...
	switch($type){
		case we_base_link::TYPE_ALL:
			...
			$yuiSuggest->setAcId($intID . 'selector');
...
//				'<input type="hidden" name="' . $ext_elem_Name . '" /><input type="hidden" name="' . $intID_elem_Name . '" value="' . $intID . '" /><input type="text" name="' . $intPath_elem_Name . '" value="' . $intPath . '" ' . $attr . ' readonly /></td>'.
		...
		case we_base_link::TYPE_INT:
			...
			$but = ((($directory && $file) || $file) ?
					we_button::create_button(we_button::WE_IMAGE_BUTTON_IDENTIFY . 'edit_link', "javascript:we_cmd('openDocselector', document.forms[0].elements['$intID_elem_Name'].value, '" . FILE_TABLE . "','" . $wecmdenc1 . "','" . $wecmdenc2 . "','" . $wecmdenc3 . "','" . session_id() . "', '" . $rootdirid . "', '', " . (permissionhandler::hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ",''," . ($directory ? 1 : 0) . ");") :
...
//				'<input type="hidden" name="' . $ext_elem_Name . '" /><input type="hidden" name="' . $intID_elem_Name . '" value="' . $intID . '" /><input type="text" name="' . $intPath_elem_Name . '" value="' . $intPath . '" ' . $attr . ' readonly /></td>'.
		...
		case we_base_link::TYPE_EXT:
			...
			$but2 = (permissionhandler::hasPerm("CAN_SELECT_EXTERNAL_FILES") ?
					((($directory && $file) || $file) ?
