...
function we_tag_img(array $attribs){
	...
	$tagAttribs = removeAttribs($attribs, ['id', 'showcontrol', 'showthumbcontrol', 'showimage', 'showinputs', 'startid', 'parentid']);
	...
	if($GLOBALS['we_editmode'] && !$showimage){
		...
		$tagAttribs = removeAttribs($tagAttribs, ['thumbnail', 'only', 'name']);
...
//	$dropzoneCmdencExt = we_base_request::encCmd($btnSelectCallback . 'setTimeout(self.close, 250);');
	...
	if($GLOBALS['we_editmode'] && $out && $showcontrol){ //in editMode we surround image with dropzone
		$out = we_fileupload_ui_base::getExternalDropZone($GLOBALS['we_doc']->Name . '_' . $name, $out, 'width:auto;height:auto;padding:12px;', true, true, 'dd_weimg_insertFromTree,' . $name . ',' . $fname, 'dd_weimg_insertFromExt,' . $name . ',' . $fname, [we_base_ContentTypes::IMAGE]);
	}
...
<table class="weEditTable padding2 spacing2">
	...
	<tr><td class="weEditmodeStyle" colspan="2" style="text-align:center;width: 180px;">
		<table class="weEditTable padding0 spacing0 border0">
			<tr>
				<td class="weEditmodeStyle" style="color: black; font-size: 12px;">' . g_l('weClass', '[alt_kurz]') . ':&nbsp;</td>
			...
			<tr>
				<td class="weEditmodeStyle" style="color: black; font-size: 12px;">' . g_l('weClass', '[Title]') . ':&nbsp;</td>
	...
	<tr><td class="weEditmodeStyle" colspan="2" style="text-align:center;width: 180px;">
		<table class="weEditTable padding0 spacing0 border0">
			<tr>
				<td class="weEditmodeStyle" style="color: black; font-size: 12px;">' . g_l('weClass', '[thumbnails]') . ':&nbsp;</td>
