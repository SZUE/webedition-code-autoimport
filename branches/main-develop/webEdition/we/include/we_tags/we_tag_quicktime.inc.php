...
function we_tag_quicktime($attribs){
	...
	if($showcontrol && $GLOBALS['we_editmode']){
		...
		$quicktime_button = we_button::create_button(
				"image:btn_edit_quicktime", "javascript:we_cmd('openDocselector','" . ($id != "" ? $id : $startid) . "', '" . FILE_TABLE . "','" . $wecmdenc1 . "','','" . $wecmdenc3 . "','" . session_id() . "'," . $parentid . ", 'video/quicktime', " . (we_hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")", true);
