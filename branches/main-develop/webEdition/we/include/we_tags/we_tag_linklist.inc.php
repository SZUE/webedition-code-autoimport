...
function we_parse_tag_linklist($attribs, $content){
	return '<?php $GLOBALS[\'we_ll\']=' . we_tag_tagParser::printTag('linklist', $attribs) . '; while($ll->next()){?>' . $content . '<?php } $ll->last(); unset($GLOBALS[\'we_ll\']);' . we_tag_tagParser::printTag('linklist', array('_type' => 'stop')) . ';?>';
}
