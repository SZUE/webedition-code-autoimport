...
function we_tag_flashmovie($attribs){
	...
	if($showcontrol && $GLOBALS['we_editmode']){
		...
		$flash_button = we_button::create_button("image:btn_edit_flash", "javascript:we_cmd('openDocselector','" . ($id != "" ? $id : $startid) . "', '" . FILE_TABLE . "', '" . $wecmdenc1 . "','','" . $wecmdenc3 . "','" . session_id() . "'," . $parentid . ", 'application/x-shockwave-flash', " . (we_hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")", true);
