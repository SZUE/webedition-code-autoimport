...
function we_tag_flashmovie($attribs){
	...
	if($showcontrol && $GLOBALS['we_editmode']){
		...
		$button = we_html_button::create_button('image:btn_edit_flash', "javascript:we_cmd('openDocselector','" . ($id ? : $startid) . "', '" . FILE_TABLE . "','" . $wecmdenc1 . "','','" . $wecmdenc3 . "',''," . $parentid . ", '" . we_base_ContentTypes::FLASH . "', " . (permissionhandler::hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")", true);
