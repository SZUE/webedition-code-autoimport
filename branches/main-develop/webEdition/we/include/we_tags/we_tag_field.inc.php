...
function we_tag_field(array $attribs){
	...
	switch($type){
		...
		case 'week' :
			...
			$out = we_listview_base::getCalendarField($GLOBALS['lv']->calendar_struct['calendar'], $type, $langcode);
			break;
		...
		if(isset($GLOBALS['we_doc']->OF_ID) && ($GLOBALS['we_doc']->InWebEdition)){
			...
			if($triggerid){
				...
				$linkAttribs['href'] = (!empty($GLOBALS['lv']->objectseourls)) ? // objectseourls=true
					(empty($triggerpath_parts['dirname']) ? '' : rtrim($triggerpath_parts['dirname'], '/') ) . '/' .
