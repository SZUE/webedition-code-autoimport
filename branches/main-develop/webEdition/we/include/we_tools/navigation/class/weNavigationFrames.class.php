...
class weNavigationFrames extends weToolFrames{
	...
	function getHTMLEditorHeader(){
		...
		$body = we_html_element::htmlBody(
				...
				"onresize" => "setFrameSize()"
					...
					100, 3) . $we_tabs->getHTML() . '</div>' . we_html_element::jsElement($extraJS))//			$js.
		//			$table->getHtml() .
		//			$tabsBody
		;
	...
	function getHTMLGeneral(){
		...
		if(isset($this->Model->Text) && $this->Model->Text != '' && isset($this->Model->ID) && $this->Model->ID != ''){
			$_parentid = f(
				'SELECT ParentID FROM ' . NAVIGATION_TABLE . ' WHERE ID=' . intval($this->Model->ID), 'ParentID', $this->db);
		...
		$_table->setColContent(
			0, 2, we_button::create_button_table(
				...
				we_button::create_button(
					'image:direction_up', 'javascript:' . $this->topFrame . '.we_cmd("move_up");', true, 100, 22, '', '', (($this->Model->Ordn > 0) ? false : true), false),
				we_button::create_button(
					'image:direction_down', 'javascript:' . $this->topFrame . '.we_cmd("move_down");', true, 100, 22, '', '', (($this->Model->Ordn < ($_num - 1)) ? false : true), false)
				), 10, array(
	...
	function getHTMLPropertiesGroup(){
		...
		if($this->Model->LinkID == 0){
			...
			$_path = id_to_path(
				$this->Model->LinkID, ($this->Model->FolderSelection == 'docLink' ? FILE_TABLE : (defined('OBJECT_TABLE') && $this->Model->FolderSelection == 'objLink' ? OBJECT_FILES_TABLE : FILE_TABLE)));
		}
		$_seltype = array(
			'docLink' => g_l('navigation', '[docLink]'), 'urlLink' => g_l('navigation', '[urlLink]')
		);
	...
	function getHTMLDependedProfile(){
		...
		$_table->setColContent(
			1, 0, ($this->Model->SelectionType == 'catLink' ? g_l('navigation', '[catLink]') : ($this->Model->SelectionType == 'docLink' ? g_l('navigation', '[docLink]') : g_l('navigation', '[objLink]'))) . ':');

		$_table->setColContent(
			1, 1, id_to_path(
				$this->Model->LinkID, ($this->Model->SelectionType == 'catLink' ? CATEGORY_TABLE : ($this->Model->SelectionType == 'docLink' ? FILE_TABLE : OBJECT_FILES_TABLE))));
	...
	function getHTMLDynamic(){
			...
			while($this->db->next_record()) {
				if(in_array($this->db->f('ID'), $allowedClasses)){
					...
					$objectPaths[] = $this->db->f('ID') . ':"' . f(
							'SELECT * FROM ' . OBJECT_FILES_TABLE . ' WHERE ID=' . intval($this->db->f('ParentID')), 'Path', new DB_WE()) . '"';
				}
			}
		}
		...
		<div style="display: block;">
			...
			' . $this->getHTMLFieldChooser(
				g_l('navigation', '[title_field]'), 'TitleField', $this->Model->TitleField, 'putTitleField', $this->Model->SelectionType, ($this->Model->SelectionType == 'classname' ? $this->Model->ClassID : $this->Model->DocTypeID)) . '
			</div>
			...
			' . $this->getHTMLFieldChooser(
				g_l('navigation', '[sort]'), 'SortField', $_sortVal, 'putSortField', $this->Model->SelectionType, ($this->Model->SelectionType == 'classname' ? $this->Model->ClassID : $this->Model->DocTypeID), $_sortSelect, 120) . '
			</div>
			...
			' . we_html_tools::htmlFormElementTable(
				we_html_tools::htmlTextInput(
					'dynamic_Parameter', 58, $this->Model->Parameter, '', 'onchange="' . $this->topFrame . '.mark();"', 'text', $this->_width_size, 0), g_l('navigation', '[parameter]')) . '
			</div>
			' . $this->getHTMLCount() . '
	...
	function getHTMLStatic($disabled = false){
		...
		if($this->Model->LinkID == 0){
			...
			$_path = id_to_path(
				$this->Model->LinkID, ($this->Model->SelectionType == 'docLink' ? FILE_TABLE : ($this->Model->SelectionType == 'catLink' ? CATEGORY_TABLE : (defined(
	...
	function getHTMLContentInfo(){
		...
		if($this->Model->SelectionType == 'doctype'){
			...
			if(!empty($this->Model->DocTypeID)){
				$_dt = f(
					'SELECT DocType FROM ' . DOC_TYPES_TABLE . ' WHERE ID=' . intval($this->Model->DocTypeID), 'DocType', new DB_WE());
		...
		if($this->Model->SelectionType == 'category'){
			...
			$_cn = f(
				'SELECT Text FROM ' . OBJECT_TABLE . ' WHERE ID=' . intval($this->Model->ClassID), 'Text', new DB_WE());
			$_table->setCol(1, 0, array(
				'style' => 'font-weight: bold;'
			...
			'html' => we_button::create_button_table(
				array(
					we_button::create_button(
						'preview', 'javascript:' . $this->topFrame . '.we_cmd("dyn_preview");'),
					we_button::create_button(
						'refresh', 'javascript:' . $this->topFrame . '.we_cmd("populate");'),
					we_button::create_button(
						'delete_all', 'javascript:' . $this->topFrame . '.we_cmd("depopulate");')
			)),
			'space' => $this->_space_size
	...
	if(document.we_form.Selection.value=="static"){
		' .
				(defined("OBJECT_TABLE") ?
					'YAHOO.autocoml.modifySetById("yuiAcInputLinkPath","table",(elem.value=="docLink" || elem.value=="doclink" ? "' . FILE_TABLE . '":(elem.value=="objLink" || elem.value=="classname" ? "' . OBJECT_TABLE . '":"' . CATEGORY_TABLE . '")));' :
					'YAHOO.autocoml.modifySetById("yuiAcInputLinkPath","table",(elem.value=="docLink" || elem.value=="doclink"? "' . FILE_TABLE . '":"' . CATEGORY_TABLE . '"));'
				) . '
	} else {
		' . (defined("OBJECT_FILES_TABLE") ?
					'YAHOO.autocoml.modifySetById("yuiAcInputFolderPath","table",(elem.value=="doctype"?"' . FILE_TABLE . '":(elem.value=="classname"?"' . OBJECT_FILES_TABLE . '":"' . CATEGORY_TABLE . '")));' :
					'YAHOO.autocoml.modifySetById("yuiAcInputFolderPath","table",(elem.value=="doctype"?"' . FILE_TABLE . '":"' . CATEGORY_TABLE . '"));'
				) . '
	}
}
function onFolderSelectionChangeJS(elem) {
			...
			$out .= $yuiSuggest->getYuiJsFiles() .
				we_html_element::htmlDiv(
					...
					($this->Model->IsFolder ? we_multiIconBox::getHTML(
							...
							'', '100%', $this->getHTMLPropertiesItem(), 30, '', -1, '', '', false, $preselect)) . (($this->Model->Selection == 'static' || $this->Model->IsFolder) ? $this->getHTMLAttributes() : '')) . ($this->Model->IsFolder ? (we_html_element::htmlDiv(
						array(
	...
	function getHTMLCategory(){
		$addbut = we_button::create_button(
				"add", "javascript:we_cmd('openCatselector','','" . CATEGORY_TABLE . "','','','fillIDs();opener.addCat(top.allPaths);opener." . $this->topFrame . ".mark();')");
		$del_but = addslashes(
	...
	function getHTMLFieldSelector(){
		...
		if($_type == 'doctype'){
			...
			$_fields = array();
			$_templates = f(
				'SELECT Templates FROM ' . DOC_TYPES_TABLE . ' WHERE ID=' . intval($_selection), 'Templates', $_db);
	...
	function getHTMLDirSelector(){
		...
		$_button_cat = we_button::create_button(
				'select', "javascript:we_cmd('openCatselector',document.we_form.elements['FolderID'].value,'" . CATEGORY_TABLE . "','document.we_form.elements[\\'FolderID\\'].value','document.we_form.elements[\\'FolderPath\\'].value','opener." . $this->topFrame . ".mark();','" . session_id() . "','$rootDirID')");
		...
		$_path = id_to_path(
			$this->Model->FolderID, (($this->Model->SelectionType == 'doctype') ? FILE_TABLE : ($this->Model->SelectionType == 'category' ? CATEGORY_TABLE : (defined(
						'OBJECT_FILES_TABLE') ? OBJECT_FILES_TABLE : ''))));
	...
	function getHTMLDynPreview(){
		...
		foreach($_items as $_k => $_item){
			$_txt = id_to_path(
				$_item['id'], ($this->Model->SelectionType == 'doctype') ? FILE_TABLE : ($this->Model->SelectionType == 'category' ? CATEGORY_TABLE : OBJECT_FILES_TABLE));
		...
		$_parts[] = array(
			...
			'html' => we_html_tools::htmlFormElementTable(
				$_select->getHtml(), $this->Model->SelectionType == 'category' ? g_l('navigation', '[categories]') : ($this->Model->SelectionType == 'classname' ? g_l('navigation', '[objects]') : g_l('navigation', '[documents]'))),
			'space' => 0
		);
	...
	function getLangField($name, $value, $title, $width){
		$input = we_html_tools::htmlTextInput(
				"Attributes[$name]", 15, $value, "", 'onchange=' . $this->topFrame . '.mark(); ', "text", $width - 100);
	...
	function getHTMLAttributes(){
		...
		$_target = we_html_tools::htmlFormElementTable(
				we_html_tools::targetBox(
					'Attributes[target]', 30, ($this->_width_size - 100), '', $this->Model->getAttribute('target'), '' . $this->topFrame . '.mark();', 100), g_l('navigation', '[target]'));
		...
		$_popup->setCol(
			1, 0, array(), we_html_tools::htmlFormElementTable(
				we_html_tools::htmlTextInput(
		...
		$_popup->setCol(
			1, 1, array(), we_html_tools::htmlFormElementTable(
				we_html_tools::htmlTextInput(
		...
		$_popup->setCol(
			1, 2, array(), we_html_tools::htmlFormElementTable(
				we_html_tools::htmlTextInput(
		...
		$_popup->setCol(
			1, 3, array(), we_html_tools::htmlFormElementTable(
				we_html_tools::htmlTextInput(
		...
		$_popup->setCol(
			2, 0, array(), we_forms::checkboxWithHidden(
				$this->Model->getAttribute('popup_status'), 'Attributes[popup_status]', g_l('navigation', '[popup_status]'), false, "defaultfont", $this->topFrame . '.mark();"'));
		$_popup->setCol(
			2, 1, array(), we_forms::checkboxWithHidden(
				$this->Model->getAttribute('popup_scrollbars'), 'Attributes[popup_scrollbars]', g_l('navigation', '[popup_scrollbars]'), false, "defaultfont", $this->topFrame . '.mark();"'));
		$_popup->setCol(
			2, 2, array(), we_forms::checkboxWithHidden(
				$this->Model->getAttribute('popup_menubar'), 'Attributes[popup_menubar]', g_l('navigation', '[popup_menubar]'), false, "defaultfont", $this->topFrame . '.mark();"'));

		$_popup->setCol(
			3, 0, array(), we_forms::checkboxWithHidden(
				$this->Model->getAttribute('popup_resizable'), 'Attributes[popup_resizable]', g_l('navigation', '[popup_resizable]'), false, "defaultfont", $this->topFrame . '.mark();"'));
		$_popup->setCol(
			3, 1, array(), we_forms::checkboxWithHidden(
				$this->Model->getAttribute('popup_location'), 'Attributes[popup_location]', g_l('navigation', '[popup_location]'), false, "defaultfont", $this->topFrame . '.mark();"'));
		$_popup->setCol(
			3, 2, array(), we_forms::checkboxWithHidden(
				$this->Model->getAttribute('popup_toolbar'), 'Attributes[popup_toolbar]', g_l('navigation', '[popup_toolbar]'), false, "defaultfont", $this->topFrame . '.mark();"'));
