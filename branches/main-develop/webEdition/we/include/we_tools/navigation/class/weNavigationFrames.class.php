...
class weNavigationFrames extends weToolFrames
{
	...
	function getHTMLEditorHeader()
	{
		...
		if ($this->Model->IsFolder) {
			$we_tabs->addTab(
					new we_tab(
							"#",
							g_l('navigation','[preview]'),
							'((' . $this->topFrame . '.activ_tab=="preview") ? TAB_ACTIVE : TAB_NORMAL)',
							"setTab('preview');",
							array(
								"id" => "tab_preview"
							)));
		...
		$table = new we_htmlTable(array(
			"width" => "3000", "cellpadding" => "0", "cellspacing" => "0", "border" => "0"
		), 3, 1);
		...
		$table->setCol(
				...
				getPixel(15, 2) . we_htmlElement::htmlB(
						($this->Model->IsFolder ? g_l('navigation','[group]') :g_l('navigation','[entry]')) . ':&nbsp;' . str_replace(
								'&amp;',
								'&',
								$this->Model->Text) . '<div id="mark" style="display: none;">*</div>' . we_htmlElement::htmlImg(
								array(
		...
		$body = we_htmlElement::htmlBody(
				...
				'<div id="main" >' . getPixel(100, 3) . '<div style="margin:0px;" id="headrow">&nbsp;' . we_htmlElement::htmlB(
						($this->Model->IsFolder ? g_l('navigation','[group]') :g_l('navigation','[entry]')) . ':&nbsp;' . str_replace(
								'&amp;',
								'&',
								$this->Model->Text) . '<div id="mark" style="display: none;">*</div>') . '</div>' . getPixel(
						100,
	...
	function getHTMLGeneral()
	{
		...
		$_table->setColContent(
				...
				htmlTextInput(
						...
						($this->Model->Ordn + 1),
						'',
						'disabled="true" readonly style="width: 35px" onBlur="var r=parseInt(this.value);if(isNaN(r)) this.value=' . $this->Model->Ordn . '; else{ this.value=r; ' . $this->topFrame . '.mark();}"'));
				...
				$we_button->create_button_table(
						array(
								...
								$we_button->create_button(
										'image:direction_up',
										'javascript:' . $this->topFrame . '.we_cmd("move_up");',
										true,
										100,
										22,
										'',
										'',
										(($this->Model->Ordn > 0) ? false : true),
										false),
								$we_button->create_button(
										'image:direction_down',
										'javascript:' . $this->topFrame . '.we_cmd("move_down");',
										true,
										100,
										22,
										'',
										'',
										(($this->Model->Ordn < ($_num - 1)) ? false : true),
										false)
						),
						10,
						array(
		...
		array_push(
				...
				array(
						...
						'html' => we_htmlElement::htmlHidden(
								array(
								...
								htmlTextInput(
										...
										'',
										'style="width: ' . $this->_width_size . '" onChange="' . $this->topFrame . '.mark();"'),
								g_l('navigation','[name]')) . htmlFormElementTable(
								htmlTextInput(
										'Display',
										'',
										$this->Model->Display,
										'',
										'style="width: ' . $this->_width_size . '" onChange="' . $this->topFrame . '.mark();"'),
								g_l('navigation','[display]')) . $this->getHTMLChooser(
								g_l('navigation','[group]'),
								NAVIGATION_TABLE,
								0,
								'ParentID',
								$_parentid,
								...
								'folder',
								($this->Model->IsFolder == 0 && $this->Model->Depended == 1)),
	...
	function getHTMLPropertiesItem()
	{
		...
		if ($this->Model->Selection == "dynamic") {
			$_seltype = array(
				'doctype' => g_l('navigation','[documents]')
			);
			if (defined('OBJECT_TABLE')) {
				$_seltype['classname'] = g_l('navigation','[objects]');
			}
			$_seltype['category'] = g_l('navigation','[categories]');

		} else {
			$_seltype = array(
				'docLink' => g_l('navigation','[docLink]'), 'urlLink' => g_l('navigation','[urlLink]')
			);
			if (defined('OBJECT_TABLE')) {
				$_seltype['objLink'] = g_l('navigation','[objLink]');
			}
			$_seltype['catLink'] = g_l('navigation','[catLink]');
		}
		if (defined("OBJECT_TABLE")) {
		...
		$_selection_block = $this->Model->Depended == 1 ? $this->getHTMLDependedProfile() : $this->View->htmlHidden(
				'CategoriesControl',
				(isset($_REQUEST['CategoriesCount']) ? $_REQUEST['CategoriesCount'] : 0)) . $this->View->htmlHidden(
				'SortControl',
				(isset($_REQUEST['SortCount']) ? $_REQUEST['SortCount'] : 0)) . $this->View->htmlHidden(
				'CategoriesCount',
				(isset($this->Model->Categories) ? count($this->Model->Categories) : '0')) . $this->View->htmlHidden(
				'SortCount',
			...
			' . htmlSelect(
				'Selection',
				array(
					'dynamic' => g_l('navigation','[dyn_selection]'), 'static' => g_l('navigation','[stat_selection]')
				),
				1,
				...
				$_seltype,
				1,
				$this->Model->SelectionType,
				false,
				'onChange="onSelectionTypeChangeJS(this); closeAllType();clearFields();closeAllStats();toggle(this.value);setWorkspaces(this.value);setStaticSelection(this.value);' . $this->topFrame . '.mark();" style="width: ' . $this->_width_size . 'px; margin-top: ' . $this->_margin_top . ';"',
				'value',
				$this->_width_size) .
	...
	function getHTMLPropertiesGroup()
	{
		...
		$rootDirID = 0;
		$_cmd_doc = "javascript:we_cmd('openDocselector',document.we_form.elements['LinkID'].value,'" . FILE_TABLE . "','document.we_form.elements[\\'LinkID\\'].value','document.we_form.elements[\\'LinkPath\\'].value','','" . session_id() . "','$rootDirID',''," . (we_hasPerm(
				"CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")";
		$_cmd_obj = defined('OBJECT_TABLE') ? "javascript:we_cmd('openDocselector',document.we_form.elements['LinkID'].value,'" . OBJECT_FILES_TABLE . "','document.we_form.elements[\\'LinkID\\'].value','document.we_form.elements[\\'LinkPath\\'].value','opener." . $this->topFrame . ".we_cmd(\"populateFolderWs\");','" . session_id() . "','$rootDirID','objectFile'," . (we_hasPerm(
				"CAN_SELECT_OTHER_USERS_OBJECTS") ? 0 : 1) . ")" : '';
		...
		if ($this->Model->LinkID == 0) {
			...
			$_path = id_to_path(
					$this->Model->LinkID,
					($this->Model->FolderSelection == 'docLink' ? FILE_TABLE : (defined('OBJECT_TABLE') && $this->Model->FolderSelection == 'objLink' ? OBJECT_FILES_TABLE : FILE_TABLE)));
		}
		$_seltype = array(
			'docLink' => g_l('navigation','[docLink]'), 'urlLink' => g_l('navigation','[urlLink]')
		);
		...
		$yuiSuggest->setContentType(
				$this->Model->FolderSelection == 'docLink' ? "folder,text/xml,text/webedition,image/*,text/html,application/*,application/x-shockwave-flash,video/quicktime" : (defined(
						'OBJECT_TABLE') && $this->Model->FolderSelection == 'objLink' ? 'folder,objectFile' : "folder,text/xml,text/webedition,image/*,text/html,application/*,application/x-shockwave-flash,video/quicktime"));
		...
		' . htmlSelect(
				...
				$_seltype,
				1,
				$this->Model->FolderSelection,
				false,
				'onChange="onFolderSelectionChangeJS(this);setFolderSelection(this.value);' . $this->topFrame . '.mark();" style="width: ' . $this->_width_size . 'px; margin-top: ' . $this->_margin_top . ';"',
				'value',
				$this->_width_size) . '
		...
		' . $this->getHTMLWorkspace('object', 0, 'FolderWsID') . htmlFormElementTable(
				htmlTextInput(
						'FolderParameter',
						58,
						$this->Model->FolderParameter,
						'',
						'onchange="' . $this->topFrame . '.mark();"',
						'text',
						$this->_width_size,
						...
						'CurrentOnUrlPar',
						g_l('navigation','[current_on_urlpar]'),
						false,
						"defaultfont",
						$this->topFrame . '.mark();"'),
				g_l('navigation','[parameter]'));
		...
		if (function_exists('mb_convert_encoding')) {
			array_push(
					...
					array(
							...
							'html' => htmlAlertAttentionBox(g_l('navigation','[charset_desc]'), 2, $this->_width_size) . $this->getHTMLCharsetTable(),
							'space' => $this->_space_size,
							'noline' => 1
					));
		}
		...
		array_push(
				...
				array(
						...
						'html' => "<script type='text/javascript'>\nvar selfNaviPath ='" . addslashes(
								...
								true,
								0) . "</div>" . "<div style='padding-top:{$_padding}px'>" . $_button_copyFolder . "</div>" . we_htmlElement::htmlHidden(
								array(
	...
	function getHTMLDependedProfile()
	{
		...
		$_table->setColContent(
				1,
				0,
				($this->Model->SelectionType == 'catLink' ? g_l('navigation','[catLink]') : ($this->Model->SelectionType == 'docLink' ? g_l('navigation','[docLink]') : g_l('navigation','[objLink]'))) . ':');
		...
		$_table->setColContent(
				...
				id_to_path(
						$this->Model->LinkID,
						($this->Model->SelectionType == 'catLink' ? CATEGORY_TABLE : ($this->Model->SelectionType == 'docLink' ? FILE_TABLE : OBJECT_FILES_TABLE))));
		...
		if (!empty($this->Model->Url) && $this->Model->Url != 'http://') {
			...
			if (!empty($this->Model->UrlID) && $this->Model->UrlID) {
				$_table->setColContent(2, 0, g_l('navigation','[linkSelection]') . ':');
		...
		$_selection_block = '
			<div style="display: block;">
			' . g_l('navigation','[stat_selection]'). '<br />' . ($this->Model->SelectionType == 'catLink' ? g_l('navigation','[catLink]') : ($this->Model->SelectionType == 'docLink' ? g_l('navigation','[docLink]') : g_l('navigation','[objLink]'))) . ':' . id_to_path(
				$this->Model->LinkID,
				($this->Model->SelectionType == 'catLink' ? CATEGORY_TABLE : ($this->Model->SelectionType == 'docLink' ? FILE_TABLE : OBJECT_FILES_TABLE))) . '
			</div>
			<div>
	...
	function getHTMLDynamic()
	{
		...
		if (defined('OBJECT_TABLE')) {
			...
			$_firstClass = 0;
			$this->db->query(
					"SELECT DISTINCT  " . OBJECT_TABLE . ".ID," . OBJECT_TABLE . ".Text," . OBJECT_FILES_TABLE . ".ParentID FROM " . OBJECT_TABLE . " LEFT JOIN " . OBJECT_FILES_TABLE . " ON (" . OBJECT_TABLE . ".ID=" . OBJECT_FILES_TABLE . ".TableID)");
			while ($this->db->next_record()) {
		...
		<div style="display: block;">
			<div id="doctype" style="' . ($this->Model->SelectionType == 'doctype' ? 'display: block' : 'display: none') . '; width: ' . $this->_width_size . ';margin-top:' . $this->_margin_top . '">
				...
				htmlSelect(
						...
						$docTypes,
						1,
						$this->Model->DocTypeID,
						false,
						'onChange="clearFields();' . $this->topFrame . '.mark();"',
						'value',
						$this->_width_size),
			...
			<div id="classname" style="' . ($this->Model->SelectionType == 'classname' ? 'display: block' : 'display: none') . '; width: ' . $this->_width_size . ';margin-top:' . $this->_margin_top . ';">
				' . (defined('OBJECT_TABLE') ? htmlFormElementTable(
				htmlSelect(
						'ClassID',
						$classNames,
						1,
						$this->Model->ClassID,
						false,
						'onChange="clearFields();document.we_form.elements[\'FolderID\'].value=objectDirs[document.we_form.elements[\'ClassID\'].options[document.we_form.elements[\'ClassID\'].selectedIndex].value];document.we_form.elements[\'FolderPath\'].value=objectPaths[document.we_form.elements[\'ClassID\'].options[document.we_form.elements[\'ClassID\'].selectedIndex].value];' . $this->topFrame . '.we_cmd(\'populateWorkspaces\');' . $this->topFrame . '.mark();"',
						'value',
						$this->_width_size),
				g_l('navigation','[class]')) . $this->getHTMLWorkspace('class', $_firstClass) : '') . '
			</div>
			...
			' . $this->getHTMLFieldChooser(
				g_l('navigation','[title_field]'),
				'TitleField',
				$this->Model->TitleField,
				'putTitleField',
				$this->Model->SelectionType,
				($this->Model->SelectionType == 'classname' ? $this->Model->ClassID : $this->Model->DocTypeID)) . '
			</div>
			...
			' . $we_button->create_button_table(
				array(
						...
						$we_button->create_button(
								'preview',
								'javascript:' . $this->topFrame . '.we_cmd("dyn_preview");'),
						($this->Model->hasDynChilds() ? $we_button->create_button(
								'delete_all',
								'javascript:' . $this->topFrame . '.we_cmd("depopulate");') : '')
				)
				,
				10,
				array(
	...
	function getHTMLStatic($disabled = false)
	{
		...
		if (defined('OBJECT_TABLE')) {
			$seltype['objLink'] = g_l('navigation','[objLink]');
		}

		$rootDirID = 0;
		$_cmd_doc = "javascript:we_cmd('openDocselector',document.we_form.elements['LinkID'].value,'" . FILE_TABLE . "','document.we_form.elements[\\'LinkID\\'].value','document.we_form.elements[\\'LinkPath\\'].value','','" . session_id() . "','$rootDirID',''," . (we_hasPerm(
				"CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")";
		$_cmd_obj = defined('OBJECT_TABLE') ? "javascript:we_cmd('openDocselector',document.we_form.elements['LinkID'].value,'" . OBJECT_FILES_TABLE . "','document.we_form.elements[\\'LinkID\\'].value','document.we_form.elements[\\'LinkPath\\'].value','opener." . $this->topFrame . ".we_cmd(\"populateWorkspaces\");','" . session_id() . "','$rootDirID',''," . (we_hasPerm(
				"CAN_SELECT_OTHER_USERS_OBJECTS") ? 0 : 1) . ")" : '';
		...
		if ($this->Model->LinkID == 0) {
			...
			$_path = id_to_path(
					$this->Model->LinkID,
					($this->Model->SelectionType == 'docLink' ? FILE_TABLE : ($this->Model->SelectionType == 'catLink' ? CATEGORY_TABLE : (defined(
		...
		$yuiSuggest->setAcId("LinkPath");
		$yuiSuggest->setContentType(
				$this->Model->SelectionType == 'docLink' ? "folder,text/xml,text/webedition,image/*,text/html,application/*,application/x-shockwave-flash,video/quicktime" : ($this->Model->SelectionType == 'folder,objectFile' ? OBJECT_FILES_TABLE : ''));
		$yuiSuggest->setInput('LinkPath', $_path, array(
			"onChange" => $this->topFrame . ".mark();"
		));
		$yuiSuggest->setMaxResults(10);
		...
		$yuiSuggest->setSelector("Docselector");
		$yuiSuggest->setTable(
				$this->Model->SelectionType == 'docLink' ? FILE_TABLE : ($this->Model->SelectionType == 'objLink' ? OBJECT_FILES_TABLE : CATEGORY_TABLE));
		$yuiSuggest->setWidth($this->_width_size - 120);
		$yuiSuggest->setSelectButton($_buttons);
	...
	function getHTMLTab2()
	{
		...
		$_selection_block = $this->View->htmlHidden(
			...
			<div style="display: block;">
				...
				$this->Model->Selection,
				false,
				'style="width: ' . $this->_width_size . 'px;" onChange="toggle(\'dynamic\');setPresentation(\'dynamic\');setWorkspaces(\'\');' . $this->topFrame . '.mark();setCustomerFilter(this);"',
				'value',
				$this->_width_size) . '
				</div>
				...
				$this->Model->SelectionType,
				false,
				'onChange="onSelectionTypeChangeJS(this); closeAllType();clearFields();toggle(this.value);setWorkspaces(this.value);setStaticSelection(this.value);' . $this->topFrame . '.mark();"',
				'value',
				$this->_width_size) . $this->getHTMLDynamic() . '</div>
			</div>';
	...
	function getHTMLContentInfo()
	{
		...
		$parts[] = array(
				...
				'html' => htmlSelect(
						'dynContent',
						$this->View->getItems($this->Model->ID),
						20,
						'',
						false,
						'style="width: ' . $this->_width_size . 'px; height: 200;  margin: 0 0 ' . $this->_margin_bottom . ' 0;"'),
				'space' => $this->_space_size,
				'noline' => 0
		);
		...
		$parts[] = array(
				...
				'html' => we_button::create_button_table(
						array(
								...
								$we_button->create_button(
										'refresh',
										'javascript:' . $this->topFrame . '.we_cmd("populate");'),
								$we_button->create_button(
										'delete_all',
										'javascript:' . $this->topFrame . '.we_cmd("depopulate");')
						)),
	...
	function getHTMLEditorPreview()
	{
		...
		<tr>
			<td>' . we_htmlElement::htmlTextArea(
				array(
		...
		<tr>
			<td align="right">' . we_button::create_button_table(
				array(
						...
						$we_button->create_button(
								'reset',
								'javascript: document.getElementById("previewCode").value = "' . $defaultPreviewCode . '"; showPreview();')
		...
		)) . htmlDialogLayout($out, g_l('navigation','[preview]'));
	}
...
function onFolderSelectionChangeJS(elem) {
	YAHOO.autocoml.modifySetById("yuiAcInputLinkPath","table",(elem.value=="docLink"?"' . FILE_TABLE . '":"' . (defined(
		...
		if ($tabNr == 'preview') {
			...
			$out .= we_htmlElement::htmlDiv(
					...
					$this->View->htmlHidden(
							'IsFolder',
							(isset($this->Model->IsFolder) ? $this->Model->IsFolder : '0')) . $this->View->htmlHidden(
							'presetFolder',
							(isset($_REQUEST['presetFolder']) ? $_REQUEST['presetFolder'] : '0')) .
					...
					($this->Model->IsFolder ? we_multiIconBox::getHTML(
							...
							'',
							false,
							$preselect) : we_multiIconBox::getHTML(
							'',
							'100%',
							$this->getHTMLPropertiesItem(),
							30,
							'',
							-1,
							'',
							'',
							false,
							$preselect)) . (($this->Model->Selection == 'static' || $this->Model->IsFolder) ? $this->getHTMLAttributes() : '')) . ($this->Model->IsFolder ? (we_htmlElement::htmlDiv(
					array(
			...
			'') . ((defined('CUSTOMER_TABLE')) ? we_htmlElement::htmlDiv(
					array(
	...
	function getHTMLFieldChooser($title, $name, $value, $cmd, $type, $selection, $extraField = "", $extraFieldWidth = 0)
	{
		...
		if ($extraField == "") {
			...
			return htmlFormElementTable(
					hidden($name, $value) . $this->htmlTextInput(
							"__" . $name,
							58,
							$showValue,
							'',
							'onchange="setFieldValue(\'' . $name . '\',this); ' . $this->topFrame . '.mark();"',
							'text',
							($this->_width_size - 120),
							0),
					$title,
			...
			return htmlFormElementTable(
					hidden($name, $value) . $this->htmlTextInput(
							...
							'onchange="setFieldValue(\'' . $name . '\',this); ' . $this->topFrame . '.mark();"',
							'text',
							($this->_width_size - 120) - abs($extraFieldWidth) - 8,
							0),
					$title,
					'left',
					'defaultfont',
	...
	function getHTMLChooser($title, $table = FILE_TABLE, $rootDirID = 0, $IDName = 'ID', $IDValue = '', $PathName = 'Path', $cmd = '', $filter = 'text/webedition', $disabled = false, $showtrash = false)
	{
		...
		if ($table == NAVIGATION_TABLE) {
			...
			if ($filter == 'folder') {
				...
				$_cmd = "javascript:we_cmd('openDocselector',document.we_form.elements['$IDName'].value,'$table','document.we_form.$IDName.value','document.we_form.$PathName.value','" . $cmd . "','" . session_id() . "','$rootDirID','$filter'," . (we_hasPerm(
						"CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")";
			...
			$_button = $we_button->create_button_table(
					array(
							...
							$we_button->create_button(
									...
									'javascript:document.we_form.elements["' . $IDName . '"].value=0;document.we_form.elements["' . $PathName . '"].value="/";',
									true,
									27,
									22)
					),
		...
		$_result = we_htmlElement::htmlHidden(
				array(
		...
		if (isset($weAcSelector)) {
			...
			return htmlFormElementTable(
					htmlTextInput($PathName, 58, $_path, '', 'readonly', 'text', ($this->_width_size - $_width), 0),
					$title,
					'left',
					'defaultfont',
					we_htmlElement::htmlHidden(array(
	...
	function getHTMLCategory()
	{
		...
		$addbut = $we_button->create_button(
				"add",
				"javascript:we_cmd('openCatselector','','" . CATEGORY_TABLE . "','','','fillIDs();opener.addCat(top.allPaths);opener." . $this->topFrame . ".mark();')");
		$del_but = addslashes(
	...
	function getHTMLFieldSelector()
	{
		...
		$_parts[] = array(
				...
				'html' => htmlSelect(
						...
						'',
						($_multi ? true : false),
						'style="width: 300px; height: 200px; margin: ' . $this->_margin_bottom . 'px 0px ' . $this->_margin_bottom . 'px 0px;" onClick="setTimeout(\'selectItem();\',100);"'),
				'space' => 0
		);
		...
		$we_button->create_button_table(
				array(
					$we_button->create_button(
						...
						'',
						true,
						false), $we_button->create_button('close', 'javascript:self.close();')
				));
		...
		$_body = we_htmlElement::htmlBody(
				...
				we_htmlElement::htmlForm(
						...
						we_multiIconBox::getHTML(
								...
								'',
								false,
								g_l('navigation','[select_field_txt]'))));
	...
	{
		$_code = '<div style="width: ' . $this->_width_size . '; margin-top: ' . $this->_margin_top . '">' . htmlFormElementTable(
				htmlTextInput(
						'ShowCount',
						30,
						$this->Model->ShowCount,
						'',
						'onBlur="var r=parseInt(this.value);if(isNaN(r)) this.value=' . $this->Model->ShowCount . '; else{ this.value=r; ' . $this->topFrame . '.mark();}"',
						'text',
						$this->_width_size,
						0),
				g_l('navigation','[show_count]')) . '</div>';
	...
	function getHTMLDirSelector()
	{
		...
		$_button_obj = defined('OBJECT_TABLE') ? $we_button->create_button(
				...
				"",
				"",
				($_countClasses ? false : true),
				false,
				"_XFolder") : '';

		$_button_cat = $we_button->create_button(
				'select',
		...
		$_path = id_to_path(
				$this->Model->FolderID,
				(($this->Model->SelectionType == 'doctype') ? FILE_TABLE : ($this->Model->SelectionType == 'category' ? CATEGORY_TABLE : (defined(
						'OBJECT_FILES_TABLE') ? OBJECT_FILES_TABLE : ''))));
		...
		$yuiSuggest->setTable(
				$this->Model->SelectionType == 'docLink' ? FILE_TABLE : ($this->Model->SelectionType == 'objLink' ? OBJECT_FILES_TABLE : FILE_TABLE));
		$yuiSuggest->setWidth($this->_width_size - 120);
		$yuiSuggest->setSelectButton($_buttons);

		$weAcSelector = $yuiSuggest->getHTML();

		return we_htmlElement::htmlDiv(array(
			'style' => 'margin-top:' . $this->_margin_top . ';'
		), $weAcSelector);
	...
	function getHTMLDynPreview()
	{
		...
		foreach ($_items as $_k => $_item) {
			$_txt = id_to_path(
					$_item['id'],
					($this->Model->SelectionType == 'doctype') ? FILE_TABLE : ($this->Model->SelectionType == 'category' ? CATEGORY_TABLE : OBJECT_FILES_TABLE));
			if (!empty($_item['field'])) {
				$_opt = we_htmlSelect::getNewOptionGroup(
						array(
		...
		$_body = we_htmlElement::htmlBody(
				...
				we_htmlElement::htmlForm(
						...
						we_multiIconBox::getHTML(
								...
								$_parts,
								30,
								'<div style="float:right;">' . $we_button->create_button(
										'close',
										'javascript:self.close();') . '</div>',
								-1,
								'',
								'',
								false,
								g_l('navigation','[dyn_selection]'))));
	...
	function getHTMLWorkspace($type = 'object', $defClassID = 0, $field = 'WorkspaceID')
	{
		...
		if ($type == 'class') {
			...
			return '<div id="objLinkWorkspaceClass" style="display: ' . (($this->Model->Selection == 'dynamic') ? 'block' : 'none') . ';margin-top: ' . $this->_margin_top . ';">' . htmlFormElementTable(
					htmlSelect(
							'WorkspaceIDClass',
							$_wsid,
							1,
							...
							false,
							'style="width: ' . $this->_width_size . 'px;" onChange="' . $this->topFrame . '.mark();"',
							'value'),
					g_l('navigation','[workspace]')) . '</div>';
			...
			if ($field == 'WorkspaceID') {
				...
				return '<div id="objLinkWorkspace" style="display: ' . (($this->Model->SelectionType == 'objLink' && ($this->Model->WorkspaceID > -1)) ? 'block' : 'none') . ';margin-top: ' . $this->_margin_top . ';">' . htmlFormElementTable(
						htmlSelect(
								'WorkspaceID',
								$_wsid,
								1,
				...
				return '<div id="objLinkFolderWorkspace" style="display: ' . (($this->Model->FolderSelection == 'objLink' && ($this->Model->FolderWsID > -1)) ? 'block' : 'none') . ';margin-top: ' . $this->_margin_top . ';">' . htmlFormElementTable(
						htmlSelect(
								'FolderWsID',
								$_wsid,
								1,
								$this->Model->FolderWsID,
								false,
								'style="width: ' . $this->_width_size . 'px;" onChange="' . $this->topFrame . '.mark();"',
								'value'),
						g_l('navigation','[workspace]')) . '</div>';
	...
	function getHTMLLink($prefix = '')
	{
		...
		$_cmd = "javascript:we_cmd('openDocselector',document.we_form.elements['" . $prefix . "UrlID'].value,'" . FILE_TABLE . "','document.we_form.elements[\\'" . $prefix . "UrlID\\'].value','document.we_form.elements[\\'" . $prefix . "UrlIDPath\\'].value','opener." . $this->topFrame . ".mark()','" . session_id() . "',0,'text/webedition'," . (we_hasPerm(
				"CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")";
		...
		return we_htmlElement::jsElement(
				...
				') . '<div id="' . $prefix . 'LinkSelectionDiv" style="display: ' . (($this->Model->SelectionType == 'catLink' || $this->Model->SelectionType == 'category') ? 'block' : 'none') . ';margin-top: ' . $this->_margin_top . ';">' . htmlFormElementTable(
				htmlSelect(
						$prefix . 'LinkSelection',
						array(
							'intern' => g_l('navigation','[intern]'),
								'extern' => g_l('navigation','[extern]')
						),
						1,
						$this->Model->LinkSelection,
						false,
						'style="width: ' . $this->_width_size . 'px;" onChange="' . $prefix . 'setLinkSelection(this.value);' . $this->topFrame . '.mark();"',
						'value'),
				g_l('navigation','[linkSelection]')) . '</div>
				...
				<div id="' . $prefix . 'extern" style="display: ' . (($this->Model->LinkSelection == 'extern' || $this->Model->SelectionType == 'urlLink') ? 'block' : 'none') . ';margin-top: ' . $this->_margin_top . ';">' . htmlTextInput(
				$prefix . 'Url',
				58,
				$this->Model->Url,
				'',
				'onchange="' . $this->topFrame . '.mark();"',
				'text',
				$this->_width_size) . '</div>
				<div style="margin-top: ' . $this->_margin_top . ';">' . htmlFormElementTable(
				htmlTextInput(
						$prefix . 'CatParameter',
						58,
						...
						'text',
						$this->_width_size),
				g_l('navigation','[catParameter]')) . '</div>';
	...
	function getHTMLCharsetTable()
	{
		...
		$table->setCol(
				...
				htmlSelect(
						...
						$value,
						false,
						"onblur='document.forms[0].elements[\"Charset\"].value=this.options[this.selectedIndex].value;' onchange='document.forms[0].elements[\"Charset\"].value=this.options[this.selectedIndex].value;document.we_form.submit();'",
						'value',
						($this->_width_size - 122),
						"defaultfont",
						false));
	...
	function getHTMLAttributes()
	{
		...
		$_parts[] = array(
				...
				'headline' => '',
				'html' => htmlAlertAttentionBox(g_l('navigation','[linkprops_desc]'), 2, $this->_width_size),
				'space' => $this->_space_size,
				'noline' => 1
		);
		...
		$_title = htmlFormElementTable(
				htmlTextInput(
						'Attributes[title]',
						30,
						$this->Model->getAttribute('title'),
						'',
						'onchange="' . $this->topFrame . '.mark();"',
						'text',
						$this->_width_size),
				g_l('navigation','[title]'));

		$_anchor = htmlFormElementTable(
				htmlTextInput(
						'Attributes[anchor]',
						30,
						$this->Model->getAttribute('anchor'),
						'',
						'onchange="' . $this->topFrame . '.mark();"',
						'text',
						...
						'CurrentOnAnker',
						g_l('navigation','[current_on_anker]'),
						false,
						"defaultfont",
						$this->topFrame . '.mark();"'),
				g_l('navigation','[anchor]'));
		...
		$_target = htmlFormElementTable(
				targetBox(
						...
						($this->_width_size - 100),
						'',
						$this->Model->getAttribute('target'),
						'' . $this->topFrame . '.mark();',
						'text',
						100),
				g_l('navigation','[target]'));

		$_link = htmlFormElementTable(
				htmlTextInput(
						'Attributes[link_attribute]',
						30,
						$this->Model->getAttribute('link_attribute'),
						'',
						'onchange="' . $this->topFrame . '.mark();"',
						'text',
						$this->_width_size),
				g_l('navigation','[link_attribute]'));
		...
		$_lang = $this->getLangField(
				'lang',
				$this->Model->getAttribute('lang'),
				g_l('navigation','[link_language]'),
				$this->_width_size);
		$_hreflang = $this->getLangField(
				'hreflang',
				$this->Model->getAttribute('hreflang'),
				g_l('navigation','[href_language]'),
				$this->_width_size);
		...
		$_accesskey = htmlFormElementTable(
				htmlTextInput(
						...
						30,
						$this->Model->getAttribute('accesskey'),
						'',
						'onchange="' . $this->topFrame . '.mark();"',
						'text',
						$this->_width_size),
				g_l('navigation','[accesskey]'));

		$_tabindex = htmlFormElementTable(
				htmlTextInput(
						'Attributes[tabindex]',
						30,
						$this->Model->getAttribute('tabindex'),
						'',
						'onchange="' . $this->topFrame . '.mark();"',
						'text',
						$this->_width_size),
				g_l('navigation','[tabindex]'));
		...
		$_popup->setCol(
				...
				we_forms::checkboxWithHidden(
						$this->Model->getAttribute('popup_open'),
						'Attributes[popup_open]',
						g_l('navigation','[popup_open]'),
						false,
						"defaultfont",
						$this->topFrame . '.mark();"'));
		$_popup->setCol(
				...
				we_forms::checkboxWithHidden(
						$this->Model->getAttribute('popup_center'),
						'Attributes[popup_center]',
						g_l('navigation','[popup_center]'),
						false,
						"defaultfont",
						$this->topFrame . '.mark();"'));
		...
		$_popup->setCol(
				...
				htmlFormElementTable(
						htmlTextInput(
								'Attributes[popup_xposition]',
								5,
								$this->Model->getAttribute('popup_xposition'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								$_input_width),
						g_l('navigation','[popup_x]')));
		$_popup->setCol(
				1,
				1,
				array(),
				htmlFormElementTable(
						htmlTextInput(
								'Attributes[popup_yposition]',
								5,
								$this->Model->getAttribute('popup_yposition'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								$_input_width),
						g_l('navigation','[popup_y]')));
		$_popup->setCol(
				1,
				2,
				array(),
				htmlFormElementTable(
						htmlTextInput(
								...
								5,
								$this->Model->getAttribute('popup_width'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								$_input_width),
						g_l('navigation','[popup_width]')));
		...
		$_popup->setCol(
				...
				htmlFormElementTable(
						htmlTextInput(
								'Attributes[popup_height]',
								5,
								$this->Model->getAttribute('popup_height'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								$_input_width),
						g_l('navigation','[popup_height]')));
		...
		$_popup->setCol(
				...
				array(),
				we_forms::checkboxWithHidden(
						$this->Model->getAttribute('popup_status'),
						'Attributes[popup_status]',
						g_l('navigation','[popup_status]'),
						false,
						"defaultfont",
						$this->topFrame . '.mark();"'));
		$_popup->setCol(
				...
				array(),
				we_forms::checkboxWithHidden(
						$this->Model->getAttribute('popup_scrollbars'),
						'Attributes[popup_scrollbars]',
						g_l('navigation','[popup_scrollbars]'),
						false,
						"defaultfont",
						$this->topFrame . '.mark();"'));
		$_popup->setCol(
				...
				array(),
				we_forms::checkboxWithHidden(
						$this->Model->getAttribute('popup_menubar'),
						'Attributes[popup_menubar]',
						g_l('navigation','[popup_menubar]'),
						false,
						"defaultfont",
						$this->topFrame . '.mark();"'));
		...
		$_popup->setCol(
				...
				array(),
				we_forms::checkboxWithHidden(
						$this->Model->getAttribute('popup_resizable'),
						'Attributes[popup_resizable]',
						g_l('navigation','[popup_resizable]'),
						false,
						"defaultfont",
						$this->topFrame . '.mark();"'));
		$_popup->setCol(
				...
				array(),
				we_forms::checkboxWithHidden(
						$this->Model->getAttribute('popup_location'),
						'Attributes[popup_location]',
						g_l('navigation','[popup_location]'),
						false,
						"defaultfont",
						$this->topFrame . '.mark();"'));
		$_popup->setCol(
				...
				array(),
				we_forms::checkboxWithHidden(
						$this->Model->getAttribute('popup_toolbar'),
						'Attributes[popup_toolbar]',
						g_l('navigation','[popup_toolbar]'),
						false,
						"defaultfont",
						$this->topFrame . '.mark();"'));
		...
		return we_multiIconBox::getHTML(
				...
				'',
				0,
				g_l('navigation','[more_attributes]'),
				g_l('navigation','[less_attributes]'),
				($wepos == 'down'));
	...
	function getHTMLImageAttributes()
	{
		$_parts = array();
		$_input_width = 70;
		$_img_props = new we_htmlTable(array(
		...
		$_img_props->setCol(
				...
				htmlFormElementTable(
						htmlTextInput(
								'Attributes[icon_width]',
								5,
								$this->Model->getAttribute('icon_width'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								$_input_width),
						g_l('navigation','[icon_width]')));
		$_img_props->setCol(
				0,
				1,
				array(),
				htmlFormElementTable(
						htmlTextInput(
								'Attributes[icon_height]',
								5,
								$this->Model->getAttribute('icon_height'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								$_input_width),
						g_l('navigation','[icon_height]')));
		$_img_props->setCol(
				0,
				2,
				array(),
				htmlFormElementTable(
						htmlTextInput(
								'Attributes[icon_border]',
								5,
								$this->Model->getAttribute('icon_border'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								$_input_width),
						g_l('navigation','[icon_border]')));
		$_img_props->setCol(
				0,
				3,
				array(),
				htmlFormElementTable(
						htmlTextInput(
								'Attributes[icon_hspace]',
								5,
								$this->Model->getAttribute('icon_hspace'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								$_input_width),
						g_l('navigation','[icon_hspace]')));
		$_img_props->setCol(
				0,
				4,
				array(),
				htmlFormElementTable(
						htmlTextInput(
								'Attributes[icon_vspace]',
								5,
								$this->Model->getAttribute('icon_vspace'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								$_input_width),
						g_l('navigation','[icon_vspace]')));
		$_img_props->setCol(
				1,
				0,
				array(
				...
				htmlFormElementTable(
						htmlSelect(
								...
								1,
								$this->Model->getAttribute('icon_align'),
								false,
								'style="width: ' . ($this->_width_size - 50) . 'px;"'),
						g_l('navigation','[icon_align]')));
		$_img_props->setCol(
				2,
				0,
				array(
				...
				htmlFormElementTable(
						htmlTextInput(
								'Attributes[icon_alt]',
								5,
								$this->Model->getAttribute('icon_alt'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								($this->_width_size - 50)),
						g_l('navigation','[icon_alt]')));
		$_img_props->setCol(
				3,
				0,
				array(
				...
				htmlFormElementTable(
						htmlTextInput(
								'Attributes[icon_title]',
								5,
								$this->Model->getAttribute('icon_title'),
								'',
								'onchange="' . $this->topFrame . '.mark();"',
								'text',
								($this->_width_size - 50)),
		...
		array_push(
				...
				array(
						...
						'html' => $_view->getFilterHTML(
								$this->Model->IsFolder == 0 && $this->Model->Selection == "dynamic"),
						'space' => $_space_size,
						'noline' => 1
				));
	...
	function getHTMLEditorFooter()
	{
		...
		if (isset($_REQUEST["home"])) {
			return $this->getHTMLDocument(we_htmlElement::htmlBody(array(
				"bgcolor" => "#F0EFF0"
			), ""));
		}
		...
		$table2->setColContent(
				...
				we_forms::checkbox(
						...
						false,
						"makeNewDoc",
						($this->View->Model->IsFolder ? g_l('global',"[we_new_folder_after_save]") : g_l('global',"[we_new_entry_after_save]")),
						false,
						"defaultfont",
						""));
