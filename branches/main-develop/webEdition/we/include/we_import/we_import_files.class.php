 ...
 * $Rev: 3539 $
 * $Author: mokraemer $
 * $Date: 2011-12-11 19:17:49 +0100 (So, 11. Dez 2011) $
 *
...
class we_import_files{
	...
	function _getJS($fileinput){
		...
		$js .= 'var we_fileinput = \'<form name="we_upload_form_WEFORMNUM" method="post" action="' . WEBEDITION_DIR . 'we_cmd.php" enctype="multipart/form-data" target="imgimportbuttons">' . str_replace("\n", " ", str_replace("\r", " ", $this->_getHiddens("buttons", $this->step + 1))) . $fileinput . '</form>\';
				...
				g_l('importFiles', "[finished]"), WE_MESSAGE_NOTICE) . '
				}
	...
	function getStep1(){
		...
		$button = we_button::create_button(
				"select", "javascript:we_cmd('openDirselector',document.we_startform.importToID.value,'" . FILE_TABLE . "','" . $wecmdenc1 . "','" . $wecmdenc2 . "','','','0')");
...
';
		...
		if(isset($_SESSION["WE_IMPORT_FILES_ERRORs"]) && $formnum == $formcount && $formnum != 0){
			...
			$js .= "			" . we_message_reporting::getShowMessageCall(
					sprintf(g_l('importFiles', "[error]"), $filelist), WE_MESSAGE_ERROR) . "\n";
		} else{
			$js .= "			" . we_message_reporting::getShowMessageCall(
					g_l('importFiles', "[finished]"), WE_MESSAGE_NOTICE) . "\n";
		}
		...
		$nextButton = we_button::create_button(
				"next", "javascript:next();", true, -1, -1, "", "", $this->step > 0, false);
