<?php

/**
 * webEdition CMS
 *
 * This source is part of webEdition CMS. webEdition CMS is
 * free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation; either version 3 of the License, or
 * any later version.
 *
 * The GNU General Public License can be found at
 * http://www.gnu.org/copyleft/gpl.html.
 * A copy is found in the textfile
 * webEdition/licenses/webEditionCMS/License.txt
 *
 * @category   webEdition
 * @package    webEdition_language
 * @license    http://www.gnu.org/copyleft/gpl.html  GPL
 */
/**
 * Language file: fileselector.inc.php
 * Provides language strings.
 * Language: Deutsch
 */
$l_fileselector = array(
		'username' => "Benutzername",
		'catname' => "Kategorie",
		'cat_in_use' => "Folgende Kategorien können nicht gelöscht werden, da sie benutzt werden:",
		'cat_in_use_rename' => "Die Kategorie kann nicht geändert werden, da sie benutzt wird!",
		'deleteQuestion' => "Ausgewählte Einträge löschen! Sind Sie sicher?",
		'overwriteImg' => "Überschreibe Grafik wenn schon vorhanden",
		'renameImg' => "Benenne neue Grafik um wenn schon vorhanden",
		'open' => "Öffnen",
		'close' => "Schließen",
		'lookin' => "Suchen in",
		'type' => "Dateityp",
		'title' => "Titel",
		'filename' => "Dateiname",
		'groupname' => "Gruppenname",
		'name' => "Name",
		'foldername' => "Verzeichnis",
		'folders' => "Verzeichnisse",
		'files' => "Dateien",
		'up_folder' => "Übergeordnetes Verzeichnis",
		'new_folder' => "Neues Verzeichnis erstellen",
		'new_cat' => "Neue Kategorie erstellen",
		'delete_entry' => "Eintrag löschen",
		'new_folder_name' => "NeuesVerzeichnis",
		'new_cat_name' => "NeueKategorie",
		'created' => "Erstellungsdatum",
		'modified' => "Änderungsdatum",
		'root_folder' => "Wurzelverzeichnis",
		'ok' => "Ok",
		'cancel' => "Abbrechen",
		'all_Types' => "Alle Dateien",
		'new_image' => "Neue Grafik erstellen",
		'new_file' => "Neue Datei erstellen",
		'folder' => "Verzeichnis",
		'text/xml' => "XML-Datei",
		'edit_file' => "Datei bearbeiten",
		'data' => "Datei",
		'create_folder_nok' => "Das Verzeichnis kann nicht erstellt werden!",
		'rename_folder_nok' => "Das Verzeichnis kann nicht umbenannt werden!",
		'rename_file_nok' => "Die Datei kann nicht umbenannt werden!",
		'edit_file_nok' => "Der Dateiname ist leer!",
		'edit_file_is_folder' => "Sie haben keine Datei zum Bearbeiten ausgewählt!",
		'already_root' => "Dies ist bereits das Wurzelverzeichnis",
		'delete' => "Datei löschen",
		'overwriteImg' => "Überschreiben",
		'renameImg' => "Umbenennen",
		'can_not_overwrite_we_file' => "Eine webEdition-Datei kann nicht überschrieben werden!",
		'filesize' => "Dateigröße",
		'image_not_uploaded' => "Es wurde noch kein Bild hochgeladen.",
);