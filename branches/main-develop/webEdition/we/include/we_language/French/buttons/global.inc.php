<?php

/**
 * webEdition CMS
 *
 * $Rev$
 * $Author$
 * $Date$
 *
 * This source is part of webEdition CMS. webEdition CMS is
 * free software; you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation; either version 3 of the License, or
 * any later version.
 *
 * The GNU General Public License can be found at
 * http://www.gnu.org/copyleft/gpl.html.
 * A copy is found in the textfile
 * webEdition/licenses/webEditionCMS/License.txt
 *
 * @category   webEdition
 * @package    webEdition_language
 * @license    http://www.gnu.org/copyleft/gpl.html  GPL
 */


/**
 * Language file: global.inc.php
 * Provides language strings.
 * Language: English
 */

/*****************************************************************************
 * LOGIN-SCREEN
 *****************************************************************************/

$l_button["login"]["value"] = "Authentification";
$l_button["login"]["alt"] = "Entrer dans webEdition";
$l_button["login"]["width"] = 100;

$l_button["back_to_login"]["value"] = "Back";
$l_button["back_to_login"]["alt"] = "De Retour au webEdition-Authentification";
$l_button["back_to_login"]["width"] = 100;

/*****************************************************************************
 * STARTMENU
 *****************************************************************************/

$l_button["open_document"]["value"] = "Open document";
$l_button["open_document"]["alt"] = "Ouvrir un Document";
$l_button["open_document"]["width"] = 200;

$l_button["new_document"]["value"] = "New document";
$l_button["new_document"]["alt"] = "Créer un nouveau Document";
$l_button["new_document"]["width"] = 200;

$l_button["new_template"]["value"] = "New template";
$l_button["new_template"]["alt"] = "Créer une nouveau Modèle";
$l_button["new_template"]["width"] = 200;

$l_button["new_directory"]["value"] = "New directory";
$l_button["new_directory"]["alt"] = "Créer un nouveau Répertoire";
$l_button["new_directory"]["width"] = 200;

$l_button["unpublished_pages"]["value"] = "Unpublished documents";
$l_button["unpublished_pages"]["alt"] = "Aperçu des sites non-publiées";
$l_button["unpublished_pages"]["width"] = 200;

$l_button["preferences"]["value"] = "Preferences";
$l_button["preferences"]["alt"] = "Préferences";
$l_button["preferences"]["width"] = 200;

/*****************************************************************************
 * DELETE SCREEN
 *****************************************************************************/

$l_button["quit_delete"]["value"] = "Finish delete mode";
$l_button["quit_delete"]["alt"] = "Quittter le mode-supprimer";
$l_button["quit_delete"]["width"] = 175;

/*****************************************************************************
 * MOVE SCREEN
 *****************************************************************************/

$l_button["quit_move"]["value"] = "Finish move mode";
$l_button["quit_move"]["alt"] = "Finish move mode";
$l_button["quit_move"]["width"] = 175;

/*****************************************************************************
 * STANDARD
 *****************************************************************************/

$l_button["ok"]["value"] = "OK";
$l_button["ok"]["alt"]   = "Okay";
$l_button["ok"]["width"] = 100;

$l_button["cancel"]["value"] = "Cancel";
$l_button["cancel"]["alt"] = "Cancel";
$l_button["cancel"]["width"] = 100;

$l_button["yes"]["value"] = "Yes";
$l_button["yes"]["alt"]   = "Oui";
$l_button["yes"]["width"] = 100;

$l_button["no"]["value"] = "No";
$l_button["no"]["alt"]   = "Non";
$l_button["no"]["width"] = 100;

$l_button["save"]["value"] = "Save";
$l_button["save"]["alt"] = "Save";
$l_button["save"]["width"] = 100;

$l_button["publish"]["value"] = "Publish";
$l_button["publish"]["alt"]   = "Save and publish the document"; // TRANSLATE
$l_button["publish"]["width"] = 100;

$l_button["saveversion"]["value"] = "Save Version";// TRANSLATE
$l_button["saveversion"]["alt"]   = "Saves the template and creates a new version";// TRANSLATE
$l_button["saveversion"]["width"] = 120;// TRANSLATE

$l_button["delete"]["value"] = "Delete";
$l_button["delete"]["alt"] = "Delete";
$l_button["delete"]["width"] = 100;

$l_button["go"]["value"] = "Execute now";
$l_button["go"]["alt"]   = ""; // TRANSLATE
$l_button["go"]["width"] = 120;

$l_button["openVersionWizard"]["value"] = "Versions-Wizard";
$l_button["openVersionWizard"]["alt"]   = "Versions-Wizard"; // TRANSLATE
$l_button["openVersionWizard"]["width"] = 120;

$l_button["next"]["value"] = "Next";
$l_button["next"]["alt"]   = "Avancer";
$l_button["next"]["width"] = 100;

$l_button["back"]["value"] = "Back";
$l_button["back"]["alt"] = "Reculer";
$l_button["back"]["width"] = 100;

$l_button["open"]["value"] = "Open";
$l_button["open"]["alt"] = "Open";
$l_button["open"]["width"] = 100;

$l_button["default"]["value"] = "Default";
$l_button["default"]["alt"] = ""; // TRANSLATE
$l_button["default"]["width"] = 100;

$l_button["reset"]["value"] = "Reset";
$l_button["reset"]["alt"] = ""; // TRANSLATE
$l_button["reset"]["width"] = 100;

/*****************************************************************************
 * SAVING, PUBLISHING, ETC.
 *****************************************************************************/

$l_button["unpublish"]["value"] = "Unpublish";
$l_button["unpublish"]["alt"] = "Depublier";
$l_button["unpublish"]["width"] = 100;


/*****************************************************************************
 * MAKE AN NEW DOCUMENT BASED ON TEMPLATE
 *****************************************************************************/

$l_button["make_new_document"]["value"] = "New document";
$l_button["make_new_document"]["alt"] = "Créer un nouveau Document";
$l_button["make_new_document"]["width"] = 125;

/*****************************************************************************
 * SUPER-EASY-EDIT-MODE
 *****************************************************************************/

$l_button["preview"]["value"] = "Preview";
$l_button["preview"]["alt"] = "Prévision";
$l_button["preview"]["width"] = 100;

$l_button["properties"]["value"] = "Properties";
$l_button["properties"]["alt"] = "Indiquer les préférences";
$l_button["properties"]["width"] = 100;

$l_button["thumbnails"]["value"] = "Thumbnails";
$l_button["thumbnails"]["alt"]   = "Show thumbnails"; // TRANSLATE
$l_button["thumbnails"]["width"] = 100;

$l_button["shopVariants"]["value"] = "Variants";
$l_button["shopVariants"]["alt"] = "Éditer les variantes";
$l_button["shopVariants"]["width"] = 100;

/*****************************************************************************
 * DOCUMENT TYPES
 *****************************************************************************/

$l_button["new_doctype"]["value"] = "New document type";
$l_button["new_doctype"]["alt"] = "Créer un nouveau Type-de-Document";
$l_button["new_doctype"]["width"] = 174;

$l_button["delete_doctype"]["value"] = "Delete document type";
$l_button["delete_doctype"]["alt"] = "Supprimer le Type-de-Document séléctioné";
$l_button["delete_doctype"]["width"] = 174;

/*****************************************************************************
 * XML
 *****************************************************************************/

$l_button["import"]["value"] = "Import";
$l_button["import"]["alt"]   = "Import selected file"; // TRANSLATE
$l_button["import"]["width"] = 100;

$l_button["export"]["value"] = "Export";
$l_button["export"]["alt"]   = "Export selected files"; // TRANSLATE
$l_button["export"]["width"] = 100;

$l_button["browse"]["value"] = "Browse";
$l_button["browse"]["alt"] = "Fouiller des Répertoires";
$l_button["browse"]["width"] = 100;

/*****************************************************************************
 * FILE-SELECTOR
 *****************************************************************************/

$l_button["root_dir"]["value"] = "/";
$l_button["root_dir"]["alt"]   = "Répertoire principal";
$l_button["root_dir"]["width"] = 40;


/*****************************************************************************
 * UPLOAD-DIALOG
 *****************************************************************************/

$l_button["upload"]["value"] = "Upload";
$l_button["upload"]["alt"]   = "Télécharger un fichier";
$l_button["upload"]["width"] = 100;

$l_button["close"]["value"] = "Close";
$l_button["close"]["alt"]   = "Fermer le fenêtre";
$l_button["close"]["width"] = 100;

$l_button["overwrite"]["value"] = "Overwrite";
$l_button["overwrite"]["alt"]   = "Éffacer le fichier";
$l_button["overwrite"]["width"] = 100;

$l_button["newName"]["value"] = "New name";
$l_button["newName"]["alt"]   = "Saisissez un nouveau Nom";
$l_button["newName"]["width"] = 100;


/*****************************************************************************
 * PREFERENCES
 *****************************************************************************/

$l_button["add_languages"]["value"] = "Add languages";
$l_button["add_languages"]["alt"]   = "Installe un language supplémentaire pour webEdition";
$l_button["add_languages"]["width"] = 175;

$l_button["apply_current_dimension"]["value"] = "Apply current dimension";
$l_button["apply_current_dimension"]["alt"]   = "Adopte la taille actuelle du fenêtre de webEdition";
$l_button["apply_current_dimension"]["width"] = 175;

$l_button["res_800"]["value"] = "800x600";
$l_button["res_800"]["alt"]   = "Mettre 800x600";
$l_button["res_800"]["width"] = 100;

$l_button["res_1024"]["value"] = "1024x768";
$l_button["res_1024"]["alt"]   = "Mettre 1024x768";
$l_button["res_1024"]["width"] = 100;

$l_button["res_1280"]["value"] = "1280x960";
$l_button["res_1280"]["alt"]   = "Mettre 1280x960";
$l_button["res_1280"]["width"] = 100;

$l_button["res_1600"]["value"] = "1600x1200";
$l_button["res_1600"]["alt"]   = "Mettre 1600x1200";
$l_button["res_1600"]["width"] = 100;

$l_button["apply_current_editor_dimension"]["value"] = "Apply current dimension";
$l_button["apply_current_editor_dimension"]["alt"]   = "Adopte la taille actuelle de l'éditeur";
$l_button["apply_current_editor_dimension"]["width"] = 175;

$l_button["res_500"]["value"] = "500x300";
$l_button["res_500"]["alt"]   = "Mettre 500x300";
$l_button["res_500"]["width"] = 100;

$l_button["res_700"]["value"] = "700x320";
$l_button["res_700"]["alt"]   = "Mettre 700x320";
$l_button["res_700"]["width"] = 100;

$l_button["res_960"]["value"] = "960x420";
$l_button["res_960"]["alt"]   = "mettre 960x420";
$l_button["res_960"]["width"] = 100;

$l_button["res_1300"]["value"] = "1300x650";
$l_button["res_1300"]["alt"]   = "Mettre 1300x650";
$l_button["res_1300"]["width"] = 100;


/*****************************************************************************
 * Rebuild
 *****************************************************************************/

$l_button["rebuild"]["value"] = "Rebuild";
$l_button["rebuild"]["alt"]   = "Start rebuild"; // TRANSLATE
$l_button["rebuild"]["width"] = 100;

/*****************************************************************************
 * UPDATE
 *****************************************************************************/

$l_button["demoversion"]["value"] = "Demo version";
$l_button["demoversion"]["alt"]   = "Version-Demo";
$l_button["demoversion"]["width"] = 100;

$l_button["register"]["value"] = "Register";
$l_button["register"]["alt"]   = "Enrégistrer";
$l_button["register"]["width"] = 100;

$l_button["backup"]["value"] = "Backup";
$l_button["backup"]["alt"]   = "Sauvegardes";
$l_button["backup"]["width"] = 100;

$l_button["search"]["value"] = "Search";
$l_button["search"]["alt"]   = "Recherche";
$l_button["search"]["width"] = 100;

$l_button["replace"]["value"] = "Replace"; //Translate
$l_button["replace"]["alt"]   = "";
$l_button["replace"]["width"] = 100;

/*****************************************************************************
 * Backup
 *****************************************************************************/

$l_button["restore_backup"]["value"] = "Recover Backup";
$l_button["restore_backup"]["alt"]   = "Restaurer la sauvegarde";
$l_button["restore_backup"]["width"] = 180;

$l_button["make_backup"]["value"] = "Create Backup";
$l_button["make_backup"]["alt"]   = "Créer une sauvegarde";
$l_button["make_backup"]["width"] = 150;

$l_button["delete_backup"]["value"] = "Delete backup file";
$l_button["delete_backup"]["alt"]   = "Supprimer le fichier de sauvegarde choisi";
$l_button["delete_backup"]["width"] = 150;

/*****************************************************************************
 * Thumbnails
 *****************************************************************************/

$l_button["edit_all_thumbs"]["value"] = "Edit thumbnails...";
$l_button["edit_all_thumbs"]["alt"]   = "Editer les imagettes...";
$l_button["edit_all_thumbs"]["width"] = 150;

/*****************************************************************************
 * Navigation
 *****************************************************************************/
$l_button["new_item"]["value"] = "New item";
$l_button["new_item"]["alt"]   = ""; // TRANSLATE
$l_button["new_item"]["width"] = 200;

$l_button["new_folder"]["value"] = "New folder";
$l_button["new_folder"]["alt"]   = ""; // TRANSLATE
$l_button["new_folder"]["width"] = 200;

/*****************************************************************************
 * Logbuch Formmail
 *****************************************************************************/
$l_button["clear_log"]["value"] = "Clear logbook";
$l_button["clear_log"]["alt"] = "Clear logbook";
$l_button["clear_log"]["width"] = 120;

$l_button["logbook"]["value"] = "Logbook";
$l_button["logbook"]["alt"]   = "Afficher le journal";
$l_button["logbook"]["width"] = 100;

/*****************************************************************************
 * Info
 *****************************************************************************/
$l_button["revert_published"]["value"] = "Restore published version";
$l_button["revert_published"]["alt"] = "Discard changes and restore published version."; // TRANSLATE
$l_button["revert_published"]["width"] = 220;

$l_button["delete_all"]["value"] = "Supprimer tous";
$l_button["delete_all"]["alt"]   = "Supprimer tous";
$l_button["delete_all"]["width"] = 100;

$l_button["add"]["value"] = "Add";
$l_button["add"]["alt"] = "Ajouter";
$l_button["add"]["width"] = 100;

$l_button["select"]["value"] = "Select";
$l_button["select"]["alt"] = "Choisir";
$l_button["select"]["width"] = 100;

$l_button["edit"]["value"] = "Edit";
$l_button["edit"]["alt"] = "Éditer";
$l_button["edit"]["width"] = 100;

$l_button["editor"]["value"] = "Start Editor";
$l_button["editor"]["alt"] = ""; // TRANSLATE
$l_button["editor"]["width"] = 100;

$l_button["new_service"]["value"] = "New Service";
$l_button["new_service"]["alt"] = "Nouveau Service";
$l_button["new_service"]["width"] = 100;

$l_button["new_entry"]["value"] = "New Entry";
$l_button["new_entry"]["alt"] = "Nouvelle Entrée";
$l_button["new_entry"]["width"] = 100;

$l_button["attributes"]["value"] = "Attributes...";
$l_button["attributes"]["alt"] = "Attributs...";
$l_button["attributes"]["width"] = 100;

$l_button["selectAll"]["value"] = "Select all";
$l_button["selectAll"]["alt"]   = "Choisir tous";
$l_button["selectAll"]["width"] = 110;

$l_button["deselectAll"]["value"] = "Deselect all";
$l_button["deselectAll"]["alt"]   = "Démarquer tous";
$l_button["deselectAll"]["width"] = 110;

$l_button["prepend"]["value"] = "Prepend";
$l_button["prepend"]["alt"]   = "Insérer le texte choisi à l'avant du texte source";
$l_button["prepend"]["width"] = 110;

$l_button["append"]["value"] = "Append";
$l_button["append"]["alt"]   = "Insérer le texte choisi en arrière du texte source";
$l_button["append"]["width"] = 110;

$l_button["addCursorPosition"]["value"] = "Insert at cursor position";
$l_button["addCursorPosition"]["alt"]   = "Insert selected text at cursor position"; // TRANSLATE
$l_button["addCursorPosition"]["width"] = 180;

$l_button["startEditor"]["value"] = "Start editor";
$l_button["startEditor"]["alt"]   = "Démarrer l'éditeur par le Plugin-Editeur";
$l_button["startEditor"]["width"] = 100;

$l_button["refresh"]["value"] = "Refresh";
$l_button["refresh"]["alt"]   = "Mettre à jour";
$l_button["refresh"]["width"] = 100;

$l_button["ignore"]["value"] = "Ignore";
$l_button["ignore"]["alt"]   = "Ignore";
$l_button["ignore"]["width"] = 100;

$l_button["change"]["value"] = "Change";
$l_button["change"]["alt"]   = "Change";
$l_button["change"]["width"] = 100;

$l_button["apply"]["value"] = "Apply";
$l_button["apply"]["alt"]   = "Apply";
$l_button["apply"]["width"] = 100;

$l_button["check"]["value"] = "Check";
$l_button["check"]["alt"]   = "Check";
$l_button["check"]["width"] = 100;

$l_button["iconview"]["value"] = "Iconview";
$l_button["iconview"]["alt"]   = ""; // TRANSLATE
$l_button["iconview"]["width"] = 100;

$l_button["listview"]["value"] = "Listview";
$l_button["listview"]["alt"]   = ""; // TRANSLATE
$l_button["listview"]["width"] = 100;

$l_button["reindent"]["value"] = "Reindent code";
$l_button["reindent"]["alt"]   = "";
$l_button["reindent"]["width"] = 100;


$l_button["btn_messages_copy"]["alt"] = "Copier des Messages";
$l_button["btn_messages_create"]["alt"] = "Créer un Message";
$l_button["btn_messages_cut"]["alt"] = "Couper des Messages";
$l_button["btn_messages_paste"]["alt"] = "Coller des Messages";
$l_button["btn_messages_reply"]["alt"] = "Répondre à des Messges";
$l_button["btn_messages_tasks"]["alt"] = "Changer au System-Tâche";
$l_button["btn_messages_trash"]["alt"] = "Supprimer des Messages";
$l_button["btn_messages_update"]["alt"] = "Relever les nouveaux Messages";

$l_button["btn_task_copy"]["alt"] = "Copier des Tâches";
$l_button["btn_task_create"]["alt"] = "Créer des Tâches";
$l_button["btn_task_cut"]["alt"] = "Couper des Tâches";
$l_button["btn_task_forward"]["alt"] = "Transmettre des Tâches";
$l_button["btn_task_messages"]["alt"] = "Changer au System-Messages";
$l_button["btn_task_paste"]["alt"] = "Insérer des Tâches";
$l_button["btn_task_reject"]["alt"] = "Rejeter des Tâches";
$l_button["btn_task_status"]["alt"] = "Actualiser le état";
$l_button["btn_task_trash"]["alt"] = "Supprimer des Tâche";
$l_button["btn_task_update"]["alt"] = "Voir s'il y a des nouveau Tâche";


$l_button["btn_add_field"]["alt"] = "Ajouter un Champ";
$l_button["btn_add_link"]["alt"] = "Ajouter un lien";
$l_button["btn_add_listelement"]["alt"] = "Ajouter un element de liste";
$l_button["btn_add_schedule"]["alt"] = "Ajoute une tâche dirigée par temps";
$l_button["btn_add_template"]["alt"] = "Ajouter un modèle";
$l_button["btn_add_image"]["alt"] = "Ajouter une graphique";
$l_button["btn_add_thumbnail"]["alt"] = "Ajouter une imagette";
$l_button["btn_add_flash"]["alt"] = "Ajouter un Vidéo-Flash";
$l_button["btn_add_quicktime"]["alt"] = "Ajouter un Film-Quicktime";
$l_button["btn_add_file"]["alt"] = "Ajouter un Fichier";
$l_button["btn_add_cat"]["alt"] = "Ajouter une Catégorie";
$l_button["btn_fs_back"]["alt"] = "Réculer d'un plan";
$l_button["btn_new_dir"]["alt"] = "Créer un nouveau répertoire";
$l_button["btn_new_bannergroup"]["alt"] = "Créer un noveau groupe de banière";

$l_button["btn_direction_down"]["alt"] = "Vers le bas";
$l_button["btn_direction_left"]["alt"] = "À gauche";
$l_button["btn_direction_right"]["alt"] = "À droite";
$l_button["btn_direction_up"]["alt"] = "Vers le hauthaut";

$l_button["btn_edit_edit"]["alt"] = "Éditer";
$l_button["btn_edit_flash"]["alt"] = "Choisir un Vidéo-Flash";
$l_button["btn_edit_image"]["alt"] = "Éditer la Graphique";
$l_button["btn_edit_include"]["alt"] = "Éditer le fichier-inclus";
$l_button["btn_edit_link"]["alt"] = "Éditer le Lien";
$l_button["btn_edit_list"]["alt"] = "Éditer la Liste";
$l_button["btn_edit_object"]["alt"] = "Éditer l'Object";
$l_button["btn_edit_pdf"]["alt"] = "Choisir un document-PDF";
$l_button["btn_edit_quicktime"]["alt"] = "Choisir un Film-Quicktime";
$l_button["btn_edit_real"]["alt"] = "Choisir un fichier de Real Player";
$l_button["btn_edit_wmp"]["alt"] = "Choisir un fichier de Windows Media Player";

$l_button["btn_function_plus"]["alt"] = "Ajouter";
$l_button["btn_function_search"]["alt"] = "Rechercher";
$l_button["btn_function_trash"]["alt"] = "Supprimer";
$l_button["btn_function_view"]["alt"] = "Regarder";

$l_button["btn_select_image"]["alt"] = "Choisir une Graphique";
$l_button["btn_help"]["alt"] = "Aide";

$l_button["btn_open_sidebar"]["alt"] = "Open sidebar"; // TRANSLATE
$l_button["date_picker"]["alt"] = "Choisir le date";
$l_button['btn_add_note']['alt']='Ajouter une note';
$l_button['btn_function_reload']['alt']='Recharge la page';//TRANSLATE