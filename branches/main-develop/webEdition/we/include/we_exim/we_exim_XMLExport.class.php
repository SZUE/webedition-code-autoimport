	...
	function getSelectedItems($selection, $extype, $art, $type, $doctype, $classname, $categories, $dir, &$selDocs, &$selTempl, &$selObjs, &$selClasses){
		...
		switch($selection){
			case "manual":
				...
				break;
			case 'doctype':
				$cat_sql = ($categories ? ' AND ' . we_category::getCatSQLTail('', 'f', true, $this->db, 'Category', $categories) : '');
				if($dir != 0){
					$workspace = id_to_path($dir, FILE_TABLE, $this->db);
					$ws_where = ' AND (f.Path LIKE "' . $this->db->escape($workspace) . '/%" OR f.ID="' . $dir . '")';
				} else {
					$ws_where = '';
				}

				$this->db->query('SELECT distinct ID FROM ' . FILE_TABLE . ' f WHERE 1 ' . $ws_where . '  AND f.IsFolder=0 AND f.DocType="' . $this->db->escape($doctype) . '"' . $cat_sql);
				$selDocs = $this->db->getAll(true);
				break;
			default:
				if(defined('OBJECT_FILES_TABLE')){
					$where = $this->queryForAllowed(OBJECT_FILES_TABLE);
					$cat_sql = ' ' . ($categories ? ' AND ' . we_category::getCatSQLTail('', 'of', true, $this->db, 'Category', $categories) : '');
