...
<table id="footer">
	...
	protected function printCreateFolderHTML(){
		...
		$txt = $this->FolderText;
		if(!$txt){
			$weCmd->addMsg(g_l('modules_banner', '[group_empty]'), we_base_util::WE_MESSAGE_ERROR);
		} else {
			...
			if($this->db->next_record()){
				$weCmd->addMsg(sprintf(g_l('modules_banner', '[group_path_exists]'), $folder->Path), we_base_util::WE_MESSAGE_ERROR);
			} else if(preg_match('|[%/\\"\']|', $folder->Text)){
				$weCmd->addMsg(g_l('modules_banner', '[wrongtext]'), we_base_util::WE_MESSAGE_ERROR);
			} else {
				$folder->we_save();
	...
	protected function printDoRenameFolderHTML(){
		...
		if(!$txt){
			...
			if($this->db->next_record()){
				$weCmd->addMsg(sprintf(g_l('modules_banner', '[group_path_exists]')), we_base_util::WE_MESSAGE_ERROR);
			} else {
