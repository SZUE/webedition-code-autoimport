...
class we_glossary_frameEditorException extends we_glossary_frameEditor{
	...
	function Body(&$weGlossaryFrames){
		...
		$out = $js . we_html_element::htmlDiv(array('id' => 'tab1', 'style' => ($tabNr == 1 ? '' : 'display: none')), we_multiIconBox::getHTML('weMultibox', "100%", we_glossary_frameEditorException::getHTMLTabProperties($weGlossaryFrames), 30, '', -1, '', '', false));

		return self::buildBody($weGlossaryFrames, $out);
	}

	function Footer(&$weGlossaryFrames){
		$_we_button = we_button::create_button("save", "javascript:top.opener.top.we_cmd('save_exception')", true, 100, 22, '', '', (!permissionhandler::hasPerm('NEW_GLOSSARY') && !permissionhandler::hasPerm('EDIT_GLOSSARY')));
