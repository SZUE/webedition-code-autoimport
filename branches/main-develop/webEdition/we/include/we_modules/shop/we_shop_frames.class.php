...
// wether the resultset ist empty?
		...
		$iconBarTable->setCol(0, $c++, null, we_html_button::create_button('fa:btn_shop_extArt,fa-lg fa-cart-plus', "javascript:top.opener.top.we_cmd('new_article')", '', 0, 0, "", "", !we_base_permission::hasPerm("NEW_USER")));
		$iconBarTable->setCol(0, $c++, null, we_html_button::create_button('fa:btn_shop_delOrd,fa-lg fa-shopping-cart,fa-lg fa-trash-o', "javascript:top.opener.top.we_cmd('delete_shop')", '', 0, 0, "", "", !we_base_permission::hasPerm("NEW_USER")));
		...
		$iconBarTable->setCol(0, $c++, null, we_html_button::create_button('fa:btn_shop_pref,fa-lg fa-pencil,fa-lg fa-list-alt', "javascript:top.opener.top.we_cmd('pref_shop')", '', 0, 0, "", "", !we_base_permission::hasPerm("NEW_USER")));
		$iconBarTable->setCol(0, $c++, null, we_html_button::create_button('fa:btn_payment_val,fa-lg fa-long-arrow-right,fa-lg fa-money', "javascript:top.opener.top.we_cmd('payment_val')", '', 0, 0, "", "", !we_base_permission::hasPerm("NEW_USER")));
...
// wether the resultset ist empty?
		...
		if($home){
			...
			$bodyURL = WEBEDITION_DIR . 'we_showMod.php?mod=shop&pnt=edit_shop_article_extend&typ=document';
		} elseif(!$resultD && $resultO){ // no docs but objects
			$bodyURL = WEBEDITION_DIR . 'we_showMod.php?mod=shop&pnt=edit_shop_article_extend&typ=object&ViewClass=' . $classid;
		} elseif($resultD && $resultO){
			$bodyURL = WEBEDITION_DIR . 'we_showMod.php?mod=shop&pnt=edit_shop_article_extend&typ=document';
		} else {
			$bodyURL =  WEBEDITION_DIR . 'we_showMod.php?mod=shop&pnt=edit_shop_revenueTop';
		}
...
function setTab(tab) {
	switch (tab) {
		...
		case ' . self::TAB_ADMIN2 . ':
			parent.edbody.document.location = WE().consts.dirs.WEBEDITION_DIR + "we_showMod.php?mod=shop&pnt=edit_shop_article_extend&typ=object&ViewClass=' . $classid . '";
			break;
		' . (isset($yearTrans) ? '
		case ' . self::TAB_ADMIN2 . ':
	...
	public function getHTML($what = '', $mode = '', $step = 0){
		...
		switch($what){
			...
			case 'edit_shop_article_extend':
				$protect = we_base_moduleInfo::isActive(we_base_moduleInfo::SHOP) && we_users_util::canEditModule(we_base_moduleInfo::SHOP) ? null : [false];
				we_html_tools::protect($protect);
				return we_shop_articleExtend::getDialog();
			case 'edit_shop_revenueTop':
				$protect = we_base_moduleInfo::isActive(we_base_moduleInfo::SHOP) && we_users_util::canEditModule(we_base_moduleInfo::SHOP) ? null : [false];
				we_html_tools::protect($protect);
				return we_shop_revenueTop::getDialog();
			default:
