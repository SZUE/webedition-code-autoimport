...
class we_shop_view extends we_modules_view{
	...
	function getJSProperty(array $jsVars = []){
		return we_html_element::jsScript(WE_JS_MODULES_DIR . 'shop/we_shop_property.js');
	}
	...
	function getProperties(){
		...
		$orderData = getHash('SELECT customerID,customOrderNo,pricesNet,calcVat,shippingCost,shippingNet,shippingVat,customFields' . ($format ? ',' . implode(',', $format) : '') . ' FROM ' . SHOP_ORDER_TABLE . ' WHERE ID=' . $bid);
...
<table style="width:300px">
	<tr>
		...
		<table style="width:99%" class="default defaultfont">' .
			$this->getMailDates($weShopStatusMails, $showDateFields, $advanced, $orderData, $bid) . '
			<tr height="5">
...
// table row of one article
		...
		we_html_element::jsScript(WE_JS_MODULES_DIR . 'shop/we_shop_view2.js', '', ['id' => 'loadVarShop', 'data-shop' => setDynamicVar([
				'bid' => $bid,
		...
		$parts = [
			['html' => $orderDataTable],
			['html' => $orderTable],
			['html' => $this->getCustomCartFieldsTable($bid, $customCartFields)]
		];
	...
	function processCommands(we_base_jsCmd $jscmd){
		...
		<form name="we_intern_form">' . we_html_element::htmlHiddens(['bid' => $_REQUEST['bid'],
			...
			<table class="default">
				...
				echo we_html_tools::getHtmlTop('', '', '', we_html_element::jsScript(WE_JS_MODULES_DIR . 'shop/we_shop_view2.js', '', ['id' => 'loadVarShop', 'data-shop' => setDynamicVar([
							'cmd0' => $_REQUEST['we_cmd'][0],
							'bid' => $_REQUEST['bid'],
							'searchArticle' => (isset($_REQUEST['searchArticle']) ? $_REQUEST['searchArticle'] : ''),
							'page' => $page
					])])
	...
	public function getHomeScreen(){
		...
		$content = we_html_button::create_button('pref_shop', "javascript:we_cmd('pref_shop');", '', 0, 0, "", "", !we_base_permission::hasPerm("NEW_USER")) . '<br/>' .
			we_html_button::create_button('payment_val', "javascript:we_cmd('payment_val');", '', 0, 0, "", "", !we_base_permission::hasPerm("NEW_USER")) . '<br/>';
		if(($resultD) && $resultO){ //docs and objects
			...
			$content .= we_html_button::create_button('quick_rev', "javascript:top.content.editor.location='" . WEBEDITION_DIR . 'we_showMod.php?mod=shop' . "&pnt=editor&top=1&typ=object&ViewClass=$classid '") . '<br/>';
	...
	private function getMailDates($weShopStatusMails, $showDateFields, $advanced, $orderData, $bid){
		$dateform = '00.00.0000';
		...
		foreach($showDateFields as $field){
			...
			<tr height="25">
				...
				<td style="width:300px;height:25px"  class="defaultfont">' . $weShopStatusMails->getEMailHandlerCode(substr($field, 4), $advanced[$field]) . '</td>
				</tr>';
		}
		return $ret;
	}
