...
abstract class we_shop_revenueTop{
	...
	private static function yearSelect($select_name){
		$yearStart = 2001;
		$yearNow = date('Y');
		$opts = [];
	...
	private static function monthSelect($select_name, $selectedMonth){
	...
	public static function getDialog(){
		global $selectedYear, $selectedMonth, $orderDesc, $actPage;
...
// get some preferences!
		...
		$parts = [
			[
				...
				'html' => self::yearSelect("ViewYear"),
				'space' => we_html_multiIconBox::SPACE_MED2,
				'noline' => 1
			], [
				'headline' => '<label for="ViewMonth">' . g_l('modules_shop', '[selectMonth]') . '</label>',
				'html' => self::monthSelect("ViewMonth", $selectedMonth),
				'space' => we_html_multiIconBox::SPACE_MED2,
				'noline' => 1
			], [
...
FROM ' . SHOP_ORDER_TABLE . ' o JOIN ' . SHOP_ORDER_ITEM_TABLE . ' oi ON o.ID=oi.orderID WHERE ' . $queryShopDateCondtion;
			...
			$headline = [
				['dat' => self::getTitleLink(g_l('modules_shop', '[bestellung]'), 'o.ID')],
				['dat' => g_l('modules_shop', '[ArtName]')], // 'shoptitle'
				['dat' => g_l('modules_shop', '[anzahl]')],
				['dat' => self::getTitleLink(g_l('modules_shop', '[artPrice]'), 'Price')],
				['dat' => g_l('modules_shop', '[Gesamt]')],
				['dat' => self::getTitleLink(g_l('modules_shop', '[artOrdD]'), 'DateOrder')],
				['dat' => self::getTitleLink(g_l('modules_shop', '[ArtID]'), 'orderDocID')],
				['dat' => self::getTitleLink(g_l('modules_shop', '[artPay]'), 'DatePayment')],
			];
			$content = [];
...
ORDER BY ' . we_base_request::_(we_base_request::STRING, 'orderBy', 'o.ID') . ($orderDesc ? ' DESC' : '') . ' LIMIT ' . ($actPage * $nrOfPage) . ',' . $nrOfPage);
			...
			$parts[] = ['html' => we_shop_pager::getStandardPagerHTML(self::getPagerLink(), $actPage, $nrOfPage, $maxRows),
			];
		} else {
		...
		return we_html_tools::getHtmlTop('', '', '', we_html_element::jsScript(WE_JS_MODULES_DIR . 'shop/edit_shop_revenueTop.js'), we_html_element::htmlBody(['class' => "weEditorBody",
