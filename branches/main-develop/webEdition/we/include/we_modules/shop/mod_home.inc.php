...
if(($resultD > 0) && (!empty($resultO))){ //docs and objects
	$prefshop2 = we_button::create_button("quick_rev", "javascript:top.content.right.editor.location='" . WE_MODULES_DIR . "shop/edit_shop_frameset.php?pnt=editor&top=1&typ=document '", true);
} elseif(($resultD < 1) && (!empty($resultO))){ // no docs but objects
	$prefshop2 = we_button::create_button("quick_rev", "javascript:top.content.right.editor.location='" . WE_MODULES_DIR . "shop/edit_shop_frameset.php?pnt=editor&top=1&typ=object&ViewClass=$classid '", true);
