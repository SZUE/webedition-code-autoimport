		...
		if($GLOBALS['DB_WE']->num_rows()){
			...
			while($GLOBALS['DB_WE']->next_record()) {
				...
				if(we_hasPerm("EDIT_SHOP_ORDER")){
					$orderStr .= ($sameModul ?
							('<td>' . we_button::create_button('image:btn_edit_edit', 'javascript:top.content.resize.shop_properties.location = \'' . WE_SHOP_MODULE_DIR . 'edit_shop_editorFrameset.php?bid=' . $GLOBALS['DB_WE']->f('IntOrderID') . '\';') . '</td>') :
