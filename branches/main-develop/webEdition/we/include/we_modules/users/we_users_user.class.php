...
class we_users_user{
	...
	// Preferences array
	private $preference_slots = ['sizeOpt', 'weWidth', 'weHeight', 'usePlugin', 'autostartPlugin', 'promptPlugin', 'Language', 'BackendCharset', 'seem_start_file',
	...
	function saveToDB(we_base_jsCmd $jsCmd){
		$db_tmp = new DB_WE();
		...
		$this->saveToSession($jsCmd);
		return true;
	}

	function saveToSession(we_base_jsCmd $jsCmd){
		if($this->ID != $_SESSION['user']['ID']){
			return;
		}

		$this->rememberPreference($jsCmd, isset($this->Preferences['Language']) ? $this->Preferences['Language'] : null, 'Language');
		$this->rememberPreference($jsCmd, isset($this->Preferences['BackendCharset']) ? $this->Preferences['BackendCharset'] : null, 'BackendCharset');
		$this->rememberPreference($jsCmd, isset($this->Preferences['default_tree_count']) ? $this->Preferences['default_tree_count'] : null, 'default_tree_count');

		if(isset($this->Preferences['seem_start_type'])){
			switch($this->Preferences['seem_start_type']){
				case 'cockpit':
					$this->rememberPreference($jsCmd, 0, 'seem_start_file');
					$this->rememberPreference($jsCmd, "cockpit", 'seem_start_type');
					$this->rememberPreference($jsCmd, '', 'seem_start_weapp');
					break;
				case 'object':
					$this->rememberPreference($jsCmd, isset($this->Preferences['seem_start_object']) ? $this->Preferences['seem_start_object'] : 0, 'seem_start_file');
					$this->rememberPreference($jsCmd, 'object', 'seem_start_type');
					$this->rememberPreference($jsCmd, '', 'seem_start_weapp');
					break;
				case 'weapp':
					$this->rememberPreference($jsCmd, (isset($this->Preferences['seem_start_weapp']) ? $this->Preferences['seem_start_weapp'] : ''), 'seem_start_weapp');
					$this->rememberPreference($jsCmd, 'weapp', 'seem_start_type');
					$this->rememberPreference($jsCmd, 0, 'seem_start_file');
					break;
				case 'document':
					$this->rememberPreference($jsCmd, isset($this->Preferences['seem_start_document']) ? $this->Preferences['seem_start_document'] : 0, 'seem_start_file');
					$this->rememberPreference($jsCmd, 'document', 'seem_start_type');
					$this->rememberPreference($jsCmd, '', 'seem_start_weapp');
					break;
				default:
					$this->rememberPreference($jsCmd, '0', 'seem_start_type');
					$this->rememberPreference($jsCmd, '', 'seem_start_weapp');
					$this->rememberPreference($jsCmd, 0, 'seem_start_file');
			}
		}

		$this->rememberPreference($jsCmd, isset($this->Preferences['sizeOpt']) ? $this->Preferences['sizeOpt'] : null, 'sizeOpt');
		$this->rememberPreference($jsCmd, isset($this->Preferences['weWidth']) ? $this->Preferences['weWidth'] : null, 'weWidth');
		$this->rememberPreference($jsCmd, isset($this->Preferences['weHeight']) ? $this->Preferences['weHeight'] : null, 'weHeight');
		$this->rememberPreference($jsCmd, isset($this->Preferences['editorMode']) ? $this->Preferences['editorMode'] : null, 'editorMode');
		$this->rememberPreference($jsCmd, isset($this->Preferences['editorFont']) ? $this->Preferences['editorFont'] : null, 'editorFont');
		$this->rememberPreference($jsCmd, isset($this->Preferences['editorFontname']) ? $this->Preferences['editorFontname'] : null, 'editorFontname');
		$this->rememberPreference($jsCmd, isset($this->Preferences['editorFontsize']) ? $this->Preferences['editorFontsize'] : null, 'editorFontsize');
		$this->rememberPreference($jsCmd, isset($this->Preferences['editorSizeOpt']) ? $this->Preferences['editorSizeOpt'] : null, 'editorSizeOpt');
		$this->rememberPreference($jsCmd, isset($this->Preferences['editorWidth']) ? $this->Preferences['editorWidth'] : null, 'editorWidth');
	...
	private function setPermission($perm_name, $perm_value){
	...
	private function loadWorkspaces(){
		if($this->workSpace){
	...
	private function rememberPreference(we_base_jsCmd $jsCmd, $settingvalue, $settingname){
			...
			switch($settingname){
				case 'Language'://FIXME: reload corrupt. fix when needless frames are eliminated
					...
					if($settingvalue != $GLOBALS['WE_LANGUAGE']){
						$jsCmd->addCmd('rememberPreference', 'Language', we_base_request::_(we_base_request::INT, 'tab', 0));
					}
					break;
				case 'BackendCharset'://FIXME: reload corrupt. fix when needless frames are eliminated
					...
					if($settingvalue != $GLOBALS['WE_BACKENDCHARSET']){
						$jsCmd->addCmd('rememberPreference', 'BackendCharset', we_base_request::_(we_base_request::INT, 'tab', 0));
					}
					break;
				...
				case 'weWidth':
					if($_SESSION['prefs']['sizeOpt'] == 1){
						...
						if($generate_java_script){
							...
							$jsCmd->addCmd('rememberPreference', 'weWidth', [$settingvalue, $height]);
						}
					}
					break;
				...
				case 'editorFont':
					...
					$jsCmd->addCmd('rememberPreference', 'reload', '');
					break;
	...
	function formDefinition(we_base_jsCmd $jsCmd, $tab, $perm_branch){
		...
		switch($tab){
			...
			case self::TAB_WORKSPACES:
				return $this->formWorkspace();
			case self::TAB_SETTINGS:
				return $this->formPreferences($jsCmd, $perm_branch);
		}
		foreach(array_keys($this->extensions_slots) as $k){
			return $this->extensions_slots[$k]->formDefinition($jsCmd, $tab, $perm_branch);
		}
	...
	function formGroupData(){
		...
		$tableObj->setCol(1, 0, ['style' => 'padding-top:5px;'], we_html_tools::htmlFormElementTable($description, g_l('modules_users', '[description]')));
		$tableObj->setCol(2, 0, ['style' => 'padding-top:10px;'], we_html_tools::htmlFormElementTable($weAcSelector, g_l('modules_users', '[group]')));
	 ...
	 * @return     string
	 */
	private function formPermissions(we_base_jsCmd $jsCmd, $branch){
		// Set output text
		...
		$content = $dynamic_controls->fold_checkbox_groups($this->permissions_slots, $parentPerm, $this->permissions_main_titles, $this->permissions_titles, $this->Name, $branch, [
...
function toggleRebuildPerm(disabledOnly) {';
	...
	toggleRebuildPerm(true);";
		...
		foreach($this->permissions_slots as $gname => $group){
			foreach($group as $pname => $pvalue){
				if($pname != 'ADMINISTRATOR'){
					...
					$defaultjs .= 'document.we_form.' . $this->Name . '_Permission_' . $pname . '.checked = ' . (!empty($this->permissions_defaults[$gname][$pname]) ? 'true' : 'false') . ';';
				}
			}
		}

		$button_uncheckall = we_html_button::create_button('uncheckall', 'javascript:' . $uncheckjs . 'top.content.setHot();');
		$button_checkall = we_html_button::create_button('checkall', 'javascript:' . $checkjs . 'top.content.setHot();');
		$button_default = we_html_button::create_button('default', 'javascript:' . $defaultjs . 'top.content.setHot();');
		$parts[] = ['headline' => '',
		...
		if($this->ParentID){
			$jsCmd->addCmd('showParentPerms', ($this->ParentPerms ? 1 : 0));
			$parts[] = ['headline' => '',
...
<tr><td colspan="2">' . $weAcSelector . '</td>
	...
	private function formPreferences(we_base_jsCmd $jsCmd, $branch = ''){
		//FIXME: change we_html_dynamicControls
		$dynamic_controls = new we_html_dynamicControls();
...
//FIXME: where is the difference between force_glossary_check + force_glossary_action?!
	...
	private function formPreferencesUI(we_base_jsCmd $jsCmd){
		$settings = [];
		// LANGUAGE
		...
		if(!empty($language)){ // Build language select box
			$languages = new we_html_select(['name' => $this->Name . '_Preference_Language', 'class' => 'weSelect']);
			...
			// Build dialog
			$settings[] = ['headline' => g_l('prefs', '[choose_language]'), 'html' => $languages->getHtml(), 'space' => we_html_multiIconBox::SPACE_BIG, 'noline' => 1];
		}
		...
		$settings[] = ['headline' => g_l('prefs', '[choose_backendcharset]'),
			'html' => $charset->getHtml(),
			'space' => we_html_multiIconBox::SPACE_BIG
		];

		//AMOUNT Number of Columns
		$amount = new we_html_select(['name' => $this->Name . '_Preference_cockpit_amount_columns', 'class' => 'weSelect', 'onchange' => "top.content.setHot();"]);
		...
		$settings[] = ['headline' => g_l('prefs', '[cockpit_amount_columns]'),
			'html' => $amount->getHtml(),
			'space' => we_html_multiIconBox::SPACE_BIG
		];
		...
		$start_type = new we_html_select(['name' => 'seem_start_type', 'class' => 'weSelect', 'id' => 'seem_start_type', 'onchange' => "show_seem_chooser(this.value); top.content.setHot();"]);
		...
		$seem_weapp_chooser = we_html_element::htmlSpan(['id' => 'seem_start_weapp', 'style' => 'display:none'], $start_weapp->getHtml());
		...
		if(we_base_permission::hasPerm('CHANGE_START_DOCUMENT')){
			$jsCmd->addCmd('show_seem_chooser', $seem_start_type);
			$settings[] = ['headline' => g_l('prefs', '[seem_startdocument]'),
				'html' => $seem_html->getHtml(),
				'space' => we_html_multiIconBox::SPACE_BIG
			];
		}
		...
		$file_tree_count = new we_html_select(['name' => $this->Name . '_Preference_default_tree_count', 'class' => 'weSelect', 'onchange' => 'top.content.setHot();']);
		...
		$settings[] = ['headline' => g_l('prefs', '[tree_title]'), 'html' => we_html_tools::htmlAlertAttentionBox(g_l('prefs', '[tree_count_description]'), we_html_tools::TYPE_INFO) . '<br/>' . $file_tree_count->getHtml(),
			'space' => we_html_multiIconBox::SPACE_BIG];
...
//		$window_specify_table->setCol(0, 0, ['class' => 'defaultfont', 'style' => 'padding-left:40px;padding-right:10px;'], g_l('prefs', '[width]') . ':');
		...
		// Build dialog
		$settings[] = ["headline" => g_l('prefs', '[predefined]'), "html" => $window_predefined_table->getHtml(), 'space' => we_html_multiIconBox::SPACE_BIG];
	...
	function formPreferencesEditor(){
		...
		//Editor Mode
		$template_editor_mode = new we_html_select(['class' => "weSelect", "name" => $this->Name . "_Preference_editorMode", "onchange" => "displayEditorOptions(this.options[this.options.selectedIndex].value);"]);
		...
		$settings = [["headline" => g_l('prefs', '[editor_mode]'), "html" => $template_editor_mode->getHtml(), 'space' => we_html_multiIconBox::SPACE_MED2]
		];
//FIXME: use code from preferences for font-selection
		$template_fonts = ['Arial', 'Courier', 'Courier New', 'Helvetica', 'Monaco', 'Mono', 'Tahoma', 'Verdana', 'serif', 'sans-serif', 'none'];
		$template_font_sizes = [8, 9, 10, 11, 12, 14, 16, 18, 24, 32, 48, 72, -1];
		...
		// Create specify window dimension input
		$template_editor_font_specify_table = new we_html_table(['class' => 'default withSpace', 'style' => 'margin:5px 0px 0px 50px;'], 2, 2);

		$template_editor_font_specify_table->setCol(0, 0, ['class' => 'defaultfont', 'style' => 'padding-right:10px;'], g_l('prefs', '[editor_fontname]') . ":");
		$template_editor_font_specify_table->setCol(0, 1, null, $template_editor_font_select_box->getHtml());
		$template_editor_font_specify_table->setCol(1, 0, ['class' => 'defaultfont', 'style' => 'padding-right:10px;'], g_l('prefs', '[editor_fontsize]') . ":");
		$template_editor_font_specify_table->setCol(1, 1, null, $template_editor_font_sizes_select_box->getHtml());

		// Build dialog
		$settings[] = ['headline' => g_l('prefs', '[editor_font]'),
	...
	function formAliasData(){
		...
		$weAcSelectorName = $weSuggest->getHTML();
		...
		$weAcSelectorGroup = $weSuggest->getHTML();
...
<table class="default" style="width:500px;">
	...
	static function setEffectiveWorkspaces($user, we_database_base $db, $onlyParent = false){
		$workspaces = [
			FILE_TABLE => ['key' => 'workSpace', 'value' => [], 'parent' => 0, 'parentKey' => 'ParentWs', 'explodeValue' => true],
			TEMPLATES_TABLE => ['key' => 'workSpaceTmp', 'value' => [], 'parent' => 0, 'parentKey' => 'ParentWst', 'explodeValue' => true],
			NAVIGATION_TABLE => ['key' => 'workSpaceNav', 'value' => [], 'parent' => 0, 'parentKey' => 'ParentWsn', 'explodeValue' => true],
		];

		if(defined('OBJECT_FILES_TABLE')){
			$workspaces[OBJECT_FILES_TABLE] = ['key' => 'workSpaceObj', 'value' => [], 'parent' => 0, 'parentKey' => 'ParentWso', 'explodeValue' => true];
		}

		if(defined('NEWSLETTER_TABLE')){
			$workspaces[NEWSLETTER_TABLE] = ['key' => 'workSpaceNwl', 'value' => [], 'parent' => 0, 'parentKey' => 'ParentWsnl', 'explodeValue' => true];
		}

		if(defined('CUSTOMER_TABLE')){
			$workspaces[CUSTOMER_TABLE] = ['key' => 'workSpaceCust', 'value' => [], 'parent' => 0, 'parentKey' => 'ParentWsCust', 'explodeValue' => false];
		}
	...
	static function writePrefs($id, $db, array $data = []){
		...
		foreach($old as $key => $val){
			if($key != 'userID' && (!isset($data[$key]) || $data[$key] != $val)){
				$upd[] = '(' . $id . ',"' . $db->escape($key) . '","' . $db->escape(isset($data[$key]) ? (is_array($data[$key]) ? implode(',', $data[$key]) : $data[$key]) : (is_array($val) ? implode(',', $val) : $val)) . '")';
			}
		}
		if(!empty($upd)){
