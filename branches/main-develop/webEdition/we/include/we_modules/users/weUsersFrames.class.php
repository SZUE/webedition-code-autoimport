...
function urlEntry(icon,name,vorfahr,text,contentType,table,published,denied) {
	...
	*/
	...
	function getHTMLProperties(){
		...
		$content = we_html_element::htmlHidden($attribs = array("ucmd" => "",)). 
			we_html_element::htmlHidden($attribs = array("ucmd" => "",)).
		...
		$form = we_html_element::htmlForm($_form, $content);
		
		print we_html_element::htmlBody(array('class' => 'weEditorBody', 'onload' => 'loaded = 1;', 'onUnload' => 'doUnload()'), $content);
	}
