...
class we_messaging_messaging extends we_class{
	...
	function set_search_settings($search_fields, $search_folder_ids){
		$this->search_fields = $this->search_folder_ids = [];
	...
	function save_addresses(&$addressbook){
		...
		foreach($addressbook as $elem){
			if(!empty($elem)){
				$this->DB_WE->query('INSERT INTO ' . MSG_ADDRBOOK_TABLE . ' SET ' . we_database_base::arraySetter(['UserID' => $this->userid,
						'strMsgType' => $elem[0],
						'strID' => $elem[1],
						'strAlias' => $elem[2],
						]
