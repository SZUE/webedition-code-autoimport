...
$onload = ($res['ok'] && substr(we_base_request::_(we_base_request::STRING, "mode"), 0, 2) != 'u_' ?
		'if (opener && opener.top && opener.top.content) {opener.top.content.update_messaging();}' : '');

echo we_html_tools::getHtmlTop(g_l('modules_messaging', '[message_send]'), '', '', '', we_html_element::htmlBody([
			'class' => "weDialogBody",
			'onload' => $onload
				], we_html_tools::htmlDialogLayout($tbl, g_l('modules_messaging', '[message_send]') . '...', we_html_button::create_button(we_html_button::OK, "javascript:window.close()"), "100%", 20, "", "hidden")));
