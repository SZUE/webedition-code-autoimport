...
if(we_base_request::_(we_base_request::BOOL, 'save')){
	...
	echo we_html_tools::getHtmlTop(g_l('modules_messaging', '[search_advanced]'), '', '', '', we_html_element::htmlBody([
			'onload' => "window.close();"
	]));
	exit();
}
echo we_html_tools::getHtmlTop(g_l('modules_messaging', '[search_advanced]'), '', '', we_html_element::jsScript(WE_JS_MODULES_DIR . 'messaging/messaging.js'), we_html_element::htmlBody([
		'class' => "weDialogBody"
		], '<form action="' . WE_MESSAGING_MODULE_DIR . 'messaging_search_advanced.php" name="search_adv">' .
		we_html_element::htmlHiddens(["we_transaction" => we_base_request::_(we_base_request::TRANSACTION, 'we_transaction'),
			"save" => 1]) .
		we_html_tools::htmlDialogLayout('<table cellpadding="10">
<tr>
...
</table>', "", we_html_button::position_yes_no_cancel(we_html_button::create_button(we_html_button::OK, "javascript:save_settings();"), null, we_html_button::create_button(we_html_button::CANCEL, "javascript:self.close()")), "90%") .
		'</form>'
));
