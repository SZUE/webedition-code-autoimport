		...
		switch($_REQUEST["mcmd"]){
			...
			case 'show_message':
				if(isset($id)){
					...
					top.content.resize.right.editor.edbody.msg_mfv.messaging_msg_view.location="' . (WE_MESSAGING_MODULE_DIR . 'messaging_message_view.php?we_transaction=' . $this->transaction . '&id= ' . $id) . '";
					');
				}
			...
			case 'paste_msg':
				...
				$js_out = '
				top.content.resize.right.editor.edbody.entries_selected = new Array();
				top.content.resize.right.editor.edbody.messaging_fv_headers.location="' . we_class::url($this->frameset) . '&pnt=msg_fv_headers&si=' . $this->messaging->get_sortitem() . '&so=' . $this->messaging->get_sortorder() . '&viewclass=" + top.content.viewclass;
				top.content.resize.right.editor.edbody.msg_mfv.messaging_messages_overview.location="' . we_class::url(WE_MESSAGING_MODULE_DIR . 'messaging_show_folder_content.php') . '";
				top.content.resize.right.editor.edbody.msg_mfv.messaging_msg_view.location="' . HTML_DIR . 'white.html";
				';
			...
			case 'edit_folder':
				if($_REQUEST['mode'] == 'new' || ($_REQUEST['mode'] == 'edit')){
					$out .= we_html_element::jsElement('
					top.content.resize.right.editor.location = "' . WE_MESSAGING_MODULE_DIR . 'messaging_edit_folder.php?we_transaction=' . $this->transaction . '&mode=' . $_REQUEST['mode'] . '&fid=' . (isset($_REQUEST['fid']) ? $_REQUEST['fid'] : -1) . '";
					');
				}
				return $out;
				break;
...
top.content.drawEintraege();
	...
	private function print_fc_html($blank = true){
		...
		return we_html_element::jsElement('
top.content.resize.right.editor.edbody.entries_selected = new Array(' . $this->messaging->get_ids_selected() .');
top.content.resize.right.editor.edbody.messaging_fv_headers.location="' . we_class::url($this->frameset) . '&pnt=msg_fv_headers&si=' . $this->messaging->get_sortitem() . '&so=' . $this->messaging->get_sortorder() . '&viewclass=" + top.content.viewclass;
...
($blank ? 'top.content.resize.right.editor.edbody.msg_mfv.messaging_msg_view.location="' . HTML_DIR . 'white.html";' : '')
	...
	private function get_folder_content($id, $sort = '', $entrsel = '', $searchterm = '', $usecache = 1){
		...
		if($id != $this->messaging->Folder_ID){
			$this->messaging->reset_ids_selected();
			$out = we_html_element::jsElement('top.content.resize.right.editor.edbody.last_entry_selected = -1;');
		}
