	...
	private function getJSCodeTodo(){
		return we_html_element::jsElement('
			...
			function copy_messages() {
				if (top.content.resize.right.editor.edbody.entries_selected && top.content.resize.right.editor.edbody.entries_selected.length > 0) {
					top.content.cmd.location = "' . WE_MESSAGING_MODULE_DIR . 'edit_messaging_frameset.php?pnt=cmd&we_transaction=' . $this->weTransaction . '&mcmd=copy_msg&entrsel=" + top.content.resize.right.editor.edbody.entries_selected.join(",");
				}
			}

			function cut_messages() {
				if (top.content.resize.right.editor.edbody.entries_selected && top.content.resize.right.editor.edbody.entries_selected.length > 0) {
					top.content.cmd.location = "' . WE_MESSAGING_MODULE_DIR . 'edit_messaging_frameset.php?pnt=cmd&we_transaction=' . $this->weTransaction . '&mcmd=cut_msg&entrsel=" + top.content.resize.right.editor.edbody.entries_selected.join(",");
				}
			}

			function paste_messages() {
				if (top.content.resize.right.editor.edbody.entries_selected) {
					top.content.cmd.location = "' . WE_MESSAGING_MODULE_DIR . 'edit_messaging_frameset.php?pnt=cmd&we_transaction=' . $this->weTransaction . '&mcmd=paste_msg&entrsel=" + top.content.resize.right.editor.edbody.entries_selected.join(",");
				}
			}
			...
			function delete_messages() {
				if (top.content.resize.right.editor.edbody.entries_selected && top.content.resize.right.editor.edbody.entries_selected.length > 0) {
					...
					top.content.cmd.location = "' . WE_MESSAGING_MODULE_DIR . 'edit_messaging_frameset.php?pnt=cmd&we_transaction=' . $this->weTransaction . '&mcmd=delete_msg&entrsel=" + top.content.resize.right.editor.edbody.entries_selected.join(",");
				}
			}
	...
	private function getJSCodeMsg(){
		...
		return we_html_element::jsElement('
			function new_message(mode) {
				if (mode == "re" && (top.content.resize.right.editor.edbody.last_entry_selected == -1)) {
					return;
				}
