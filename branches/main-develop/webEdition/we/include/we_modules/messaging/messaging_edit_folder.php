...
if(we_base_request::_(we_base_request::STRING, 'mcmd') === 'save_folder_settings'){
	...
	if($ID >= 0){
		...
		$jsCmd->addCmd('reloadMsgContent', [
			'query' => '&we_transaction=' . $transaction . '&mcmd=save_folder_settings&name=' . $foldername . '&id=' . $ID . '&mode=' . $mode . '&parent_id=' . $parentfolder . '&type=' . $types,
...
echo we_html_tools::getHtmlTop(g_l('modules_messaging', '[folder_settings]'), '', '', we_html_element::jsScript(WE_JS_MODULES_DIR . 'messaging/messaging.js') . $jsCmd->getCmds(), we_html_element::htmlBody([
		...
		'style' => "border-top: 1px solid black;"
			...
			'name' => "edit_folder",
			'action' => WE_MESSAGING_MODULE_DIR . 'messaging_edit_folder.php',
			'method' => "post"], we_html_element::htmlHiddens(['we_transaction' => $transaction,
				'mcmd' => 'save_folder_settings',
				'mode' => $mode,
				($fid ? 'fid' : '') => $fid
			]) .
			we_html_tools::htmlDialogLayout('<table cellpadding="5" >
	<tr>
	...
	<tr>
      ...
      </table>', $heading, we_html_button::position_yes_no_cancel(we_html_button::create_button(we_html_button::SAVE, "javascript:save()"), "", we_html_button::create_button(we_html_button::CANCEL, "javascript:top.content.we_cmd('messaging_start_view','', '" . we_base_request::_(we_base_request::TABLE, "table", "") . "')")
				), "100%", 30, "", "none")
		)
));
