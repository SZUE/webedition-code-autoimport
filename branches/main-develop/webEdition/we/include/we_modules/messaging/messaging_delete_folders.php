...
if(isset($_REQUEST['mcmd']) && $_REQUEST['mcmd'] == 'delete_folders'){
	...
	if($folders[0] != ""){
		...
		if($v > 0){
			...
			?>
							top.content.cmd.location = '<?php print WE_MESSAGING_MODULE_DIR; ?>messaging_cmd.php?we_transaction=<?php echo $_REQUEST['we_transaction'] ?>&mcmd=delete_folders&folders=<?php echo join(',', $v) ?>';
