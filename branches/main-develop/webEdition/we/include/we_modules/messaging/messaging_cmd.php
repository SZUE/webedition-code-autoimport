...
function print_fc_html($blank = true){
	...
	<script type="text/javascript">
		<!--
		top.content.resize.right.editor.entries_selected = new Array(<?php echo $GLOBALS['messaging']->get_ids_selected() ?>);
		top.content.resize.right.editor.edbody.messaging_fv_headers.location="<?php echo we_class::url($this->frameset) . '&pnt=msg_fv_headers&si=' . $GLOBALS['messaging']->get_sortitem() . '&so=' . $GLOBALS['messaging']->get_sortorder(); ?>&viewclass=" + top.content.viewclass;
		if (top.content.resize.right.editor.edbody.msg_mfv.messaging_messages_overview) {
			top.content.resize.right.editor.edbody.msg_mfv.messaging_messages_overview.location="<?php echo we_class::url(WE_MESSAGING_MODULE_DIR . 'messaging_show_folder_content.php'); ?>";
		}


	<?php
	if($blank){
		echo 'top.content.resize.right.editor.edbody.msg_mfv.messaging_msg_view.location="' . HTML_DIR . 'white.html";';
	}
	?>
		//-->
	</script>
...
function get_folder_content($id, $sort = '', $entrsel = '', $searchterm = '', $usecache = 1){
	...
	if($id != $GLOBALS['messaging']->Folder_ID){
		...
		print we_html_element::jsElement('top.content.resize.right.editor.last_entry_selected = -1;');
	}
...
switch($_REQUEST["mcmd"]){
	...
	case 'show_message':
		if(isset($id)){
			...
			<script type="text/javascript">
				<!--
				top.content.resize.right.editor.edbody.msg_mfv.messaging_msg_view.location="<?php print WE_MESSAGING_MODULE_DIR . "messaging_message_view.php?we_transaction=" . $_REQUEST['we_transaction'] . "&id=$id" ?>";
				//-->
			</script>
			<?php
		}
	...
	case 'reset_right_view':
		...
		<script type="text/javascript">
			...
			top.content.resize.right.editor.edbody.msg_mfv.messaging_messages_overview.location="<?php echo we_class::url(WE_MESSAGING_MODULE_DIR . 'messaging_show_folder_content.php'); ?>";
			top.content.resize.right.editor.edbody.msg_mfv.messaging_msg_view.location="<?php echo HTML_DIR ?>white.html";
			//-->
		</script>
		<?php
		break;
	...
	case 'paste_msg':
		...
		<script type="text/javascript">
			...
			top.content.resize.right.editor.entries_selected = new Array();
			top.content.resize.right.editor.edbody.messaging_fv_headers.location="<?php echo we_class::url($this->frameset) . '&pnt=msg_fv_headers&si=' . $messaging->get_sortitem() . '&so=' . $messaging->get_sortorder();
	...
	case 'edit_folder':
		if($_REQUEST['mode'] == 'new' || ($_REQUEST['mode'] == 'edit')){
			...
			<script type="text/javascript">
				<!--
				top.content.resize.right.editor.location = "<?php print WE_MESSAGING_MODULE_DIR; ?>messaging_edit_folder.php?we_transaction=<?php echo $_REQUEST['we_transaction'] ?>&mode=<?php echo $_REQUEST['mode'] ?>&fid=<?php echo (isset($_REQUEST['fid']) ? $_REQUEST['fid'] : -1) ?>";
				//-->
			</script>
			<?php
		}
