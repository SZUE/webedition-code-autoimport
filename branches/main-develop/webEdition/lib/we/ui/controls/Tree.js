...
this.id = treeId;
				 ...
				 *
				 */
				this.rootNode = window["tree_" + this.id].getRoot();
				/**
				 * adds a node
				 *
				 * @param id integer
				...
				text = text.replace(/</g, "&lt;");
								text = text.replace(/>/g, "&gt;");
								if (parentId > 0) {
				var mParentNode = window["tree_" + this.id ].getNodeByProperty('id', parentId);
				}
				else {
				...
				childNode.labelStyle = contentType;
								window["tree_" + this.id + "_activEl"] = childNode.data.id;
								window["tree_" + this.id].draw();
				}
				}
...
var mNode = window["tree_" + this.id].getNodeByProperty('id', id);
				window["tree_" + this.id].popNode(mNode);
				if (newParentId == 0) {
mNode.appendTo(this.rootNode);
}
else {
var mParentNode = window["tree_" + this.id].getNodeByProperty('id',newParentId);
...
var nodes = window["tree_" + this.id]._nodes;
				for (var i in nodes) {
var n = nodes[i];
...
var mNode = window["tree_" + this.id].getNodeByProperty('id',id);
				window["tree_" + this.id].removeNode(mNode);
				window["tree_" + this.id].draw();
}
...
var mNode = window["tree_" + this.id].getNodeByProperty('text',label);
				if (mNode == null) {
return false;
}
return true;
