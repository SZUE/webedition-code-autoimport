...
class we_net_rpc_JsonRpc{
	...
	public static function getReply($namespace = 'we'){
		...
		if(strstr($phpObj['service'], "..") !== false){
			$error->SetError(we_net_rpc_JsonRpcError::kErrorIllegalService, "Illegal use of two consecutive dots in service name");
			return $error->getError();
		}
		...
		if($serviceComponents[0] !== $namespace){
			$error->SetError(we_net_rpc_JsonRpcError::kErrorIllegalService, "First service name component is not '$namespace'");
			return $error->getError();
			/* never gets here */
		}
		...
		if($serviceComponents[1] !== 'service'){
			$error->SetError(we_net_rpc_JsonRpcError::kErrorIllegalService, "Second service name component is not 'service'");
			return $error->getError();
			/* never gets here */
		}
