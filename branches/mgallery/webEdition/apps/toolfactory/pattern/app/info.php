...
$html .= $translate->_('MinWeVersion').': <strong><span style="color:red">'.$appconfig->dependencies->version.'</span></strong> '.$translate->_('AktWeVersion').' <strong>' .WE_VERSION.'</strong>';
} else {
$html .= $translate->_('MinWeVersion').': <strong>'.$appconfig->dependencies->version.'</strong>';
}
}
if(!empty($appconfig->dependencies->sdkversion)){
$html .= '<br/>'.$translate->_('SdkVersion').': <strong>'.$appconfig->dependencies->sdkversion.'</strong>';
}
