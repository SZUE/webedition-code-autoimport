...
<table class="weEditTable" cellpadding="2" cellspacing="2">
	...
	<tr>
		<td class="weEditmodeStyle" colspan="2" align="center">' .
					we_html_button::create_button_table(array(we_html_button::create_button('image:btn_select_image', "javascript:we_cmd('we_selector_image', '" . $imgId . "', '" . FILE_TABLE . "','" . we_base_request::encCmd($cmd1) . "','','" . $wecmdenc3 . "','', '', '" . we_base_ContentTypes::IMAGE . "', " . (permissionhandler::hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")", true), we_html_button::create_button('fa:btn_function_trash,fa-lg fa-trash-o', "javascript:" . $cmd1 . "='';refreshForm();", true)), 5) .
					'</td>
	</tr>
</table>';
			default:
			...
			case g_l('modules_customer', '[objectTab]'):
				...
				if($DB_WE->num_rows()){
					...
					while($DB_WE->next_record()){
						$objectStr.='<tr>
	<td>' . we_html_button::create_button('fa:btn_edit_edit,fa-lg fa-pencil', "javascript: if(top.opener.top.doClickDirect){top.opener.top.doClickDirect(" . $DB_WE->f('ID') . ",'" . $DB_WE->f('ContentType') . "','" . OBJECT_FILES_TABLE . "'); }") . '</td>
	<td>' . $DB_WE->f('ID') . '</td>
	...
	<td class="' . ($DB_WE->f('Published') ? ($DB_WE->f('ModDate') > $DB_WE->f('Published') ? 'changed defaultfont' : 'defaultfont') : 'npdefaultfont') . '">' . date('d.m.Y H:i', $DB_WE->f('ModDate')) . '</td>
			...
			case g_l('modules_customer', '[documentTab]'):
				...
				if($DB_WE->num_rows()){
					...
					while($DB_WE->next_record()){
						$documentStr.='<tr>' .
							'<td>' . we_html_button::create_button('fa:btn_edit_edit,fa-lg fa-pencil', "javascript: if(top.opener.top.doClickDirect){top.opener.top.doClickDirect(" . $DB_WE->f('ID') . ",'" . $DB_WE->f('ContentType') . "','" . FILE_TABLE . "'); }") . '</td>' .
							'<td>' . $DB_WE->f('ID') . '</td>' .
							'<td title="' . $DB_WE->f('Path') . '">' . $DB_WE->f('Text') . '</td>' .
							'<td class="' .
