	...
	case 'refresh':
		...
		while(false !== ($entry = $_dir->read())){
			if($entry != '.' && $entry != '..' && strpos($entry, '.zip') !== false){
				...
				$table->setCol($_i, 4, array('valign' => 'top', 'align' => 'center'), '<div style="display: none;" id="updateIcon_' . $_name . '"><i class="fa fa-2x fa-spinner fa-pulse"></i></div><div style="display: block;" id="updateBut_' . $_name . '">' . we_html_button::create_button(we_html_button::RELOAD, 'javascript: updateDict("' . $_name . '");') . '</div>');
				$table->setCol($_i, 5, array('valign' => 'top', 'align' => 'center'), we_html_button::create_button(we_html_button::TRASH, 'javascript: deleteDict("' . $_name . '");'));
			}
		}
		$_dir->close();
