...
//this happens when we_cmd[3] is set and not we_cmd[2]
						...
						if($we_doc->we_save()){
							...
							$we_JavaScript .= "try{_EditorFrame.getDocumentReference().frames.editHeader.we_setPath('" . $we_doc->Path . "', '" . $we_doc->Text . "', " . intval($we_doc->ID) . ",'" . ($we_doc->Published == 0 ? 'notpublished' : ($we_doc->ModDate > $we_doc->Published ? 'changed' : 'published')) . "');}catch(e){}" .
