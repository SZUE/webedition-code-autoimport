...
foreach($tables as $ctable => $ids){
	...
	$queries[] = '(SELECT ID,Path,' . ($table !== VFILE_TABLE ? 'Icon,ContentType,' : '') . 'Text,ModDate,CreatorID,Owners,RestrictOwners,"' . $ctable . '" AS ctable FROM ' . $db->escape($table) . ' WHERE ID IN(' . implode(',', $ids) . ')' . ($paths ? (' AND (' . $paths . ')') : '') . ')';
}
