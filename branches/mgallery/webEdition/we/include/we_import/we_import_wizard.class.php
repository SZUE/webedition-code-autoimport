...
class we_import_wizard extends we_import_wizardBase{
	...
	private function formCategory2($obj, $categories){
		...
		$addbut = we_html_button::create_button('fa:add,fa-lg fa-plus', "javascript:top.we_cmd('we_selector_category',0,'" . CATEGORY_TABLE . "','','','" . $js . "fillIDs();opener.top.we_cmd(\\'add_" . $obj . "Cat\\',top.allIDs);')", false, 100, 22, "", "", (!permissionhandler::hasPerm("EDIT_KATEGORIE")));
	...
	protected function getStep0(){
		...
		return array(
			...
			function set_button_state() {
				top.wizbusy.switch_button_state('back', 'disabled');
				top.wizbusy.switch_button_state('next', 'enabled');
			}
...
");
		...
		$importFromButton = (permissionhandler::hasPerm("CAN_SELECT_EXTERNAL_FILES")) ? we_html_button::create_button("select", "javascript: self.document.we_form.elements['v[rdofloc]'][0].checked=true;we_cmd('browse_server', '" . $wecmdenc1 . "', '', document.we_form.elements['v[fserver]'].value)") : "";
...
function set_button_state() {
	top.wizbusy.back_enabled = top.wizbusy.switch_button_state('back', 'enabled');
	top.wizbusy.next_enabled = top.wizbusy.switch_button_state('next', " .
...
function set_button_state() {
	top.wizbusy.back_enabled=top.wizbusy.switch_button_state('back','enabled');
	top.wizbusy.next_enabled=top.wizbusy.switch_button_state('next','" . (($xmlWellFormed && $hasChildNode) ? "enabled" : "disabled") . "');
}
function we_submit_form(f, target, url) {
	f.target = target;
	f.action = url;
...
function set_button_state() {
	top.wizbusy.back_enabled = top.wizbusy.switch_button_state('back', 'enabled');
	top.wizbusy.next_enabled = top.wizbusy.switch_button_state('next', " . ((we_base_request::_(we_base_request::INT, 'mode') != 1) ? "'enabled'" : "'disabled'") . ");
}
function we_submit_form(f, target, url) {
	f.target = target;
	f.action = url;
...
function set_button_state() {
	top.frames.wizbusy.back_enabled = top.frames.wizbusy.switch_button_state('back', 'enabled');
	top.frames.wizbusy.next_enabled = top.frames.wizbusy.switch_button_state('next', 'enabled');
}
function we_submit_form(f, target, url) {
	f.target = target;
	f.action = url;
...
"
		...
		/*		 * *************************************************************************************************************** */
		$wecmdenc1 = we_base_request::encCmd("self.wizbody.document.we_form.elements['v[fserver]'].value");
		$importFromButton = (permissionhandler::hasPerm('CAN_SELECT_EXTERNAL_FILES')) ? we_html_button::create_button('select', "javascript:we_cmd('browse_server', '" . $wecmdenc1 . "', '', document.we_form.elements['v[fserver]'].value)") : "";
			...
			if(!file_exists($_SERVER['DOCUMENT_ROOT'] . $v["import_from"])){
				$parts = array(
					array(
						...
						"space" => 0,
						"noline" => 1));
				$functions.='top.wizbusy.switch_button_state("next","disabled");';
