		...
		if($idArr){
			foreach($idArr as $id){
				...
				if($this->isEditable() && $this->cmd_edit){
					$edit = we_html_button::create_button("image:btn_edit_edit", "javascript:if(_EditorFrame!==undefined) _EditorFrame.setEditorIsHot(true);we_cmd('" . $this->cmd_edit . "','" . $id . "');");
				}
				...
				if(($this->isEditable() && $this->cmd_del) || $this->CanDelete){
					...
					if($this->disabledDelItems){
						...
						if(in_array($id, $DisArr)){
							$trash = we_html_button::create_button("image:btn_function_trash", "javascript:if(_EditorFrame!==undefined)_EditorFrame.setEditorIsHot(true);" . ($this->extraDelFn ? : "") . ";we_cmd('" . $this->cmd_del . "','" . $id . "');", true, 100, 22, "", "", true);
							...
							$trash = we_html_button::create_button("image:btn_function_trash", "javascript:if(_EditorFrame!==undefined)_EditorFrame.setEditorIsHot(true);" . ($this->extraDelFn ? : "") . ";we_cmd('" . $this->cmd_del . "','" . $id . "');");
