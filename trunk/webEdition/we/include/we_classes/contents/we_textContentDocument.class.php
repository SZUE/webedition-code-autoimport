...
abstract class we_textContentDocument extends we_textDocument{
	...
	function formDocType2($width = 300, $disable = false){
		...
		return $this->formSelect2($width, 'DocType', DOC_TYPES_TABLE.' dt LEFT JOIN ' . FILE_TABLE . ' dtf ON dt.ParentID=dtf.ID ' . $dtq['join'], 'ID', 'DocType', g_l('weClass', '[doctype]'), 'dt.ID AS ID, dt.DocType AS DocType', $dtq['where'], 1, $this->DocType, false, (($this->DocType !== '') ?
