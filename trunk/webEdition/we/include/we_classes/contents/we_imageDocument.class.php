...
//  If needed - js output for rollover.
	...
	function formProperties(){
		...
		$yuiSuggest->setSelectButton(we_html_button::create_button('select', "javascript:we_cmd('openDocselector',document.we_form.elements['" . $longdesc_id_name . "'].value,'" . FILE_TABLE . "','" . $wecmdenc1 . "','" . $wecmdenc2 . "','" . $wecmdenc3 . "','','','" . we_base_ContentTypes::WEDOCUMENT . "," . we_base_ContentTypes::TEXT . "," . we_base_ContentTypes::HTML . "',1)"));
	...
	function getThumbnail(){
		return ($this->getElement('data') && is_readable($this->getElement('data')) ? ($this->isSvg() ?
					...
					'<img src="' . WEBEDITION_DIR . 'thumbnail.php?id=' . $this->ID . '&size=150&path=' . urlencode(str_replace($_SERVER['DOCUMENT_ROOT'], '', $this->getElement('data'))) . '&extension=' . $this->Extension . '&size2=200" border="0" /></a>' ) :
				$this->getHtml());
	}
