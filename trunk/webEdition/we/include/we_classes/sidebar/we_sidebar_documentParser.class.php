...
abstract class we_sidebar_documentParser{
	...
	static function parseCode($srcCode, $linkPrefix = ""){
		...
		foreach($matches[0] as $index => $command){
			...
			if(method_exists(__CLASS__, $call)){
				self::$call($parameters, $linkPrefix);
			}
	...
	private static function parseFor_openUrl(&$parameters, $linkPrefix){

		return $linkPrefix . $parameters;
	}

	private static function parseFor_openDocument(&$parameters, $linkPrefix){
	...
	private static function parseFor_openTemplate(&$parameters, $linkPrefix){
	...
	private static function parseFor_openObject(&$parameters, $linkPrefix){
	...
	private static function parseFor_openClass(&$parameters, $linkPrefix){
