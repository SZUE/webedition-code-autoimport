...
abstract class we_html_element{
	...
	public static function htmlIFrame($name, $src, $style = '', $iframestyle = '', $onload = '', $scroll = true, $class = ''){
		...
		return self::htmlDiv(array('style' => $style, 'name' => $name . 'Div', 'id' => $name . 'Div', 'class' => $class.($isApple?' iframeScrollIpad':''))
				, we_html_baseElement::getHtmlCode(new we_html_baseElement('iframe', true, array('name' => $name, 'id' => $name, 'src' => $src, 'style' => $iframestyle,'class'=>($isApple ? 'iframeScrollIpad':''), 'onload' =>  $onload))
		));
	}
