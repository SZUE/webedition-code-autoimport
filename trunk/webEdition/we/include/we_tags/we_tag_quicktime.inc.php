...
function we_tag_quicktime($attribs){
	...
	if($showcontrol && $GLOBALS['we_editmode']){
		...
		$quicktime_button = we_html_button::create_button("image:btn_edit_quicktime", "javascript:we_cmd('openDocselector','" . ($id ? : $startid) . "', '" . FILE_TABLE . "','" . $wecmdenc1 . "','','" . $wecmdenc3 . "',''," . $parentid . ", '" . we_base_ContentTypes::QUICKTIME . "', " . (permissionhandler::hasPerm("CAN_SELECT_OTHER_USERS_FILES") ? 0 : 1) . ")", true);
