...
' . $this->getJSSubmitFunction());
	...
	function getProperties(){
		...
		if(isset($_REQUEST['deletethisorder'])){
			$this->db->query('DELETE FROM ' . SHOP_TABLE . ' WHERE IntOrderID = ' . $_REQUEST['bid']);
		...
		// Get Customer data
		$_REQUEST['cid'] = f('SELECT IntCustomerID FROM ' . SHOP_TABLE . ' WHERE IntOrderID=' . intval($_REQUEST['bid']), 'IntCustomerID', $this->db);


		if(($fields = @unserialize(f('SELECT strFelder FROM ' . ANZEIGE_PREFS_TABLE . ' WHERE strDateiname="edit_shop_properties"', 'strFelder', $this->db)))){
		...
		foreach(weShopStatusMails::$StatusFields as $field){
			if(isset($_REQUEST[$field])){
				...
				$this->db->query('UPDATE ' . SHOP_TABLE . ' SET ' . $field . '="' . $this->db->escape($DateOrder) . '" WHERE IntOrderID = ' . intval($_REQUEST["bid"]));
		...
		if(isset($_REQUEST['article'])){
			if(isset($_REQUEST['preis'])){
				$this->db->query('UPDATE ' . SHOP_TABLE . ' SET Price=' . abs($_REQUEST['preis']) . ' WHERE IntID = ' . intval($_REQUEST['article']));
			} else if(isset($_REQUEST['anzahl'])){
				$this->db->query('UPDATE ' . SHOP_TABLE . ' SET IntQuantity=' . abs($_REQUEST['anzahl']) . ' WHERE IntID = ' . intval($_REQUEST['article']));
			} else if(isset($_REQUEST['vat'])){

				$this->db->query('SELECT strSerial FROM ' . SHOP_TABLE . ' WHERE IntID = ' . $this->db->escape($_REQUEST['article']));
				...
				if($this->db->num_rows() == 1){
					...
					$this->db->query('UPDATE ' . SHOP_TABLE . ' SET strSerial="' . $this->db->escape(serialize($tmpDoc)) . '" WHERE IntID = ' . intval($_REQUEST['article']));
					unset($strSerial);
					unset($tmpDoc);
				}
			}
		...
		if(!isset($letzerartikel)){ // order has still articles - get them all
			...
			$this->db->query('SELECT IntID, IntCustomerID, IntArticleID, strSerial, strSerialOrder, IntQuantity, Price, ' . implode(',', $format) . '	FROM ' . SHOP_TABLE . ' WHERE IntOrderID = ' . intval($_REQUEST['bid']));
			...
			switch($_REQUEST['we_cmd'][0]){
				case 'add_article':
					if(intval($_REQUEST['anzahl']) > 0){
						...
						$row = getHash('SELECT IntOrderID, IntCustomerID, DateOrder, DateShipping, Datepayment, IntPayment_Type FROM ' . SHOP_TABLE . ' WHERE IntOrderID=' . intval($_REQUEST['bid']), $this->db);
