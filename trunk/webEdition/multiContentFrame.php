	...
	<div style="position:absolute;top:0px;bottom:0px;right:0px;left:0px;overflow: hidden;background-color: white;">
		...
		<div style="position:absolute;top:22px;bottom:0px;left:0px;right:0px;overflow: hidden;" id="multiEditorEditorFramesetsDiv"><?php
			...
			for($i = 0; $i < $count; $i++){
				...
				echo '<iframe frameBorder="0" style="' . ($i == 0 ? 'width:100%;height:100%;' : (we_base_browserDetect::isChrome() ? 'display:none;' : 'width:0px;height:0px;')) . 'margin:0px;border:0px;" src="' . HTML_DIR . 'blank_editor.html" name="multiEditFrame_' . $i . '" id="multiEditFrame_' . $i . '"  noresize ></iframe>';
}
			?>
		</div>
	</div>
