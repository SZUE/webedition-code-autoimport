...
var weFileUpload = (function () {
	...
	function weFileUpload_abstract() {
		...
		function AbstractController() {
			...
			this.editImageButtonOnClick = function(btn, index, general){
				btn.disabled = true;
				if(!general && !btn.form.elements['fuOpts_useCustomOpts'].checked){
					//return;
				}
				...
				switch (target.name){
					...
					case 'fuOpts_useCustomOpts':
						...
						if(target.checked){
							...
							_.view.formCustomEditOptsSync(null, pos, false);
							_.utils.setImageEditOptionsFile(fileobj);
			...
			this.processimageLoadCanvas = function(fileobj, nexttask) {
				...
				reader.onload = function() {
					if(!fileobj.img.editOptions.doEdit){ // we will not edit and rewrite image so we hold original dataUrl in fileobj to be used by fullpreview
						...
						if(fileobj.img.previewCanvas){ // preview done during eralier editing: directly jump top postprocess
							_.utils.processimagePostProcess(fileobj, _.controller.IMG_NEXT);
							return;
						}
					}
	...
	function weFileUpload_importer() {
		...
		function View() {
			...
			this.formCustomEditOptsSync = function (fileobj, pos, allGeneral) {
				var positions = [], form,
					generalForm = document.getElementById('filechooser');

				if(allGeneral){
					var forms = document.getElementsByName('we_form');
					for(var i = 0; i < forms.length; i++){
						positions.push(forms[i].getAttribute('data-index'));
					}
				} else if(pos && pos >= 0) {
					positions.push(pos);
				} else if(fileobj){
					positions.push(fileobj.index);
				}

				for(i = 0; i < positions.length; i++){
					form = document.getElementById('form_editOpts_' + positions[i]);
			...
			this.previewSyncRotation = function(pos, rotation){
				...
				if(pos >= 0){
					if(_.sender.preparedFiles[pos]){
						fileobjects.push(_.sender.preparedFiles[pos]);
					} else {
						return;
					}
				} else {
					var forms = document.getElementsByName('we_form');
					for(var i = 0; i < forms.length; i++){
						if(!forms[i].elements['fuOpts_useCustomOpts'].checked){
							fileobjects.push(_.sender.preparedFiles[forms[i].getAttribute('data-index')]);
						}
					}
				}

				for(var i = 0; i < fileobjects.length; i++){
					_.utils.processimageRotatePreview(fileobjects[i], rotation);
					_.view.replacePreviewCanvas(fileobjects[i]);
				}
			};

			this.setEditStatus = function(state, pos, general){
