...
$l_prefs=array(
	...
	'editorTooltipsIDs'=>'show paths of ID`s',
	'editorTooltips'=>'Show help for tags and attributes',
	...
	'error_notices_warning'=>'We recommend to activate the option -Log errors- on all systems; the option -Show errors- should be activated only during development. If -Log errors- is activated and webEdition is not able to write to the database, it tries to send you a mail: Therefore we recommend you to enter a mail adress even if -Send a mail- is not activated.',
	...
	'force404redirect'=>'force redicret to 404 error page',
	'force404redirect_description'=>'If set to <em>false</em>, the 404 error page is included and the url do not change. If set to <em>true </em> a redirect to the 404 error page will happen.',
	...
	'objectseourls_lowercase'=>'in lower case',
	'objectseourls_lowercase_description'=>'If <em>true</em> SEO-URLs are written in lower case. If <em>false</em> SEO-URLs are written in camel case.',
	'off'=>'off',
	'on'=>'on',
...
The difference between symmetric encryption and hash is, that it is possible to decrypt data which is stored by a symetric encryption. The shown key is also used to encrypt some fields in customer management (if selected).',
		...
		'userPassRegex'=>array(
			'desc'=>'Textual representation of the regex',
	...
	'session'=>array(
		'crypt'=>array(
			'alert'=>'Please note: changing this option will result in logout of all current users and customers.',
			'title'=>'Encrypt each user session',
		),
		'information'=>'Management of php sessions is also available by webEdition. Each session is written to the database and can be encrypted transparent or via extra cookie (highly recommended). Please note, each change of settings causes ALL users AND customers to loose their session.',
