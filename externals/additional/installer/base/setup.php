...
if(!empty($_SERVER["SCRIPT_URI"])){
	...
	if($_SERVER["HTTP_HOST"] != $referer["host"]){
		...
		header("location: " . $protocol . $_SERVER["HTTP_HOST"] . (isset($_SERVER['SCRIPT_URL']) ? $_SERVER['SCRIPT_URL'] : $_SERVER['SCRIPT_NAME']) . '?' . $_SERVER["QUERY_STRING"]);
	}
}
...
$LU_Variables = array(
	...
	'clientUpdateUrl' => str_replace("\\", "/", _getServerProtocol() . $_SERVER['HTTP_HOST']) . (isset($_SERVER['SCRIPT_URL']) ? $_SERVER['SCRIPT_URL'] : $_SERVER['SCRIPT_NAME']), // REQUEST_URI is not always available so we use PHP_SELF
