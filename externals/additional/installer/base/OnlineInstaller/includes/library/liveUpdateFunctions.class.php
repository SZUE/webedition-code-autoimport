...
class liveUpdateFunctions{
	...
	function insertUpdateLogEntry($action, $version, $errorCode){
		...
		$query = "INSERT INTO " . UPDATE_LOG_TABLE . "
	...
	function executeDropQueries($queries){
		...
		foreach($queries as $query){
			...
			if(strpos($query, '###TBLPREFIX###') !== false){ //New DB code
	...
	function executeUpdateQuery($query){
		...
		if(is_array($queries) && !empty($queries)){
			foreach($queries as $query){
				$query = strtr(trim($query), array(
					'###INSTALLONLY###' => '',
					'ENGINE=MyISAM' => 'ENGINE=' . $defaultEngine
				));
				...
				if((strpos($query, '###UPDATEONLY###') === false) && (strpos($query, '###UPDATEDROPCOL') === false)){
					...
					if(!empty($_SESSION['we_db_collation'])){
						//if(eregi("^CREATE TABLE ", $query)) {
						if(preg_match("/^CREATE TABLE /", $query)){
							$query = preg_replace("/;$/", " CHARACTER SET " . $_SESSION['we_db_charset'] . " COLLATE " . $_SESSION['we_db_collation'], $query, 1);
						}
					}
