...
class we_builder_builder{
	...
	const BUILDER_BASE_PATH = '/kunden/343047_10825/';
	const BUILDER_DESTINATION_DIR = 'sites/webedition.org/update/files/we/';
	const BUILDER_EXTERNALS_DIR = 'externals/';
	const BUILDER_SVN_DIR = 'build/svn/';
	
	public function __construct($configurations = null, $db_we = null, $isDebug = false){
		if(!$configurations || !$configurations->getActiveConfiguration()){
			return false;
		}
		...
		$this->sourceDir = self::BUILDER_BASE_PATH . self::BUILDER_SVN_DIR . $this->configurations->get('targetBranchDir') . '/';
	}

	private function getSvnLatestRevision($branch = 'trunk'){
	...
	private function getFilelistFromSvn($branch = 'trunk', $startRev = 9000, $endRev = 'HEAD', $origBranch = ''){
	...
	private function getFilelistWithExternals($branch = 'trunk', $startRev = 9000, $endRevBranch = 'HEAD', $endRevExternals = 'HEAD'){
	...
	/*
	private function getFilelistFromSvn_plusD($branch = 'trunk', $startRev = 9000, $endRev = 'HEAD'){
		$xml = simplexml_load_string(shell_exec('svn log --xml -v -r ' . $startRev . ':' . $endRev . ' http://svn.code.sf.net/p/webedition/code/' . $branch));
	...
	*/
	...
	private function getfiledescription($configurations, $builder, $svnpath, $isQuery, $isLang, $areLangsExternal = false){
		$entry = array(
			'svnpath' => $svnpath
		);
		if($isQuery){
	...
	private function correctExternalPaths($configurations, $svnpaths){
		$tmpFiles = array();
	...
	private function correctExternalPath($configurations, $svnpath){
		$externals = array(
			'zend' => array(
	...
	private function run(){
		...
		// #### get filelists by type and configuration => make extra function
		...
		if($this->isDebug){
			$this->response = "\nConfiguration:\n" .
				"targetBranch: " . $configurations->get('targetBranch') . "\n" .
				"targetBranchDir: " . $configurations->get('targetBranchDir') . "\n" .
				"type (basic): " . $configurations->get('targetNormalizedType') . "\n" .
				"type: " . $configurations->get('targetType') . "\n" .
				"targetVersion: " . $configurations->get('targetVersion') . "\n" .
				"targetCompareVersion: " . $configurations->get('targetCompareVersion') . " " . ($configurations->get('targetTakeSnapshot') ? "[not needed when snapshot]" : "") . "\n" .
				"targetRevisionFrom: " . $configurations->get('targetRevisionFrom') . " " . ($configurations->get('targetTakeSnapshot') ? "[not needed when snapshot]" : "") . "\n" .
				"targetRevisionTo: " . $targetRevision . "\n" .
				"targetTakeSnapshot: " . ($configurations->get('targetTakeSnapshot') ? 'ja' : 'nein') . "\n" .
				"isHotfix: " . ($configurations->getIsHotfix() ? 'ja' : 'nein') . "\n\n";
		}

		### find and process special files: externals, language files and sql dumps; throw out unused files of /additional ###
		// find sql dumps
		
		## make four functions: processFiles(), processDumps(), processLangFiles() and writeFilesToServer()
		$sqlfiles = array();
		...
		// #### START BUILD HERE ### (writeFilesToServer())
		$changessql = [];
		...
		// #### WRITE VERSION TO DB ### functions writeFilesToDatabase and writeVersionToDatabase()
		if($changesfiles){
