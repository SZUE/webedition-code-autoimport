...
public class SuggestorController {
	...
	TagSuggestor getTagSuggestor() {
		return tSuggestor;
	}

	AttribSuggestor getAttribSuggestor() {
		return aSuggestor;
	}
	...
	public void update() {
		...
		if (shouldShowTagSuggestor()) {
			...
			ArrayList<String> tags = applet.getTags();
			suggestedTags.clear();
			...
			for (int i = 0; i < tags.size(); i++) {
				...
				if (tagName.length() == 0 || (parts[0].length() >= tagName.length() && parts[0].substring(0, tagName.length()).equals(tagName))) {
					...
					if ((isEndTag && parts[1].equals("true")) || !isEndTag) {
						suggestedTags.addElement(parts[0]);
					}
				}
			}
		...
		if (shouldShowAttribSuggestor()) {
			...
			ArrayList<String> attribs = applet.getAttribsForTag(tagName);
			attribsForTag.clear();
	...
	private String findLastOpenStartTag() {
		Document doc = pane.getDocument();
		try {
			String content = doc.getText(0, doc.getLength()).substring(0, pane.getCaretPosition());
	...
	public void handleArrowKeyDown(boolean doSelectFirst) {
		...
		if (tSuggestor.isVisible()) {
			...
			if (doSelectFirst) {
				tSuggestor.list.setSelectedIndex(0);
			}
			...
			if (doSelectFirst) {
				aSuggestor.list.setSelectedIndex(0);
			}
