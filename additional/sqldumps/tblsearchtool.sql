...
/* query separator */
INSERT INTO _delKeys SELECT s.ID FROM ###TBLPREFIX###tblsearchtool s, ###TBLPREFIX###tblsearchtool t WHERE s.Path=t.Path AND s.ID>t.ID;
/* query separator */
DELETE FROM ###TBLPREFIX###tblsearchtool WHERE ID IN (SELECT ID FROM _delKeys);
/* query separator */
