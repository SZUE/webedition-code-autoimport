...
function new_message(mode) {
	...
	new (WE().util.jsWindow)(window, WE().consts.dirs.WE_MESSAGING_MODULE_DIR + "messaging_newmessage.php?we_transaction=" + transaction + "&mode=" + mode, "messaging_new_message", 670, 530, true, false, true, false);
}
...
function new_todo() {
	new (WE().util.jsWindow)(window, WE().consts.dirs.WE_MESSAGING_MODULE_DIR + "todo_edit_todo.php?we_transaction=" + transaction + "&mode=new", "messaging_new_todo", 690, 520, true, false, true, false);
}

function forward_todo() {
	if (top.content.editor.edbody.entries_selected && top.content.editor.edbody.entries_selected.length > 0) {
		new (WE().util.jsWindow)(window, WE().consts.dirs.WE_MESSAGING_MODULE_DIR + "todo_edit_todo.php?we_transaction=" + transaction + "&mode=forward", "messaging_new_todo", 705, 600, true, true, true, false);
	}
}

function reject_todo() {
	if (top.content.editor.edbody.entries_selected && top.content.editor.edbody.entries_selected.length > 0) {
		new (WE().util.jsWindow)(window, WE().consts.dirs.WE_MESSAGING_MODULE_DIR + "todo_edit_todo.php?we_transaction=" + transaction + "&mode=reject", "messaging_new_todo", 690, 600, true, false, true, false);
	}
}

function update_todo() {
	if (top.content.editor.edbody.entries_selected && top.content.editor.edbody.entries_selected.length > 0) {
		new (WE().util.jsWindow)(window, WE().consts.dirs.WE_MESSAGING_MODULE_DIR + "todo_update_todo.php?we_transaction=" + transaction + "&mode=reject", "messaging_new_todo", 705, 600, true, true, true, false);
	}
}
