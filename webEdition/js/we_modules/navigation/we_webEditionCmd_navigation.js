...
we_cmd_modules.navigation = function (args, url) {
	switch (args[0]) {
		...
		case "module_navigation_rules":
			...
			new (WE().util.jsWindow)(window, WE().consts.dirs.WEBEDITION_DIR + "we_showMod.php?mod=navigation&pnt=ruleFrameset", "tool_navigation_rules", 680, 580, true, true, true, true);
			return true;
		case "module_navigation_edit_navi":
			new (WE().util.jsWindow)(window, WE().consts.dirs.WEBEDITION_DIR + "we_cmd.php?we_cmd[0]=weNaviEditor&we_cmd[1]=" + args[1], "we_navieditor", 600, 350, true, false, true, true);
			return true;
	}
	return false;
};