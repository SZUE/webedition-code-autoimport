...
abstract class we_editor_preferences{
	 ...
	 * @return         string
	 */
	private static function create_dialog($name, array $content, $expand = -1, $show_text = '', $hide_text = ''){
		$output = ($expand != -1 ? we_html_element::jsScript(JS_DIR . 'multiIconBox.js') : '');
	 ...
	 * @return         unknown
	 */
	private static function get_value($settingname){
	...
	private static function getYesNoSelect($setting){
		...
		$select->selectOption(self::get_value($setting) ? 1 : 0);

		return $select->getHtml();
	}

	private static function getTrueFalseSelect($setting){
		$select = new we_html_select(['name' => 'newconf[' . $setting . ']', 'class' => 'weSelect']);
	 ...
	 * @return         bool
	 */
	private static function remember_value($settingvalue, $settingname, $comment = ''){
		$DB_WE = $GLOBALS['DB_WE'];
		...
		switch($settingname){
			...
			case 'weWidth':
				if($_SESSION['prefs']['sizeOpt'] == 1){
					...
					if($generate_java_script){
						...
						self::$jsCmd->addCmd('setNewWESize', $settingvalue, $height);
					}
				}
				return;
			...
			case 'editorFont':
				...
				if(!self::$editor_reloaded){
					self::$editor_reloaded = true;

					// editor font has changed - mark all editors to reload!
					self::$jsCmd->addCmd('reloadUsedEditors');
				}
				return;
			...
			case 'Language': //Handle both
				...
				if($settingvalue != $GLOBALS['WE_LANGUAGE'] || $_SESSION['prefs']['BackendCharset'] != $GLOBALS['WE_BACKENDCHARSET']){
					...
					self::$jsCmd->addCmd('reloadUsedEditors', 1);
				}
...
\'' . implode("',\n'", we_base_request::_(we_base_request::STRING, 'newconf', [], 'active_integrated_modules')) . '\'
	 ...
	 * @return         void
	 */
	private static function save_all_values(){
				...
				switch($name){
					...
					case 'global'://no settings in session
						if(we_base_preferences::userIsAllowed($key)){
							self::remember_value(we_base_request::_($default[1], 'newconf', null, $key), $key, $default[0]);
						}
						break;
					case 'user':
						self::remember_value(we_base_request::_($default[0], 'newconf', null, $key), $key);
						break;
					default:
						if(we_base_preferences::userIsAllowed($key)){
							self::remember_value(we_base_request::_($default[0], 'newconf', null, $key), $key);
	...
	private static function build_dialog($selected_setting = 'ui'){
		...
		switch($selected_setting){
			case 'save':
				...
				return self::create_dialog('', [['headline' => '', 'html' => g_l('prefs', '[save]'),]
				]);

			case 'saved'://SAVED SUCCESSFULLY DIALOG
				return self::create_dialog('', [['headline' => '', 'html' => g_l('prefs', '[saved]'),]
				]);
			...
			case 'ui':
				...
				if(!empty($language)){ // Build language select box
					...
					$languages->selectOption(self::get_value('Language'));
					// Lang notice
...
<td class="middlefont">' . g_l('prefs', '[language_notice]') . '
				...
				$BackendCharset->selectOption(self::get_value('BackendCharset'));
				...
				if(we_base_preferences::userIsAllowed('DEFAULT_CHARSET')){
					...
					$GLOBALS['weDefaultCharset'] = self::get_value('DEFAULT_CHARSET');
				...
				$cockpit_amount_columns->selectOption(self::get_value('cockpit_amount_columns'));
				...
				if(we_base_preferences::userIsAllowed('WE_SEEM')){
					...
					$seem_disabler = we_html_forms::checkbox(1, self::get_value('WE_SEEM') == 0 ? 1 : 0, 'newconf[WE_SEEM]', g_l('prefs', '[seem_deactivate]'));
				...
				if(we_base_preferences::userIsAllowed('seem_start_type')){
					...
					switch(self::get_value('seem_start_type')){
						default:
							$seem_start_type = 0;
							break;
						case 'cockpit':
						...
						case 'object':
							...
							if(self::get_value('seem_start_file') != 0){
								$object_id = self::get_value('seem_start_file');
						...
						case 'weapp':
							...
							if(self::get_value('seem_start_weapp') != ''){
								$seem_start_weapp = self::get_value('seem_start_weapp');
							}
						...
						case 'document':
							...
							if(self::get_value('seem_start_file') != 0){
								$document_id = self::get_value('seem_start_file');
					...
					if(we_base_permission::hasPerm('CAN_SEE_DOCUMENTS')){
						...
						$weSuggest->setInput('seem_start_document_name', $document_path, [], self::get_value('seem_start_file'));
					...
					if(defined('OBJECT_FILES_TABLE') && we_base_permission::hasPerm('CAN_SEE_OBJECTFILES')){
						...
						$weSuggest->setInput('seem_start_object_name', $object_path, [], self::get_value('seem_start_file'));
						$weSuggest->setMaxResults(20);
						$weSuggest->setRequired(true);
					...
					if($showStartType){
						...
						$settings[] = ['headline' => g_l('prefs', '[seem_startdocument]'), 'html' => $seem_html->getHtml(),
							'space' => we_html_multiIconBox::SPACE_BIG];
						self::$jsCmd->addCmd('show_seem_chooser', $seem_start_type);
					}
				...
				if(we_base_preferences::userIsAllowed('SIDEBAR_DISABLED')){
					// Settings
					$sidebar_disable = self::get_value('SIDEBAR_DISABLED');
					$sidebar_show = ($sidebar_disable) ? 'none' : 'block';

					$sidebar_id = self::get_value('SIDEBAR_DEFAULT_DOCUMENT');
					...
					// Show on Startup
					$sidebar_show_on_startup = we_html_forms::checkbox(1, self::get_value('SIDEBAR_SHOW_ON_STARTUP'), 'newconf[SIDEBAR_SHOW_ON_STARTUP]', g_l('prefs', '[sidebar_show_on_startup]'), false, 'defaultfont', '');

					// Sidebar width
					$sidebar_width = we_html_tools::htmlTextInput('newconf[SIDEBAR_DEFAULT_WIDTH]', 8, self::get_value('SIDEBAR_DEFAULT_WIDTH'), 255, "onchange=\"if ( isNaN( this.value ) ||  parseInt(this.value) < 100 ) { this.value=100; };\"", 'number', 90);
				...
				$tree_count = self::get_value('default_tree_count');
				...
				// Build maximize window
				$window_max_code = we_html_forms::radiobutton(0, self::get_value('sizeOpt') == 0, 'newconf[sizeOpt]', g_l('prefs', '[maximize]'), true, 'defaultfont', "document.getElementsByName('newconf[weWidth]')[0].disabled = true;document.getElementsByName('newconf[weHeight]')[0].disabled = true;");

				// Build specify window dimension
				$window_specify_code = we_html_forms::radiobutton(1, !(self::get_value('sizeOpt') == 0), 'newconf[sizeOpt]', g_l('prefs', '[specify]'), true, 'defaultfont', "document.getElementsByName('newconf[weWidth]')[0].disabled = false;document.getElementsByName('newconf[weHeight]')[0].disabled = false;");

				// Create specify window dimension input
				$window_specify_table = new we_html_table(['class' => 'default', 'style' => 'margin-top:10px;margin-left:50px;'], 1, 3);
				 ...
				 */
				$window_specify_table->setCol(0, 0, null, we_html_tools::htmlTextInput('newconf[weWidth]', 6, (self::get_value('sizeOpt') ? self::get_value('weWidth') : ''), 4, (self::get_value('sizeOpt') == 0 ? 'disabled="disabled"' : ""), "number", 60));
				$window_specify_table->setCol(0, 1, ['style' => 'padding:0px .5em;vertical-align: middle;'], 'x');
				$window_specify_table->setCol(0, 2, null, we_html_tools::htmlTextInput('newconf[weHeight]', 6, (self::get_value('sizeOpt') ? self::get_value('weHeight') : ''), 4, (self::get_value('sizeOpt') == 0 ? 'disabled="disabled"' : ""), "number", 60));
				...
				return self::create_dialog('', $settings, -1);
			...
			case 'defaultAttribs':
				...
				$COMMANDS_DEFAULT = we_html_tools::htmlTextInput('newconf[COMMANDS_DEFAULT]', 22, self::get_value('COMMANDS_DEFAULT'), '', 'id="commands_default_id"', 'text', 225, 0, '');
				...
				$CSSAPPLYTO_DEFAULT->selectOption(self::get_value('CSSAPPLYTO_DEFAULT') ?: 'around');
				...
				$settings = [
					['headline' => g_l('prefs', '[default_php_setting]'), 'html' => self::getTrueFalseSelect('WE_PHP_DEFAULT'), 'space' => we_html_multiIconBox::SPACE_BIG],
					['headline' => g_l('prefs', '[xhtml_default]'), 'html' => self::getTrueFalseSelect('XHTML_DEFAULT'), 'space' => we_html_multiIconBox::SPACE_BIG],
					['headline' => g_l('prefs', '[inlineedit_default]'), 'html' => self::getTrueFalseSelect('INLINEEDIT_DEFAULT'), 'space' => we_html_multiIconBox::SPACE_BIG],
					...
					['headline' => g_l('prefs', '[commands_default]'), 'html' => '<div>' . $commands_default_tmp . '</div><div style="margin-top:4px">' . $COMMANDS_DEFAULT . '</div>',
						'space' => we_html_multiIconBox::SPACE_BIG],
					['headline' => g_l('prefs', '[removefirstparagraph_default]'), 'html' => self::getTrueFalseSelect('REMOVEFIRSTPARAGRAPH_DEFAULT'), 'space' => we_html_multiIconBox::SPACE_BIG],
					['headline' => g_l('prefs', '[showinputs_default]'), 'html' => self::getTrueFalseSelect('SHOWINPUTS_DEFAULT'), 'space' => we_html_multiIconBox::SPACE_BIG],
					['headline' => g_l('prefs', '[hidenameattribinweimg_default]'), 'html' => self::getYesNoSelect('HIDENAMEATTRIBINWEIMG_DEFAULT'), 'space' => we_html_multiIconBox::SPACE_BIG],
					['headline' => g_l('prefs', '[hidenameattribinweform_default]'), 'html' => self::getYesNoSelect('HIDENAMEATTRIBINWEFORM_DEFAULT'), 'space' => we_html_multiIconBox::SPACE_BIG],
					['headline' => g_l('prefs', '[replaceacronym]'), 'html' => self::getYesNoSelect('REPLACEACRONYM'), 'space' => we_html_multiIconBox::SPACE_BIG],
			...
			case 'countries':
				...
				$countries_default = we_html_tools::htmlTextInput('newconf[WE_COUNTRIES_DEFAULT]', 22, self::get_value('WE_COUNTRIES_DEFAULT'), '', '', 'text', 225);
				...
				$countries_top = explode(',', self::get_value('WE_COUNTRIES_TOP'));
				$countries_shown = explode(',', self::get_value('WE_COUNTRIES_SHOWN'));
				...
				return self::create_dialog('', $settings);
			...
			case 'language':
				...
				$settings = [
					...
					['headline' => g_l('prefs', '[langlink_headline]'), 'html' => we_html_tools::htmlAlertAttentionBox(g_l('prefs', '[langlink_information]'), we_html_tools::TYPE_INFO, 450, false),
						'noline' => 1],
					['headline' => g_l('prefs', '[langlink_support]'), 'html' => self::getTrueFalseSelect('LANGLINK_SUPPORT'), 'space' => we_html_multiIconBox::SPACE_BIG, 'noline' => 1],
				];
			...
			case 'editor':
				...
				$template_editor_mode->selectOption(self::get_value('editorMode'));
				...
				$template_editor_font_specify = (self::get_value('editorFontname') != '' && self::get_value('editorFontname') != 'none');
				$template_editor_font_size_specify = (self::get_value('editorFontsize') != '' && self::get_value('editorFontsize') != -1);
				...
				/* 			$colorsDisabled = true;
				  ...
				  $template_editor_fontcolor_selector = getColorInput('newconf[editorFontcolor]', self::get_value('editorFontcolor'), $colorsDisabled);
				  $template_editor_we_tag_fontcolor_selector = getColorInput('newconf[editorWeTagFontcolor]', self::get_value('editorWeTagFontcolor'), $colorsDisabled);
				  $template_editor_we_attribute_fontcolor_selector = getColorInput('newconf[editorWeAttributeFontcolor]', self::get_value('editorWeAttributeFontcolor'), $colorsDisabled);
				  $template_editor_html_tag_fontcolor_selector = getColorInput('newconf[editorHTMLTagFontcolor]', self::get_value('editorHTMLTagFontcolor'), $colorsDisabled);
				  $template_editor_html_attribute_fontcolor_selector = getColorInput('newconf[editorHTMLAttributeFontcolor]', self::get_value('editorHTMLAttributeFontcolor'), $colorsDisabled);
				  $template_editor_pi_tag_fontcolor_selector = getColorInput('newconf[editorPiTagFontcolor]', self::get_value('editorPiTagFontcolor'), $colorsDisabled);
				  $template_editor_comment_fontcolor_selector = getColorInput('newconf[editorCommentFontcolor]', self::get_value('editorCommentFontcolor'), $colorsDisabled);
				 */
				foreach($template_fonts as $font){
					$template_editor_font_select_box->addOption($font, $font);
				}
				$template_editor_font_select_box->selectOption($template_editor_font_specify ? self::get_value('editorFontname') : 'Courier New');
				...
				$template_editor_font_sizes_select_box->selectOption(self::get_value('editorFontsize'));
	...
	<tr><td' . $attr . '>' . g_l('prefs', '[editor_fontsize]') . '</td><td>' . $template_editor_font_sizes_select_box->getHtml() . '</td></tr>
				...
				/*
				  $template_editor_font_color_checkbox = we_html_forms::checkboxWithHidden(self::get_value('specify_jeditor_colors'), "newconf[specify_jeditor_colors]", g_l('prefs', '[editor_font_colors]'), false, "defaultfont", "setEditorColorsDisabled(!this.checked);");
				...
				$template_editor_theme->selectOption(self::get_value('editorTheme'));

				//Build activation of line numbers
				$template_editor_linenumbers_code = we_html_forms::checkbox(1, self::get_value('editorLinenumbers'), 'newconf[editorLinenumbers]', g_l('prefs', '[editor_enable]'), true, 'defaultfont', '');
				$template_editor_highlightLine_code = we_html_forms::checkbox(1, self::get_value('editorHighlightCurrentLine'), 'newconf[editorHighlightCurrentLine]', g_l('prefs', '[editor_enable]'), true, 'defaultfont', '');

				//Build activation of code completion
				$template_editor_codecompletion_code = we_html_element::htmlHiddens([
						'newconf[editorCodecompletion][WE]' => self::get_value('editorCodecompletion-WE'),
						'newconf[editorCodecompletion][htmlTag]' => self::get_value('editorCodecompletion-htmlTag'),
						'newconf[editorCodecompletion][htmlDefAttr]' => self::get_value('editorCodecompletion-htmlDefAttr'),
						'newconf[editorCodecompletion][htmlAttr]' => self::get_value('editorCodecompletion-htmlAttr'),
						'newconf[editorCodecompletion][htmlJSAttr]' => self::get_value('editorCodecompletion-htmlJSAttr'),
						'newconf[editorCodecompletion][html5Tag]' => self::get_value('editorCodecompletion-html5Tag'),
						'newconf[editorCodecompletion][html5Attr]' => self::get_value('editorCodecompletion-html5Attr')
					]) .
					we_html_forms::checkbox(1, self::get_value('editorCodecompletion-WE'), 'editorCodecompletion0', 'WE-Tags', true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorCodecompletion][WE]\');') .
					'<br/>' .
					we_html_forms::checkbox(1, self::get_value('editorCodecompletion-htmlTag'), 'editorCodecompletion1', 'HTML-Tags', true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorCodecompletion][htmlTag]\');') .
					'<br/>' .
					we_html_forms::checkbox(1, self::get_value('editorCodecompletion-htmlDefAttr'), 'editorCodecompletion2', 'HTML-Default-Attribs', true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorCodecompletion][htmlDefAttr]\');') .
					'<br/>' .
					we_html_forms::checkbox(1, self::get_value('editorCodecompletion-htmlAttr'), 'editorCodecompletion3', 'HTML-Attribs', true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorCodecompletion][htmlAttr]\');') .
					'<br/>' .
					we_html_forms::checkbox(1, self::get_value('editorCodecompletion-htmlJSAttr'), 'editorCodecompletion4', 'HTML-JS-Attribs', true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorCodecompletion][htmlJSAttr]\');') .
					'<br/>' .
					we_html_forms::checkbox(1, self::get_value('editorCodecompletion-html5Tag'), 'editorCodecompletion5', 'HTML5-Tags', true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorCodecompletion][html5Tag]\');') .
					'<br/>' .
					we_html_forms::checkbox(1, self::get_value('editorCodecompletion-html5Attr'), 'editorCodecompletion6', 'HTML5-Attribs', true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorCodecompletion][html5Attr]\');');


				$template_editor_tabstop_code = '<table class="default">
				<tr><td colspan="2">' .
					we_html_forms::checkbox(1, self::get_value('editorShowTab'), 'editorShowTab', g_l('prefs', '[editor_tabstop]'), true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorShowTab]\');') .
					we_html_element::htmlHidden('newconf[editorShowTab]', self::get_value('editorShowTab')) .
					'</td></tr>
				<tr><td class="defaultfont" style="width:200px;">' . g_l('prefs', '[editor_tabSize]') . '</td><td>' . we_html_tools::htmlTextInput("newconf[editorTabSize]", 2, self::get_value("editorTabSize"), 2, "", "number", 135) . '</td></tr>
				<tr><td colspan="2">' .
					we_html_forms::checkbox(1, self::get_value('editorAutoIndent'), 'editorAutoIndent', g_l('prefs', '[editor_autoindent]'), true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorAutoIndent]\');') .
					we_html_element::htmlHidden('newconf[editorAutoIndent]', self::get_value('editorAutoIndent')) .
					'</td></tr>
				<tr><td colspan="2">' .
					we_html_forms::checkbox(1, self::get_value('editorIndentSpaces'), 'editorIndentSpaces', g_l('prefs', '[editor][indentSpaces]'), true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorIndentSpaces]\');') .
					we_html_element::htmlHidden('newconf[editorIndentSpaces]', self::get_value('editorIndentSpaces')) .
					'</td></tr>
				<tr><td colspan="2">' .
					we_html_forms::checkbox(1, self::get_value('editorShowSpaces'), 'editorShowSpaces', g_l('prefs', '[editor][showSpaces]'), true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorShowSpaces]\');') .
					we_html_element::htmlHidden('newconf[editorShowSpaces]', self::get_value('editorShowSpaces')) .
					'</td></tr>
			</table>';

				$template_editor_Wrap_code = we_html_forms::checkbox(1, self::get_value('editorWrap'), 'editorWrap', g_l('prefs', '[editor_enable]'), true, 'defaultfont', 'set_xhtml_field(this.checked,\'newconf[editorWrap]\');') .
					we_html_element::htmlHidden('newconf[editorWrap]', self::get_value('editorWrap'));


				$template_editor_tooltips_code = we_html_forms::checkbox(1, self::get_value('editorTooltips'), 'newconf[editorTooltips]', g_l('prefs', '[editorTooltips]'), true, 'defaultfont', '') .
					'<br/>' .
					we_html_forms::checkbox(1, self::get_value('editorTooltipsIDs'), 'newconf[editorTooltipsIDs]', g_l('prefs', '[editorTooltipsIDs]'), true, 'defaultfont', '');

				$template_editor_tooltip_font_specify = (self::get_value('editorTooltipFontname') != '' && self::get_value('editorTooltipFontname') != 'none');
				$template_editor_tooltip_font_size_specify = (self::get_value('editorTooltipFontsize') != '' && self::get_value('editorTooltipFontsize') != -1);
				...
				$template_editor_tooltip_font_select_box->selectOption($template_editor_tooltip_font_specify ? self::get_value('editorTooltipFontname') : 'Tahoma');
				...
				$template_editor_tooltip_font_sizes_select_box->selectOption($template_editor_tooltip_font_specify ? self::get_value("editor_tooltip_font_size") : 11);
				...
				$template_editor_autoClose = we_html_forms::checkbox(1, self::get_value('editorDocuintegration'), 'newconf[editorDocuintegration]', g_l('prefs', '[editor_enable]'), true, 'defaultfont', '') .
...
//				array('class' => 'editor editor_java', 'headline' => g_l('prefs', '[editor_highlight_colors]'), 'html' => $template_editor_font_color_checkbox . $template_editor_font_color_table, 'space' => we_html_multiIconBox::SPACE_MED2),
				...
				return self::create_dialog("settings_editor_predefined", $settings, count($settings), g_l('prefs', '[show_predefined]'), g_l('prefs', '[hide_predefined]'));
			...
			case "recipients":
				...
				if(we_base_preferences::userIsAllowed("FORMMAIL_CONFIRM")){
					...
					$formmail_confirm->selectOption(self::get_value("FORMMAIL_CONFIRM") ? 1 : 0);
					...
					$formmail_log->selectOption(self::get_value("FORMMAIL_LOG") ? 1 : 0);
					...
					$isDisabled = (self::get_value("FORMMAIL_LOG") == 0);
					...
					$formmail_emptylog->selectOption(self::get_value("FORMMAIL_EMPTYLOG"));
					...
					$formmail_ViaWeDoc->selectOption((self::get_value("FORMMAIL_VIAWEDOC") ? 1 : 0));
					...
					$formmail_block->selectOption(self::get_value("FORMMAIL_BLOCK") ? 1 : 0);
					...
					$formmail_span->selectOption(self::get_value("FORMMAIL_SPAN"));
					...
					$formmail_blocktime->selectOption(self::get_value("FORMMAIL_BLOCKTIME"));
			...
			case 'proxy':
				...
				$proxy = self::get_value("proxy_proxy");

				$use_proxy = we_html_forms::checkbox(1, $proxy, "newconf[useproxy]", g_l('prefs', '[useproxy]'), false, "defaultfont", "set_state();");
				$proxyaddr = we_html_tools::htmlTextInput("newconf[proxyhost]", 22, self::get_value("WE_PROXYHOST"), "", "", "text", 225, 0, "", !$proxy);
				$proxyport = we_html_tools::htmlTextInput("newconf[proxyport]", 22, self::get_value("WE_PROXYPORT"), "", "", "text", 225, 0, "", !$proxy);
				$proxyuser = we_html_tools::htmlTextInput("newconf[proxyuser]", 22, self::get_value("WE_PROXYUSER"), "", 'autocomplete="off"', "text", 225, 0, "", !$proxy);
			...
			case "advanced":
				...
				$WYSIWYG_TYPE = new we_html_select(['name' => "newconf[WYSIWYG_TYPE]", "class" => "weSelect"]);
				$options = ['tinyMCE' => 'tinyMCE', 'tinyMCE_beta' => 'tinyMCE4 (Beta)'];
				foreach($options as $key => $val){
					$WYSIWYG_TYPE->addOption($key, $val);
				}
				$WYSIWYG_TYPE->selectOption(self::get_value("WYSIWYG_TYPE"));
				$settings[] = ["headline" => g_l('prefs', '[wysiwyg_type]'), "html" => $WYSIWYG_TYPE->getHtml(), 'space' => we_html_multiIconBox::SPACE_BIG];


				$we_max_upload_size = '<table class="default"><tr><td>' .
					we_html_tools::htmlTextInput("newconf[FILE_UPLOAD_MAX_UPLOAD_SIZE]", 22, self::get_value("FILE_UPLOAD_MAX_UPLOAD_SIZE"), "", ' onkeypress="return WE().util.IsDigit(event);"', "number", 60) . ' MB</td><td style="padding-left:20px;" class="small">' .
				...
				$img->addOptions($op);
				$img->selectOption(self::get_value("FILE_UPLOAD_IMG_MAX_SIZE"));

				$we_max_img_upload_size = $img->getHtml() . ' px ';

				$settings[] = ["headline" => g_l('prefs', '[upload][we_max_img_size]'), "html" => $we_max_img_upload_size, 'space' => we_html_multiIconBox::SPACE_BIG];
				...
				/* $WYSIWYG_TYPE_FRONTEND = new we_html_select(array('name' => "newconf[WYSIWYG_TYPE_FRONTEND]", "class" => "weSelect"));
				  ...
				  $WYSIWYG_TYPE_FRONTEND->selectOption(self::get_value("WYSIWYG_TYPE_FRONTEND"));
				 ...
				 */
				$we_doctype_workspace_behavior = self::get_value("WE_DOCTYPE_WORKSPACE_BEHAVIOR");
				...
				$NAVIGATION_ENTRIES_FROM_DOCUMENT->selectOption(self::get_value("NAVIGATION_ENTRIES_FROM_DOCUMENT") ? 1 : 0);
				...
				$NAVIGATION_RULES_CONTINUE_AFTER_FIRST_MATCH->selectOption(self::get_value("NAVIGATION_RULES_CONTINUE_AFTER_FIRST_MATCH") ? 1 : 0);
			...
			case "system":
				...
				$static_we_extensions->selectOption(self::get_value('DEFAULT_STATIC_EXT'));
				$dynamic_we_extensions->selectOption(self::get_value('DEFAULT_DYNAMIC_EXT'));
				...
				$static_html_extensions->selectOption(self::get_value('DEFAULT_HTML_EXT'));
				...
				$we_new_folder_mod = '<table class="default"><tr><td>' .
					we_html_tools::htmlTextInput("newconf[WE_NEW_FOLDER_MOD]", 22, self::get_value("WE_NEW_FOLDER_MOD"), 3, ' onkeypress="return WE().util.IsDigit(event);"', "text", 60) . '</td><td style="padding-left:20px;" class="small">' .
				...
				$auth = self::get_value("HTTP_USERNAME");
				$auth_user = self::get_value("HTTP_USERNAME");
				$auth_pass = self::get_value("HTTP_PASSWORD");
				...
				if(we_base_imageEdit::gd_version() > 0){ //  gd lib ist installiert
					...
					$inp = we_html_tools::htmlTextInput("newconf[WE_THUMBNAIL_DIRECTORY]", 12, self::get_value("WE_THUMBNAIL_DIRECTORY"), "", "", "text", 125);
					...
					$inp = we_html_tools::htmlTextInput("newconf[WE_THUMBNAIL_DIRECTORY]", 12, self::get_value("WE_THUMBNAIL_DIRECTORY"), "", "", "text", 125, 0, '', true);
				...
				$EXECUTE_HOOKS->selectOption(self::get_value("EXECUTE_HOOKS") ? 1 : 0);

				$hooksHtml = $EXECUTE_HOOKS->getHtml() . we_html_tools::htmlAlertAttentionBox(g_l('prefs', '[hooks_information]'), we_html_tools::TYPE_HELP);

				$useSession = new we_html_select(['name' => "newconf[SYSTEM_WE_SESSION]", "class" => "weSelect", 'onchange' => 'WE().util.showMessage(\'' . g_l('prefs', '[session][crypt][alert]') . '\', WE().consts.message.WE_MESSAGE_ERROR);']);
				$useSession->addOption(0, g_l('prefs', '[no]'));
				$useSession->addOption(1, g_l('prefs', '[yes]'));
				$useSession->selectOption(self::get_value("SYSTEM_WE_SESSION") ? 1 : 0);

				$sessionTime = '<table class="default"><tr><td>' .
					we_html_tools::htmlTextInput("newconf[SYSTEM_WE_SESSION_TIME]", 22, abs(self::get_value("SYSTEM_WE_SESSION_TIME")), "", ' onkeypress="return WE().util.IsDigit(event);"', "text", 60) . '</td><td style="padding-left:20px;" class="small">s</td></tr></table>';

				$cryptSession = new we_html_select(['name' => 'newconf[SYSTEM_WE_SESSION_CRYPT]', 'class' => "weSelect", 'onchange' => 'WE().util.showMessage(\'' . g_l('prefs', '[session][crypt][alert]') . '\', WE().consts.message.WE_MESSAGE_ERROR);']);
				$cryptSession->addOption(0, g_l('prefs', '[no]'));
				$cryptSession->addOption(1, 'Transparent');
				$cryptSession->addOption(2, 'Cookie');
				$cryptSession->selectOption(self::get_value('SYSTEM_WE_SESSION_CRYPT'));
				...
				$we_windowtypeselect->selectOption(self::get_value('WE_LOGIN_WEWINDOW'));
				$loginWE = we_html_forms::checkbox(1, self::get_value('WE_LOGIN_HIDEWESTATUS') == 1 ? 1 : 0, 'newconf[WE_LOGIN_HIDEWESTATUS]', g_l('prefs', '[login][deactivateWEstatus]')) .
					we_html_element::htmlBr() .
					g_l('prefs', '[login][windowtypes]') .
					we_html_element::htmlBr() .
					$we_windowtypeselect->getHtml();
				...
				$_Schedtrigger_setting->selectOption(self::get_value("SCHEDULER_TRIGGER"));

				$settings = [
					['headline' => g_l('prefs', '[we_extensions]'), 'html' => $we_extensions_html, 'space' => we_html_multiIconBox::SPACE_BIG, 'noline' => 1],
					...
					['headline' => g_l('prefs', '[login][login]'), 'html' => $loginWE, 'space' => we_html_multiIconBox::SPACE_BIG],
					...
					["headline" => g_l('prefs', '[we_scheduler_trigger][head]') . we_html_tools::htmlAlertAttentionBox(g_l('prefs', '[we_scheduler_trigger][description]'), we_html_tools::TYPE_HELP),
						"html" => we_base_moduleInfo::isActive(we_base_moduleInfo::SCHEDULER) ? $_Schedtrigger_setting->getHtml() : '-', 'space' => we_html_multiIconBox::SPACE_BIG],
			...
			case "seolinks":
				...
				$navigation_directoryindex_names = we_html_tools::htmlTextInput("newconf[NAVIGATION_DIRECTORYINDEX_NAMES]", 22, self::get_value("NAVIGATION_DIRECTORYINDEX_NAMES"), "", "", "text", 225);
				...
				$settings = [
					...
					["headline" => g_l('prefs', '[navigation_directoryindex_hide]'), "html" => self::getTrueFalseSelect('NAVIGATION_DIRECTORYINDEX_HIDE'), 'space' => we_html_multiIconBox::SPACE_BIG,
						'noline' => 1],
					["headline" => g_l('prefs', '[wysiwyglinks_directoryindex_hide]'), "html" => self::getTrueFalseSelect('WYSIWYGLINKS_DIRECTORYINDEX_HIDE'), 'space' => we_html_multiIconBox::SPACE_BIG,
						'noline' => 1],
					...
					["headline" => g_l('prefs', '[taglinks_directoryindex_hide]'), "html" => self::getTrueFalseSelect('TAGLINKS_DIRECTORYINDEX_HIDE'), 'space' => we_html_multiIconBox::SPACE_BIG],
					...
					["headline" => g_l('prefs', '[objectseourls_lowercase]'), "html" => self::getTrueFalseSelect('OBJECTSEOURLS_LOWERCASE'), 'space' => we_html_multiIconBox::SPACE_BIG,
						'noline' => 1],
					["headline" => g_l('prefs', '[navigation_objectseourls]'), "html" => self::getTrueFalseSelect('NAVIGATION_OBJECTSEOURLS'), 'space' => we_html_multiIconBox::SPACE_BIG,
						'noline' => 1],
					["headline" => g_l('prefs', '[wysiwyglinks_objectseourls]'), "html" => self::getTrueFalseSelect('WYSIWYGLINKS_OBJECTSEOURLS'), 'space' => we_html_multiIconBox::SPACE_BIG,
						'noline' => 1],
					...
					["headline" => g_l('prefs', '[taglinks_objectseourls]'), "html" => self::getTrueFalseSelect('TAGLINKS_OBJECTSEOURLS'), 'space' => we_html_multiIconBox::SPACE_BIG,
						'noline' => 1],
					["headline" => g_l('prefs', '[urlencode_objectseourls]'), "html" => self::getTrueFalseSelect('URLENCODE_OBJECTSEOURLS'), 'space' => we_html_multiIconBox::SPACE_BIG],
					["headline" => g_l('prefs', '[general_seoinside]'), "html" => we_html_tools::htmlAlertAttentionBox(g_l('prefs', '[general_seoinside_description]'), we_html_tools::TYPE_HELP),
						'space' => we_html_multiIconBox::SPACE_BIG, 'noline' => 1],
					["headline" => g_l('prefs', '[seoinside_hideineditmode]'), "html" => self::getTrueFalseSelect('SEOINSIDE_HIDEINEDITMODE'), 'space' => we_html_multiIconBox::SPACE_BIG,
						'noline' => 1],
					["headline" => g_l('prefs', '[seoinside_hideinwebedition]'), "html" => self::getTrueFalseSelect('SEOINSIDE_HIDEINWEBEDITION'), 'space' => we_html_multiIconBox::SPACE_BIG],
					['headline' => g_l('prefs', '[error_no_object_found]'), 'html' => $weSuggest->getHTML(), 'space' => we_html_multiIconBox::SPACE_BIG, 'noline' => 1],
					['headline' => g_l('prefs', '[suppress404code]'), 'html' => self::getTrueFalseSelect('SUPPRESS404CODE'), 'space' => we_html_multiIconBox::SPACE_BIG],
					['headline' => g_l('prefs', '[force404redirect]'), 'html' => self::getTrueFalseSelect('FORCE404REDIRECT') . we_html_tools::htmlAlertAttentionBox(g_l('prefs', '[force404redirect_description]'), we_html_tools::TYPE_HELP),
						'space' => we_html_multiIconBox::SPACE_BIG],
				];
				return self::create_dialog('', $settings, -1, '', '', null);
			...
			case 'error_handling':
				...
				// Create checkboxes
				$we_error_handler = we_html_forms::checkbox(1, self::get_value('WE_ERROR_HANDLER'), 'newconf[WE_ERROR_HANDLER]', g_l('prefs', '[error_use_handler]'), false, 'defaultfont', 'set_state_error_handler();');

				// Error types
				// Create checkboxes
				$error_handling_table = new we_html_table(['class' => 'default'], 8, 1);

				$error_handling_table->setCol(0, 0, null, we_html_forms::checkbox(1, self::get_value('WE_ERROR_ERRORS'), 'newconf[WE_ERROR_ERRORS]', g_l('prefs', '[error_errors]'), false, 'defaultfont', '', !self::get_value('WE_ERROR_HANDLER')));
				$error_handling_table->setCol(2, 0, null, we_html_forms::checkbox(1, self::get_value('WE_ERROR_WARNINGS'), 'newconf[WE_ERROR_WARNINGS]', g_l('prefs', '[error_warnings]'), false, 'defaultfont', '', !self::get_value('WE_ERROR_HANDLER')));
				$error_handling_table->setCol(4, 0, null, we_html_forms::checkbox(1, self::get_value('WE_ERROR_NOTICES'), 'newconf[WE_ERROR_NOTICES]', g_l('prefs', '[error_notices]'), false, 'defaultfont', '', !self::get_value('WE_ERROR_HANDLER')));
				$error_handling_table->setCol(6, 0, null, we_html_forms::checkbox(1, self::get_value('WE_ERROR_DEPRECATED'), 'newconf[WE_ERROR_DEPRECATED]', g_l('prefs', '[error_deprecated]'), false, 'defaultfont', '', !self::get_value('WE_ERROR_HANDLER')));
				...
				$error_display_table->setCol(1, 0, null, we_html_forms::checkbox(1, self::get_value('WE_ERROR_SHOW'), 'newconf[WE_ERROR_SHOW]', g_l('prefs', '[error_display]'), false, 'defaultfont', '', !self::get_value('WE_ERROR_HANDLER')));
				$error_display_table->setCol(3, 0, null, we_html_forms::checkbox(1, self::get_value('WE_ERROR_LOG'), 'newconf[WE_ERROR_LOG]', g_l('prefs', '[error_log]'), false, 'defaultfont', '', !self::get_value('WE_ERROR_HANDLER')));
				$error_display_table->setCol(5, 0, null, we_html_forms::checkbox(1, self::get_value('WE_ERROR_MAIL'), 'newconf[WE_ERROR_MAIL]', g_l('prefs', '[error_mail]'), false, 'defaultfont', '', !self::get_value('WE_ERROR_HANDLER')));
				...
				$error_mail_specify_table->setCol(0, 2, ['style' => 'text-align:left'], we_html_tools::htmlTextInput('newconf[WE_ERROR_MAIL_ADDRESS]', 6, self::get_value('WE_ERROR_MAIL_ADDRESS'), 100, 'placeholder="mail@example"', 'email', 195));
				...
				return self::create_dialog('settings_error_expert', $settings, $foldAt, g_l('prefs', '[show_expert]'), g_l('prefs', '[hide_expert]'));
			...
			case 'security':
				...
				$encryption->selectOption(self::get_value('SECURITY_ENCRYPTION_TYPE_PASSWORD'));


				$encryptinfo = we_html_tools::htmlAlertAttentionBox(g_l('prefs', '[security][encryption][hint]'), we_html_tools::TYPE_ALERT, false);
				$cryptkey = self::get_value('SECURITY_ENCRYPTION_KEY');
				...
				$storeSessionPassword->selectOption(self::get_value('SECURITY_SESSION_PASSWORD'));
				...
				$settings = [
					...
					['headline' => g_l('prefs', '[security][userPassRegex][desc]'), 'html' => we_html_tools::htmlTextInput('newconf[SECURITY_USER_PASS_DESC]', 0, self::get_value('SECURITY_USER_PASS_DESC'), 100, '', 'text', '20em'),
						'space' => we_html_multiIconBox::SPACE_MED],
				];
			...
			case 'email':
				...
				$emailSelect = we_html_tools::htmlSelect('newconf[WE_MAILER]', ['php' => g_l('prefs', '[mailer_php]'), 'smtp' => g_l('prefs', '[mailer_smtp]')], 1, self::get_value('WE_MAILER'), false, [
				...
				$smtp_table = new we_html_table(['class' => 'default', 'id' => 'smtp_table', 'width' => 300, 'style' => 'display: ' . ((self::get_value('WE_MAILER') === 'php') ? 'none' : 'block') . ';'], 4, 2);
				$smtp_table->setCol(0, 0, ['class' => 'defaultfont', 'style' => 'padding-right:10px;'], g_l('prefs', '[smtp_server]'));
				$smtp_table->setCol(0, 1, ['style' => 'text-align:right'], we_html_tools::htmlTextInput('newconf[SMTP_SERVER]', 24, self::get_value('SMTP_SERVER'), 180, '', 'text', 180));
				$smtp_table->setCol(1, 0, ['class' => 'defaultfont'], g_l('prefs', '[smtp_port]'));
				$smtp_table->setCol(1, 1, ['style' => 'text-align:right'], we_html_tools::htmlTextInput('newconf[SMTP_PORT]', 24, self::get_value('SMTP_PORT'), 180, '', 'text', 180));
				...
				$encryptSelect = we_html_tools::htmlSelect('newconf[SMTP_ENCRYPTION]', [0 => g_l('prefs', '[smtp_encryption_none]'), 'ssl' => g_l('prefs', '[smtp_encryption_ssl]'),
						'tls' => g_l('prefs', '[smtp_encryption_tls]')], 1, self::get_value('SMTP_ENCRYPTION'), false, [], 'value', 180, 'defaultfont');
				...
				$smtp_table->setCol(3, 0, ['class' => 'defaultfont', 'colspan' => 3], we_html_forms::checkbox(1, self::get_value('SMTP_AUTH'), 'newconf[SMTP_AUTH]', g_l('prefs', '[smtp_auth]'), false, 'defaultfont', "var el2 = document.getElementById('auth_table').style; if(this.checked) el2.display='block'; else el2.display='none';"));

				$auth_table = new we_html_table(['class' => 'default', 'id' => 'auth_table', 'width' => 200, 'style' => 'display: ' . ((self::get_value('SMTP_AUTH') == 1) ? 'block' : 'none') . ';'], 2, 2);
				$auth_table->setCol(0, 0, ['class' => 'defaultfont'], g_l('prefs', '[smtp_username]'));
				$auth_table->setCol(0, 1, ['style' => 'text-align:right'], we_html_tools::htmlTextInput('newconf[SMTP_USERNAME]', 14, self::get_value('SMTP_USERNAME'), 105, 'placeholder="' . g_l('prefs', '[smtp_username]') . '"', 'text', 180));
				$auth_table->setCol(1, 0, ['class' => 'defaultfont'], g_l('prefs', '[smtp_password]'));
				$auth_table->setCol(1, 1, ['style' => 'text-align:right'], we_html_tools::htmlTextInput('newconf[SMTP_PASSWORD]', 14, self::get_value('SMTP_PASSWORD') ? we_customer_customer::NOPWD_CHANGE : '', 105, 'placeholder="' . g_l('prefs', '[smtp_password]') . '"', 'password', 180));

				$settings = [
					['headline' => '', 'html' => we_html_tools::htmlAlertAttentionBox(g_l('prefs', '[mailer_information]'), we_html_tools::TYPE_INFO, 450, false),],
					array('headline' => g_l('prefs', '[mailer_type]'), 'html' => $emailSelect, 'space' => we_html_multiIconBox::SPACE_MED, 'noline' => 1),
					['headline' => '', 'html' => $smtp_table->getHtml(), 'space' => we_html_multiIconBox::SPACE_MED, 'noline' => 1],
					['headline' => '', 'html' => $auth_table->getHtml(), 'space' => we_html_multiIconBox::SPACE_MED, 'noline' => 1],
				];


				return self::create_dialog('settings_email', $settings);
			...
			case 'versions':
				...
				foreach($versionsPrefs as $v){
					foreach($v as $val){
						$_SESSION['weS']['versions']['logPrefs'][$val] = self::get_value($val);
					}
				}
				...
				foreach($versionsPrefs['ctypes'] as $k => $v){
					$checkboxes .= we_html_forms::checkbox(1, self::get_value($v), 'newconf[' . $v . ']', g_l('contentTypes', '[' . $k . ']'), false, 'defaultfont', 'checkAllRevert(this);') . '<br/>';
				}
				...
				$versions_time_days->selectOption(self::get_value('VERSIONS_TIME_DAYS'));
				...
				$versions_time_weeks->selectOption(self::get_value('VERSIONS_TIME_WEEKS'));
				...
				$versions_time_years->selectOption(self::get_value('VERSIONS_TIME_YEARS'));
				$versions_anzahl = we_html_tools::htmlTextInput('newconf[VERSIONS_ANZAHL]', 24, self::get_value('VERSIONS_ANZAHL'), 5, '', 'text', 50, 0, '');

				$versions_create_publishing = we_html_forms::radiobutton(1, (self::get_value('VERSIONS_CREATE') == 1), 'newconf[VERSIONS_CREATE]', g_l('prefs', '[versions_create_publishing]'), true, 'defaultfont', '', false, '');
				$versions_create_always = we_html_forms::radiobutton(0, (self::get_value('VERSIONS_CREATE') == 0), 'newconf[VERSIONS_CREATE]', g_l('prefs', '[versions_create_always]'), true, 'defaultfont', '', false, '');
				...
				$versions_time_days_tmpl->selectOption(self::get_value('VERSIONS_TIME_DAYS_TMPL'));
				...
				$versions_time_weeks_tmpl->selectOption(self::get_value('VERSIONS_TIME_WEEKS_TMPL'));
				...
				$versions_time_years_tmpl->selectOption(self::get_value('VERSIONS_TIME_YEARS_TMPL'));
				$versions_anzahl_tmpl = we_html_tools::htmlTextInput('newconf[VERSIONS_ANZAHL_TMPL]', 24, self::get_value('VERSIONS_ANZAHL_TMPL'), 5, '', 'text', 50, 0, '');
				$versions_create_tmpl_publishing = we_html_forms::radiobutton(1, (self::get_value('VERSIONS_CREATE_TMPL') == 1), 'newconf[VERSIONS_CREATE_TMPL]', g_l('prefs', '[versions_create_tmpl_publishing]'), true, 'defaultfont', '', false, '');
				$versions_create_tmpl_always = we_html_forms::radiobutton(0, (self::get_value('VERSIONS_CREATE_TMPL') == 0), 'newconf[VERSIONS_CREATE_TMPL]', g_l('prefs', '[versions_create_tmpl_always]'), true, 'defaultfont', '', false, '');
	...
	private static function render_dialog(){
		...
		foreach($tabs as $tab){
			$ret .= we_html_element::htmlDiv(['id' => 'setting_' . $tab, 'style' => ($tabname === 'setting_' . $tab ? '' : 'display: none;')], self::build_dialog($tab));
		}
		return $ret;
	}
	...
	private static function saveSettings(){
		...
		return [true, $acError, $acErrorMsg];
	}

	public static function show(){
		self::$jsCmd = new we_base_jsCmd();
// Check if we need to save settings
		list($doSave, $acError, $acErrorMsg) = (we_base_request::_(we_base_request::BOOL, 'save_settings') ? self::saveSettings() : [false, false, '']);
		...
		echo we_html_tools::getHtmlTop('', '', '', we_html_element::jsScript(JS_DIR . 'preferences.js') . self::$jsCmd->getCmds(), $body);
	}
