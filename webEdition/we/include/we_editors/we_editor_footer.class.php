...
//	spaceholder
	...
	static function normalMode($we_doc, $we_transaction, $haspermNew, $showPubl){
		...
		if(!$_ctrlElem || !$_ctrlElem['hide']){
			$_normalTable->addCol(2);
			$_normalTable->setColContent(0, $_pos++, we_html_button::create_button("save", "javascript:" . ($we_doc->isBinary()? we_fileupload_binaryDocument::getJsUploadBeforeLeave("save", "_EditorFrame.setEditorPublishWhenSave(false);we_save_document();") : "_EditorFrame.setEditorPublishWhenSave(false);we_save_document();")));
			$_normalTable->setColContent(0, $_pos++, we_html_tools::getPixel(10, 20));
			//}
		}
