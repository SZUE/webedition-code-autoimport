...
class we_workflow_view extends we_modules_view{
	...
	function getCategoryHTML(){
		...
		$addbut = we_html_button::create_button(we_html_button::ADD, "javascript:top.content.setHot();we_cmd('we_selector_category',0,'" . CATEGORY_TABLE . "','','','opener.we_cmd(\\'add_cat\\',top.fileSelect.data.allIDs.join(\\',\\'));')", false, 100, 22, "", "", (!permissionhandler::hasPerm("EDIT_KATEGORIE")));
	...
	function getObjCategoryHTML(){
		...
		$addbut = we_html_button::create_button(we_html_button::ADD, "javascript:top.content.setHot();we_cmd('we_selector_category',0,'" . CATEGORY_TABLE . "','','','opener.we_cmd(\\'add_objcat\\',top.fileSelect.data.allIDs.join(\\',\\'));')", false, 100, 22, "", "", (!permissionhandler::hasPerm("EDIT_KATEGORIE")));
	...
	function getObjectFileFoldersHTML(){
		...
		//avascript:top.content.setHot();we_cmd('we_selector_directory','','".OBJECT_FILES_TABLE."','','','opener.we_cmd(\\'add_object_file_folder\\',top.fileSelect.data.allIDs.join(','));','','','',true)
