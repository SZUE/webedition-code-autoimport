...
WHERE docstep.startDate!=0 AND (docstep.startDate+ ROUND(step.Worktime*3600))<UNIX_TIMESTAMP() AND docstep.finishDate=0 AND docstep.Status=' . we_workflow_documentStep::STATUS_UNKNOWN . ' AND doc.Status=' . we_workflow_workflow::STATE_INACTIVE);
	...
	static function getLogButton($docID, $table){
		$type = self::getTypeForTable($table);
		return we_html_button::create_button('logbook', "javascript:new (WE().util.jsWindow)(window, WE().consts.dirs.WEBEDITION_DIR + 'we_showMod.php?mod=workflow&pnt=log&art=" . $docID . "&type=" . $type . "','workflow_history',WE().consts.size.dialog.medium,WE().consts.size.dialog.small,true,false,true);");
	}
