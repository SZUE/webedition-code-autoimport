...
//	var $Path;
	...
	public static function cryptData($data, $key = SECURITY_ENCRYPTION_KEY, $keepBin = false){//Note we need 4 Bytes prefix + 16 Byte IV + 1$ = 21 Bytes. The rest is avail for data, which is hex'ed, so "half" of length is available
		if($data && function_exists('mcrypt_module_open') && ($res = mcrypt_module_open(MCRYPT_TWOFISH, '', MCRYPT_MODE_OFB, ''))){
			...
			return '$-3' . ($keepBin ? 'a' : '') . '$' . bin2hex($iv) . '$' . ($keepBin ? $data : bin2hex($data));
		}
		return $data;
	}
	...
	public static function getJSLangConsts(){
		return 'WE().consts.g_l.customer=JSON.parse("' . we_base_util::setLangString([
				'view' => [
					...
					'nothing_to_save' => (g_l('modules_customer', '[nothing_to_save]')),
					'reset_failed_login_successfully' => (g_l('modules_customer', '[login_reset_ok]')),
					'save_changed_customer' => g_l('modules_customer', '[save_changed_customer]'),
				],
				'admin' => [
					'branch_no_edit' => (g_l('modules_customer', '[branch_no_edit]')),
					'del_fild_question' => g_l('modules_customer', '[del_fild_question]'),
					'no_branch' => (g_l('modules_customer', '[no_branch]')),
					'no_field' => (g_l('modules_customer', '[no_field]')),
					'other' => g_l('modules_customer', '[other]'),
					'we_fieldname_notValid' => (g_l('modules_customer', '[we_fieldname_notValid]')),
				],
				'sortAdmin' => [
					'default_soting_no_del' => (g_l('modules_customer', '[default_soting_no_del]')),
					'sortname_empty' => (g_l('modules_customer', '[sortname_empty]')),
				]
				]) . '");';
	}
