...
class we_customer_EIWizard{
	...
	private function getDoExportCode(){
		...
		return we_html_tools::getHtmlTop(g_l('modules_customer', '[export_title]'), '', '', we_html_element::jsScript(WE_JS_MODULES_DIR . 'customer/EIWizard.js') . we_base_jsCmd::singleCmd('setProgress', $percent), we_html_element::htmlBody([
					'onload' => 'document.we_form.submit()'], we_html_element::htmlForm([
						'name' => 'we_form', 'method' => 'post', 'target' => 'load', 'action' => $this->frameset], $hiddens)
				)
		);
	}
