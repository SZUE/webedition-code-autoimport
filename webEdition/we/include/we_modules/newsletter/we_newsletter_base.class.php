...
class we_newsletter_base{
	...
	public static function getEmailsFromList($emails, $emails_only = 0, $group = 0, array $blocks = array()){
		$arr = explode("\n", $emails);
		if(!$arr){
			return array();
		}
		...
		foreach($arr as $row){
			if($row != ""){
				...
				if(count($arr2)){
					$ret[] = ($emails_only ?
							$arr2[0] :
							array($arr2[0], (isset($arr2[1]) && trim($arr2[1]) != '') ? trim($arr2[1]) : $default_html, isset($arr2[2]) ? trim($arr2[2]) : "", isset($arr2[3]) ? $arr2[3] : "", isset($arr2[4]) ? $arr2[4] : "", isset($arr2[5]) ? $arr2[5] : "", $group, $blocks));
				}
			}
		}
	...
	public static function getEmailsFromExtern($files, $emails_only = 0, $group = 0, array $blocks = array()){
					...
					foreach($dataArr as $value){
						...
						switch($emails_only){
							case 1:
								...
								break;
							case 2:
								$ret[] = array(trim($dat[0]), (isset($dat[1]) && trim($dat[1]) != '') ? trim($dat[1]) : $default_html, isset($dat[2]) ? trim($dat[2]) : "", isset($dat[3]) ? $dat[3] : "", isset($dat[4]) ? $dat[4] : "", isset($dat[5]) ? $dat[5] : "");
								break;
							default:
								$ret[] = array(trim($dat[0]), (isset($dat[1]) && trim($dat[1]) != '') ? trim($dat[1]) : $default_html, isset($dat[2]) ? trim($dat[2]) : "", isset($dat[3]) ? $dat[3] : "", isset($dat[4]) ? $dat[4] : "", isset($dat[5]) ? $dat[5] : "", $group, $blocks);
						}
					}
				}
			}
	 ...
	 * @return unknown
	 */
	static function getEmailsFromExtern2($files, $emails_only, $group, array $blocks, $status, &$emailkey){
		$arr = makeArrayFromCSV($files);
		if(!$arr){
			return array();
		}
					...
					foreach($dataArr as $value){
						...
						switch($emails_only){
							case 1:
								...
								break;
							case 2:
								$ret[] = array(trim($dat[0]), (isset($dat[1]) && trim($dat[1]) != '') ? trim($dat[1]) : $default_html, isset($dat[2]) ? trim($dat[2]) : "", isset($dat[3]) ? trim($dat[3]) : "", isset($dat[4]) ? trim($dat[4]) : "", isset($dat[5]) ? trim($dat[5]) : "");
								break;
							default:
								$ret[] = array(trim($dat[0]), (isset($dat[1]) && trim($dat[1]) != '') ? trim($dat[1]) : $default_html, isset($dat[2]) ? trim($dat[2]) : "", isset($dat[3]) ? trim($dat[3]) : "", isset($dat[4]) ? trim($dat[4]) : "", isset($dat[5]) ? trim($dat[5]) : "", $group, $blocks);
						}
					}
				}
			}
