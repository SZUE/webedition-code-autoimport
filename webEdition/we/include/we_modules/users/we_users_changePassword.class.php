...
abstract class we_users_changePassword{
	...
	private static function getLoad(we_base_jsCmd $jsCmd){
		$DB_WE = $GLOBALS['DB_WE'];
	...
	public static function showDialog(){
		...
		echo we_html_tools::getHtmlTop(g_l('global', '[changePass]'), '', '', we_html_element::jsScript(JS_DIR . 'comparePwd.js', '', ['id' => 'loadVarComparePwd', 'data-passwd' => setDynamicVar([
			...
			])]) . $jsCmd->getCmds(), we_html_element::htmlBody(['class' => 'weDialogBody', 'onload' => 'self.focus();'], we_html_element::htmlExIFrame('passwdcontent', self::getContent(), 'position:absolute;top:0px;bottom:1px;left:0px;right:0px;')
		));
	}
