...
if (top.content.viewclass != "' . $mode . '") {
	top.content.set_frames("' . $mode . '");
			...
			case 'rej_todo':
				return we_html_element::jsElement('new (WE().util.jsWindow)(window, WE().consts.dirs.WE_MESSAGING_MODULE_DIR+"todo_edit_todo.php?we_transaction=' . $this->transaction . '&mode=reject", "messaging_new_todo",-1,-1,690,600,true,false,true,false);');
...
top.content.editor.edbody.messaging_msg_view.location="about:blank";
				...
				if($idx > -1){
					$jscmd->addCmd('updateTreeEntry', [
						'id' => $aid,
						'parentid' => -1,
						'text' => $this->messaging->available_folders[$idx]['Name'] . ' - (' . $this->messaging->get_message_count($aid) . ')',
						'published' => 1
					]);
				}
			...
			case 'delete_msg':
				...
				$jscmd->addCmd('updateTreeEntry', [
					'id' => $aid,
					'parentid' => -1,
					'text' => $this->messaging->available_folders[we_messaging_messaging::array_ksearch('ID', $aid, $this->messaging->available_folders)]['Name'] . ' - (' . $this->messaging->get_message_count($aid) . ')',
					'published' => 1
				]);
				return we_html_element::jsElement('
top.content.editor.edbody.entries_selected = [];
...
top.content.editor.edbody.messaging_msg_view.location="about:blank";');
			...
			case 'edit_folder':
				return (($mode = we_base_request::_(we_base_request::STRING, 'mode')) === 'edit' ?
					we_html_element::jsElement('top.content.editor.location = WE().consts.dirs.WE_MESSAGING_MODULE_DIR+"messaging_edit_folder.php?we_transaction=' . $this->transaction . '&mode=' . $mode . '&fid=' . we_base_request::_(we_base_request::INT, 'fid', -1) . '";
					') :
					'');
			...
			case 'save_folder_settings':
				if(($id = we_base_request::_(we_base_request::INT, 'id')) !== false){
					if(($mode = we_base_request::_(we_base_request::STRING, "mode")) === 'new'){
						...
						$out .= we_html_element::jsElement('top.content.folder_added(' . $parent . ');');
						...
						$jscmd->addMsg(g_l('modules_messaging', '[folder_created]'), we_message_reporting::WE_MESSAGE_NOTICE);
...
if (top.content.editor.edbody.messaging_messages_overview) {
	...
	private function refresh_work(we_base_jsCmd $jscmd, $blank = false){
		if(($eSel = we_base_request::_(we_base_request::INTLISTA, "entrsel", []))){
			$this->messaging->set_ids_selected($eSel);
		}

		$this->messaging->get_fc_data($this->messaging->Folder_ID, '', '', 0);
		$this->update_treeview($jscmd);
		return $this->print_fc_html($blank);
	}
	...
	private function update_treeview(we_base_jsCmd $jscmd){
		foreach($this->messaging->available_folders as $f){
			$jscmd->addCmd('updateTreeEntry', [
				'id' => $f['ID'],
				'parentid' => $f['ParentID'],
				'text' => $f['Name'] . ' - (' . $this->messaging->get_message_count($f['ID']) . ')',
				'published' => 1
			]);
		}
		$jscmd->addCmd('drawTree');
