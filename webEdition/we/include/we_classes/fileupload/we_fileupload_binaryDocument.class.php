 ...
 * $Rev: 8160 $
 * $Author: lukasimhof $
 * $Date: 2014-09-01 13:11:19 +0200 (Mo, 01 Sep 2014) $
 *
...
class we_fileupload_binaryDocument extends we_fileupload_base{
	...
	public function getHTML($fs = '', $ft = '', $md = '', $thumbnailSmall = '', $thumbnailBig = ''){
		...
		$btnCancel = we_html_button::create_button("cancel", 'javascript:we_FileUpload.cancelUpload()', true, 170, 22, "", "", false, false, "_btn", true);
		...
		$divFileInput = we_html_element::htmlDiv(array('id' => 'div_we_File_fileInputWrapper', 'class' => 'we_fileInputWrapper', 'style' => 'height:23px;margin-top:22px;width:170px;'), $fileInput . $btnBrowse);
		...
		$divDropzone = !(self::isFallback() || self::isLegacyMode()) ? ('
			<div class="we_file_drag we_file_drag_binDoc" id="div_fileupload_fileDrag_state_0" style="' . (!$this->isDragAndDrop ? 'border-color:white;' : '') . '">
				...
				we_html_element::htmlDiv(array('class' => 'dropzone_right'),
					($thumbnailSmall ? $thumbnailSmall : we_html_element::htmlImg(array('src' => $this->binDocProperties['icon'])))) . '
			</div>
