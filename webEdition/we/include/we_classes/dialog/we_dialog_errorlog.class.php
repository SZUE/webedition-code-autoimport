...
abstract class we_dialog_errorlog{
  ...
  <tr class="first">
	...
	private static function getPosData($bt, $file, $lineNo){
		$ret = '';
		$matches = [];
	...
	public static function getDialog(){
		...
		switch(we_base_request::_(we_base_request::STRING, 'function', 'last')){
			...
			case 'export':
				...
				$sep = "\n" . str_repeat('-', 80) . "\n";
				if($cur){
					$cur['Source-Code'] = self::getPosData($cur['Backtrace'], $cur['File'], $cur['Line']);
				}
				$data = '';
...
' . $GLOBALS['DB_WE']->getInfo(false);
		...
		if($size && $cur){
			$cur['posData'] = self::getPosData($cur['Backtrace'], $cur['File'], $cur['Line']);
		}

		$data = self::getInfoTable($cur);
