	...
	function editor(){
		switch($this->EditPageNr){
			case we_base_constants::WE_EDITPAGE_THUMBNAILS:
				return new we_editor_thumbnails($this);
			default:
...
//						$create = false;
			...
			if((!empty($href)) && $inc_href){ //  use link with rollover
				...
				$this->html = (defined('WE_EDIT_IMAGE')) ?
					we_base_imageCrop::getCSS() . we_base_imageCrop::getCrop($attribs) . we_base_imageCrop::getJS($attribs) :
	...
	function formLink(){
		...
		$btnHrefExt = we_base_permission::hasPerm('CAN_SELECT_EXTERNAL_FILES') ? we_html_button::create_button(we_html_button::SELECT, "javascript:we_cmd('browse_server','" . $textname . "','',document.we_form.elements['" . $textname . "'].value,'check_radio_option," . $checkLinkname . ",1,1')") : '';
