...
class we_mail_MimeDecode{
	 ...
	 * @throws Exception
	 */
	public static function splitMessageStruct($message, $boundary, $EOL = we_mail_mime::LINEEND){
	 ...
	 * @return null
	 */
	public static function splitMessage($message, &$headers, &$body, $EOL = we_mail_mime::LINEEND){
	...
	$field, $wantedPart = null, $firstName = 0
		...
		if(!preg_match_all('%([^=\s]+)\s*=\s*("[^"]+"|[^;]+)(;\s*|$)%', $field, $matches)){
			throw new we_mail_exception('not a valid header field');
		}
